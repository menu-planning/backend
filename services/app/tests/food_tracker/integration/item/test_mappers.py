import uuid
from datetime import datetime

import pytest
from sqlalchemy import inspect
from sqlalchemy.ext.asyncio import AsyncSession
from src.contexts.food_tracker.shared.adapters.ORM.mappers.house import HouseMapper
from src.contexts.food_tracker.shared.adapters.ORM.mappers.item import ItemMapper
from src.contexts.food_tracker.shared.adapters.ORM.sa_models.houses import HouseSaModel
from src.contexts.food_tracker.shared.domain.entities.house import House
from src.contexts.food_tracker.shared.domain.entities.item import Item
from src.contexts.food_tracker.shared.domain.enums import Unit
from src.contexts.shared_kernel.domain.value_objects import Amount
from tests.food_tracker.random_refs import random_attr

pytestmark = [pytest.mark.anyio]


def list_of_members(n: int) -> list[str]:
    return [uuid.uuid4().hex for _ in range(n)]


def list_of_nutritionists(n: int) -> list[str]:
    return [uuid.uuid4().hex for _ in range(n)]


@pytest.mark.parametrize(
    "n_members,n_nutritionists,version",
    [
        (0, 0, 1),
        (2, 2, 3),
    ],
)
async def test_map_house(n_members, n_nutritionists, version) -> None:
    mapper = HouseMapper
    domain_model = House(
        id=random_attr("id"),
        owner_id=random_attr("id"),
        name=random_attr("name"),
        members_ids=list_of_members(n_members),
        nutritionists_ids=list_of_nutritionists(n_nutritionists),
        version=version,
    )
    sa_model = await mapper.map_domain_to_sa(None, domain_model)
    assert sa_model.id == domain_model.id
    assert sa_model.owner_id == domain_model.owner_id
    assert sa_model.name == domain_model.name
    assert sa_model.discarded == domain_model.discarded
    assert sa_model.version == domain_model.version
    assert len(sa_model.members) == n_members
    for id in domain_model.members_ids:
        assert id in [i.user_id for i in sa_model.members]
    assert len(sa_model.nutritionists) == n_nutritionists
    for id in domain_model.nutritionists_ids:
        assert id in [i.user_id for i in sa_model.nutritionists]

    reverted_domain = mapper.map_sa_to_domain(sa_model)
    assert reverted_domain == domain_model
    assert reverted_domain.id == domain_model.id
    assert reverted_domain.owner_id == domain_model.owner_id
    assert reverted_domain.name == domain_model.name
    assert reverted_domain.discarded == domain_model.discarded
    assert reverted_domain.version == domain_model.version
    assert len(reverted_domain.members_ids) == n_members
    for id in domain_model.members_ids:
        assert id in reverted_domain.members_ids
    assert len(reverted_domain.nutritionists_ids) == n_nutritionists
    for id in domain_model.nutritionists_ids:
        assert id in reverted_domain.nutritionists_ids


@pytest.mark.parametrize(
    "is_food,price_per_unit,barcode,cfe_key,product,nutri_facts,diet_types,process_type,score,ids_of_products_with_similar_names",
    [
        (None, None, None, None, None, None, None, None, None, None),
        (
            True,
            15.3,
            "1234567890123",
            "00001",
            True,
            True,
            set(["vegan", "vegetarian"]),
            "Óleos, gorduras, sal e açúcar",
            True,
            ["similar name"],
        ),
        (
            True,
            15.3,
            "1234567890123",
            "00001",
            True,
            True,
            set(["vegan", "vegetarian"]),
            "Óleos, gorduras, sal e açúcar",
            False,
            ["similar name"],
        ),
        (
            True,
            15.3,
            "1234567890123",
            "00001",
            True,
            False,
            set(["vegan", "vegetarian"]),
            "Óleos, gorduras, sal e açúcar",
            False,
            ["similar name"],
        ),
        (
            False,
            15.3,
            "1234567890123",
            "00001",
            True,
            False,
            set(),
            None,
            False,
            [],
        ),
        (
            False,
            15.3,
            "1234567890123",
            "00001",
            False,
            False,
            set(),
            None,
            False,
            [],
        ),
    ],
)
async def test_map_item(
    is_food,
    price_per_unit,
    barcode,
    cfe_key,
    product,
    nutri_facts,
    diet_types,
    process_type,
    score,
    ids_of_products_with_similar_names,
    async_pg_session: AsyncSession,
) -> None:
    mapper = ItemMapper
    domain_model = Item(
        id=random_attr("id"),
        date=datetime.now(),
        house_id=random_attr("house_id"),
        description=random_attr("description"),
        amount=Amount(
            quantity=150,
            unit=Unit.GRAM,
        ),
        is_food=is_food,
        price_per_unit=price_per_unit,
        barcode=barcode,
        cfe_key=cfe_key,
        product_id=(random_attr("id") if product else None),
        ids_of_productos_with_similar_names=ids_of_products_with_similar_names,
    )
    sa_model = await mapper.map_domain_to_sa(async_pg_session, domain_model)
    assert sa_model.id == domain_model.id
    assert sa_model.date == domain_model.date
    assert sa_model.house_id == domain_model.house_id
    assert sa_model.description == domain_model.description
    assert sa_model.amount.quantity == domain_model.amount.quantity
    assert sa_model.amount.unit == domain_model.amount.unit
    assert sa_model.is_food == domain_model.is_food
    assert sa_model.price_per_unit == domain_model.price_per_unit
    assert sa_model.barcode == domain_model.barcode
    assert sa_model.cfe_key == domain_model.cfe_key
    assert sa_model.discarded == domain_model.discarded
    assert sa_model.version == domain_model.version
    assert (
        sa_model.ids_of_products_with_similar_names
        == domain_model.ids_of_products_with_similar_names
    )
    assert sa_model.product_id == domain_model.product_id
