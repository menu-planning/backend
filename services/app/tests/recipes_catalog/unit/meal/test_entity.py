...
#     meal.create_recipe(**random_create_recipe_classmethod_kwargs(meal_id=meal.id, author_id=meal.author_id))
...
#     assert len(meal.recipes) == 0
#     # cmd = random_create_recipe_on_meal_kwargs()
#     recipe = meal.create_recipe(random_recipe(meal_id=meal.id, author_id=meal.author_id))
...
def test_can_calculate_nutri_facts_from_recipes():
    ...
    assert len(meal.recipes) == 1
    assert meal.nutri_facts == recipe1_kwargs["nutri_facts"]
    recipe2_kwargs = random_create_recipe_classmethod_kwargs(meal_id=meal.id, author_id=meal.author_id)
    meal.create_recipe(**recipe2_kwargs)
    assert len(meal.recipes) == 2
    type(meal).nutri_facts.fget.cache_clear()
    assert meal.nutri_facts == (recipe1_kwargs["nutri_facts"] + recipe2_kwargs["nutri_facts"])
