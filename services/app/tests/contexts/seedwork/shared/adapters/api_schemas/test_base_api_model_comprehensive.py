...
import src.contexts.seedwork.shared.adapters.api_schemas.base_api_model
...
class DomainTestEntity(Entity):
    ...
    def __init__(self, id: UUID, name: str, created_at: datetime, updated_at: datetime | None = None):
...
class TestTypeConversionUtility:
    ...
    def setup_method(self):
        ...
        self.test_datetime = datetime.now(timezone.utc)
    ...
    def test_conversion_round_trip_integrity(self):
        ...
        # UUID round-trip
        uuid_result = self.converter.string_to_uuid(self.converter.uuid_to_string(self.test_uuid))
...
class TestBaseApiModelConversionMethods:
    ...
    def setup_method(self):
        ...
        self.domain_command = DomainTestCommand(action="test_action", target_id=self.test_uuid)
    ...
    def test_to_domain_entity(self):
        ...
        assert domain_obj.name == "Test Entity"  # Should match the domain entity name from setup  
...
class TestBaseApiModelPerformance:
    ...
    def test_model_initialization_performance(self):
        ...
        assert avg_time_ms < 1.0, f"Average initialization time {avg_time_ms:.3f}ms exceeds 1ms limit"
    ...
    def test_conversion_method_performance(self):
        ...
        assert avg_from_domain_ms < 5.0, f"from_domain {avg_from_domain_ms:.3f}ms exceeds 5ms limit"
        assert avg_to_domain_ms < 5.0, f"to_domain {avg_to_domain_ms:.3f}ms exceeds 5ms limit"
        assert avg_from_orm_ms < 5.0, f"from_orm_model {avg_from_orm_ms:.3f}ms exceeds 5ms limit"
        assert avg_to_orm_ms < 5.0, f"to_orm_kwargs {avg_to_orm_ms:.3f}ms exceeds 5ms limit"
        
    def test_bulk_operations_performance(self):
        """Test bulk operations meet <100ms for 1000+ objects requirement."""
        # Create 1000 domain objects
        domain_objects = [
            DomainTestValueObject(value=f"test_{i}", number=i) 
            for i in range(1000)
        ]
        ...
        assert bulk_time_ms < 100.0, f"Bulk conversion {bulk_time_ms:.3f}ms exceeds 100ms limit"
    ...
    def test_complex_object_performance(self):
        ...
        avg_time_ms = ((end_time - start_time) / 100) * 1000
        assert avg_time_ms < 10.0, f"Complex object round-trip {avg_time_ms:.3f}ms exceeds 10ms limit"
...
class TestFieldValidationPatterns:
    ...
    @pytest.mark.parametrize("invalid_string_type", [
    ...
    @pytest.mark.parametrize("invalid_number_type", [
...
class TestEnhancedEntityPatterns:
    ...
    def test_validation_config_enforcement(self):
        ...
        entity = ApiTestEntity(id=str(uuid4()), name="Test", created_at=datetime.now(timezone.utc), updated_at=datetime.now(timezone.utc)) # type: ignore
...
class TestPerformanceMonitoring:
    ...
    def test_memory_usage_validation(self):
        ...
        assert memory_increase_mb < 50, f"Memory usage increased by {memory_increase_mb:.2f}MB, which is excessive"
    ...
    def test_validation_logging_functionality(self):
        ...
        with patch('src.logging.logger.logger') as mock_logger:
