...
class TestMealEntity(Entity):
    ...
    def __init__(self, id: str, name: str, **kwargs):
        ...
        for k, v in kwargs.items():
            if not hasattr(self, k) and k not in ['discarded', 'version', 'created_at', 'updated_at']:
                setattr(self, k, v)
...
class TestTagEntity(Entity):
    ...
    """
    def __init__(self, id: int, key: str, value: str, author_id: str, type: str, **kwargs):
        super().__init__(
            id=str(id),
            discarded=kwargs.get('discarded', False),
            version=kwargs.get('version', 1),
...
class TestRatingEntity(Entity):
    ...
    """
    def __init__(self, id: int, user_id: str, recipe_id: str, taste: int, convenience: int, **kwargs):
        super().__init__(
            id=str(id),
            discarded=kwargs.get('discarded', False),
            version=kwargs.get('version', 1),
...
class TestIngredientEntity(Entity):
    ...
    """
    def __init__(self, id: int, name: str, quantity: float, unit: str, recipe_id: str, **kwargs):
        super().__init__(
            id=str(id),
            discarded=kwargs.get('discarded', False),
            version=kwargs.get('version', 1),
