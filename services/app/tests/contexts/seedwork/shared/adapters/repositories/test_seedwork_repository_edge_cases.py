...
class TestSaGenericRepositoryEdgeCaseModels:
    ...
    async def test_circular_model_a_basic_query(self, circular_repository, test_session):
        ...
        results = await circular_repository.query(
            filter={"circular_a_name": "Test Circular A"}, 
    ...
    async def test_circular_model_with_reference(self, circular_repository, test_session):
        ...
        results = await circular_repository.query(
            filter={"circular_a_name": "Entity A1"}, 
            _return_sa_instance=True
        )
    ...
    async def test_self_referential_model_basic_operations(self, self_ref_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
    ...
    async def test_self_referential_model_parent_filtering(self, self_ref_repository, test_session):
        ...
        child_orm = create_test_ORM_self_ref(name="Child", level=1, parent_id=None)  # No parent reference
...
class TestSaGenericRepositoryFilterCombinations:
    ...
    async def test_conflicting_range_filters_real_data(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
    ...
    async def test_multiple_postfix_same_field_different_values(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
    ...
    async def test_list_and_scalar_filters_precedence(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
    ...
    async def test_none_value_filters_real_database(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
        """Test None value handling in real database queries"""
        # Given: Meal ORM instances with and without descriptions
        meal_orms = [
            create_test_ORM_meal(name="With description", description="Some description"),
        ...
        results = await meal_repository.query(filter={"description": None}, _return_sa_instance=True)
        
        # Then: Only the meal ORM without description is returned
        assert len(results) == 1
        assert results[0].name == "No description"

    async def test_empty_list_filters_real_database(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
        ...
        empty_in_results = await meal_repository.query(filter={"name": []}, _return_sa_instance=True)
        
        # Then: No results are returned for empty list filter
        assert len(empty_in_results) == 0

    async def test_zero_and_false_value_filters_real_database(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
        ...
        zero_results = await meal_repository.query(filter={"total_time": 0}, _return_sa_instance=True)
...
class TestSaGenericRepositoryInvalidFilters:
    ...
    async def test_unknown_filter_key_raises_exception(self, meal_repository):
        ...
        with pytest.raises(FilterValidationException):
            await meal_repository.query(filter={"unknown_field": "value"}, _return_sa_instance=True)
    ...
    async def test_invalid_postfix_combination_raises_exception(self, meal_repository):
        ...
        with pytest.raises(FilterValidationException):
            await meal_repository.query(filter={"total_time_invalid_postfix": 30}, _return_sa_instance=True)

    async def test_type_mismatch_handling_real_database(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
    ...
    async def test_malformed_filter_structure_handling(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
...
class TestSaGenericRepositoryConstraintViolations:
    ...
    async def test_unique_constraint_violation(self, meal_repository, test_session):
        ...
        meal_orm1 = create_test_ORM_meal(name="Meal 1", author_id="author1", menu_id="menu1")
        test_session.add(meal_orm1)
        await test_session.commit()
        
        # When: Trying to create another meal ORM with same unique constraint fields
        meal_orm2 = create_test_ORM_meal(name="Meal 1", author_id="author1", menu_id="menu1")
...
class TestSaGenericRepositoryFilterPrecedence:
    ...
    async def test_postfix_precedence_order_with_real_data(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
    ...
    async def test_filter_application_order_consistency_real_database(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
    ...
    async def test_complex_filter_precedence_with_joins(self, meal_repository, test_session):
        reset_counters()  # Ensure deterministic IDs
...
class TestSaGenericRepositoryBoundaryConditions:
    ...
    async def test_boundary_conditions(self, meal_repository, test_session, test_type, setup_data, filter_criteria, expected_count, validation_func):
        reset_counters()  # Ensure deterministic IDs
        ...
        results = await meal_repository.query(filter=filter_criteria, _return_sa_instance=True)
