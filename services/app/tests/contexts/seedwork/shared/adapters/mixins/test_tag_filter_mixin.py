"""
Unit tests for TagFilterMixin.

This module contains comprehensive tests for the TagFilterMixin class, including
edge cases, error conditions, and proper SQLAlchemy condition generation.

The tests focus on validating the core logic of the TagFilterMixin without 
...
class TestableTagFilterMixin(TagFilterMixin):
    """Concrete implementation of TagFilterMixin for testing."""
    
    tag_model = TagSaModel


class TestTagFilterMixin:
    """Test cases for TagFilterMixin functionality."""
    
    @pytest.fixture
    def mixin(self):
        """Create a testable mixin instance."""
        return TestableTagFilterMixin()
    ...
    @pytest.mark.parametrize("valid_tags", [
        ...
        [("type", "breakfast", "admin"), ("calories", "low", "admin"), ("time", "quick", "admin")]
    ])
    def test_validate_tag_format_valid_cases(self, mixin, valid_tags):
        """Test that valid tag formats pass validation without raising exceptions."""
        # Should not raise any exception
        mixin.validate_tag_format(valid_tags)

    @pytest.mark.parametrize("invalid_tags,expected_error_message", [
        # Not a list
        ...
        ([("cuisine", "italian", "user123", "extra")], "must have exactly 3 elements"),
    ...
    def test_validate_tag_format_invalid_cases(self, mixin, invalid_tags, expected_error_message):
        ...
        with pytest.raises(FilterNotAllowedError) as exc_info:
            mixin.validate_tag_format(invalid_tags)
        
        assert expected_error_message in str(exc_info.value)

    def test_validate_tag_format_complex_invalid_case(self, mixin):
    ...
    def test_build_tag_filter_empty_tags(self, mixin, mock_sa_model_class):
        """Test that empty tag list returns True (no filtering)."""
        result = mixin.build_tag_filter(mock_sa_model_class, [], "meal")
        assert result is True

    def test_build_tag_filter_model_without_tags_relationship(self, mixin, mock_sa_model_without_tags):
        """Test that model without tags relationship raises FilterNotAllowedError."""
        tags = [("cuisine", "italian", "user123")]
        
        with pytest.raises(FilterNotAllowedError) as exc_info:
            mixin.build_tag_filter(mock_sa_model_without_tags, tags, "meal")
    ...
    def test_build_tag_filter_single_tag(self, mock_and, mixin, mock_sa_model_class):
        """Test filtering with a single tag creates proper SQLAlchemy condition."""
        tags = [("cuisine", "italian", "user123")]
        mock_and.return_value = Mock()
        
        result = mixin.build_tag_filter(mock_sa_model_class, tags, "meal")
    ...
    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_build_tag_filter_multiple_tags_same_key(self, mock_and, mixin, mock_sa_model_class):
    ...
    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_build_tag_filter_multiple_tags_different_keys(self, mock_and, mixin, mock_sa_model_class):
        ...
        assert mock_and.call_count >= 2  # Called for each key group plus final combination
        assert result is not None

    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_build_tag_filter_complex_scenario(self, mock_and, mixin, mock_sa_model_class):
        ...
        assert mock_and.call_count >= 3  # Called for each key group plus final combination
        assert result is not None

    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    @pytest.mark.parametrize("tag_type", ["meal", "recipe", "menu", "custom"])
    def test_build_tag_filter_different_tag_types(self, mock_and, mixin, mock_sa_model_class, tag_type):
        """Test that different tag types are handled correctly."""
        tags = [("cuisine", "italian", "user123")]
        mock_and.return_value = Mock()
        
        result = mixin.build_tag_filter(mock_sa_model_class, tags, tag_type)
    ...
    def test_build_negative_tag_filter_empty_tags(self, mixin, mock_sa_model_class):
        """Test that empty tag list returns True (no filtering) for negative filter."""
        result = mixin.build_negative_tag_filter(mock_sa_model_class, [], "meal")
        assert result is True

    def test_build_negative_tag_filter_model_without_tags_relationship(self, mixin, mock_sa_model_without_tags):
        """Test that model without tags relationship raises FilterNotAllowedError."""
        tags = [("cuisine", "spicy", "user123")]
        
        with pytest.raises(FilterNotAllowedError) as exc_info:
            mixin.build_negative_tag_filter(mock_sa_model_without_tags, tags, "meal")
        
        assert "does not have a 'tags' relationship" in str(exc_info.value)

    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_build_negative_tag_filter_single_tag(self, mock_and, mixin, mock_sa_model_class):
        ...
        result = mixin.build_negative_tag_filter(mock_sa_model_class, tags, "meal")
    ...
    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_build_negative_tag_filter_multiple_tags(self, mock_and, mixin, mock_sa_model_class):
    ...
    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_tag_filter_integration_workflow(self, mock_and, mixin, mock_sa_model_class):
        ...
        # First validate
        mixin.validate_tag_format(tags)  # Should not raise
        
        # Then create positive filter
        positive_condition = mixin.build_tag_filter(mock_sa_model_class, tags, "meal")
        assert positive_condition is not None
        
        # And negative filter
        negative_condition = mixin.build_negative_tag_filter(mock_sa_model_class, tags, "meal")
        assert negative_condition is not None

    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_tag_filter_with_invalid_then_valid_tags(self, mock_and, mixin, mock_sa_model_class):
        ...
        mixin.validate_tag_format(valid_tags)
        result = mixin.build_tag_filter(mock_sa_model_class, valid_tags, "meal")
        assert result is not None
    ...
    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_tag_filter_with_special_characters(self, mock_and, mixin, mock_sa_model_class):
        ...
        mixin.validate_tag_format(tags)
    ...
    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_tag_filter_with_empty_strings(self, mock_and, mixin, mock_sa_model_class):
    ...
    @patch('src.contexts.seedwork.shared.adapters.mixins.tag_filter_mixin.and_')
    def test_tag_filter_case_sensitivity(self, mock_and, mixin, mock_sa_model_class):
...
@pytest.mark.parametrize("tag_format_scenario", [
    {
    ...
    },
    {
        ...
        "tags": [("cuisine", "italian", "user123"), ("difficulty", "easy", "user456")],
        "should_pass": True,
