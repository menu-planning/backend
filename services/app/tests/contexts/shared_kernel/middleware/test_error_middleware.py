...
class TestErrorMiddleware:
    ...
    async def test_successful_handler_passthrough(self, basic_middleware, mock_event):
        ...
        with patch('src.contexts.shared_kernel.middleware.error_middleware.correlation_id_ctx') as mock_ctx:
    ...
    async def test_exception_handling_basic(self, basic_middleware, mock_event):
        ...
        with patch('src.contexts.shared_kernel.middleware.error_middleware.correlation_id_ctx') as mock_ctx:
            ...
            assert "occurred while processing" in error_body["message"]  # Default safe message
    ...
    async def test_exception_categorization_by_message(self, basic_middleware, mock_event):
    ...
    async def test_pydantic_validation_error_handling(self, basic_middleware, mock_event):
        ...
        with patch('src.contexts.shared_kernel.middleware.error_middleware.correlation_id_ctx') as mock_ctx:
            ...
            assert len(error_body["errors"]) >= 3  # Should have validation errors for all fields
    ...
    async def test_production_error_safety(self, basic_middleware, mock_event):
        ...
        with patch('src.contexts.shared_kernel.middleware.error_middleware.correlation_id_ctx') as mock_ctx:
            ...
            assert "Internal system error with sensitive data" not in error_body["message"]
            assert error_body["message"] == "An error occurred while processing your request"
    ...
    async def test_cors_headers_in_error_response(self, basic_middleware, mock_event):
        ...
        with patch('src.contexts.shared_kernel.middleware.error_middleware.correlation_id_ctx') as mock_ctx:
            ...
            assert "DELETE,GET,HEAD,OPTIONS,PATCH,POST,PUT" in headers["Access-Control-Allow-Methods"]
