...
class MockSeedUser:
    ...
    def __init__(self, user_id: Optional[str] = None, permissions: Optional[dict] = None):
...
class TestAuthContext:
    ...
    def test_has_permission_authenticated_user(self):
        ...
        user_id = str(uuid4())
        mock_user = MockSeedUser(user_id, permissions={
...
class TestUnifiedIAMProvider:
    ...
    async def test_get_user_success(self, async_pg_session: AsyncSession, iam_provider):
        ...
        await async_pg_session.execute(text("""
    ...
    async def test_get_user_cache_different_contexts(self, async_pg_session: AsyncSession, iam_provider):
    ...
    async def test_role_context_isolation(self, async_pg_session: AsyncSession, iam_provider):
        ...
        # Should NOT have permissions from other contexts
        assert "access_developer_tools" not in recipes_role.permissions  # products permission
...
class TestAuthMiddleware:
    ...
    async def test_successful_authentication(self, mock_get_user, auth_middleware, lambda_event, mock_handler):
    ...
    @patch.dict(os.environ, {"IS_LOCALSTACK": "false"})
    @patch.object(UnifiedIAMProvider, 'get_user')
    async def test_authentication_failure(self, mock_get_user, auth_middleware, lambda_event, mock_handler):
    ...
    @patch.dict(os.environ, {"IS_LOCALSTACK": "false"})
    @patch.object(UnifiedIAMProvider, 'get_user')
    async def test_optional_authentication_success(self, mock_get_user, optional_auth_middleware_instance, lambda_event, mock_handler):
    ...
    @patch.dict(os.environ, {"IS_LOCALSTACK": "false"})
    async def test_optional_authentication_no_user(self, optional_auth_middleware_instance, mock_handler):
    ...
    @patch.dict(os.environ, {"IS_LOCALSTACK": "false"})
    @patch.object(UnifiedIAMProvider, 'get_user')
    async def test_authorization_error_handling(self, mock_get_user, auth_middleware, lambda_event):
    ...
    @patch.dict(os.environ, {"IS_LOCALSTACK": "false"})
    @patch.object(UnifiedIAMProvider, 'get_user')
    async def test_other_exception_propagation(self, mock_get_user, auth_middleware, lambda_event):
...
class TestFactoryFunctions:
    ...
    def test_iam_auth_middleware(self):
        ...
        assert isinstance(middleware, AuthMiddleware)
        assert middleware.caller_context == "recipes_catalog"  # IAM operations use recipes_catalog context
        assert middleware.require_authentication is True
...
class TestAuthIntegration:
    ...
    @patch.dict(os.environ, {"IS_LOCALSTACK": "false"})
    @patch.object(UnifiedIAMProvider, 'get_user')
    async def test_full_auth_flow_with_permission_check(self, mock_get_user, lambda_event):
        """Test complete authentication and authorization flow."""
        user_id = lambda_event["requestContext"]["authorizer"]["claims"]["sub"]
        mock_user = MockSeedUser(
            user_id, 
            permissions={"standard": ["read_products", "write_products"]}
        )
        mock_get_user.return_value = {
            "statusCode": 200,
            "body": mock_user
        ...
        async def protected_handler(event):
            ...
            return {
                "statusCode": 200,
                "body": json.dumps({"message": "success", "user": auth_context.user_id})
            }
    ...
    async def test_owner_or_permission_flow(self, mock_get_user, lambda_event):
        ...
        async def owner_protected_handler(event):
            ...
            if not auth_context or not auth_context.is_owner_or_has_permission(resource_owner_id, "admin"):
