...
class TestSignatureVerificationStress:
    ...
    async def test_concurrent_signature_verification_load(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            tasks = [verify_payload(payload_str, headers) for payload_str, headers in test_payloads]
            ...
            assert verifications_per_second > 50, f"Performance too low: {verifications_per_second} req/s"
            
            print(f"Concurrent verification performance: {verifications_per_second:.2f} verifications/sec")
    ...
    async def test_signature_verification_under_memory_pressure(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            assert all(verification_results), "All large payload signatures should verify"
    ...
    async def test_rapid_sequential_verification_stress(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            assert avg_verification_time < 0.01, f"Average verification too slow: {avg_verification_time:.4f}s"
            assert max_verification_time < 0.1, f"Max verification too slow: {max_verification_time:.4f}s"
            assert verifications_per_second > 100, f"Throughput too low: {verifications_per_second:.2f} req/s"
            
            print(f"Rapid verification: {verifications_per_second:.2f} req/s, avg: {avg_verification_time*1000:.2f}ms, max: {max_verification_time*1000:.2f}ms")
    ...
    async def test_signature_verification_with_malformed_edge_cases(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            for malformed_sig in malformed_signatures:
                ...
                assert not is_valid, f"Malformed signature unexpectedly valid: {malformed_sig[:50]}..."
                assert verification_time < 1.0, f"Verification too slow for malformed sig: {verification_time:.4f}s"
            
            # All malformed signatures should complete quickly
            max_verification_time = max(verification_times)
            avg_verification_time = sum(verification_times) / len(verification_times)
            
            assert max_verification_time < 0.5, f"Max malformed verification too slow: {max_verification_time:.4f}s"
            assert avg_verification_time < 0.1, f"Avg malformed verification too slow: {avg_verification_time:.4f}s"
            
            print(f"Malformed signature handling: avg {avg_verification_time*1000:.2f}ms, max {max_verification_time*1000:.2f}ms")
    ...
    async def test_signature_verification_timing_consistency(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            test_scenarios = [
                ...
                ("half_correct", correct_signature[:30] + "a" * (len(correct_signature) - 30)),
            ...
            for scenario_name, signature in test_scenarios:
                ...
                for _ in range(50):
                    ...
                    assert is_valid == expected_valid, f"Unexpected validation result for {scenario_name}"
            ...
            invalid_scenarios = [name for name in timing_results.keys() if name != "correct_signature"]
            invalid_avg_times = [timing_results[name]["avg"] for name in invalid_scenarios]
            ...
            assert timing_variance < 0.05, f"Timing variance too high: {timing_variance:.6f}s"
            
            # All timings should be reasonably fast
            for scenario_name, results in timing_results.items():
                assert results["max"] < 0.1, f"{scenario_name} max time too slow: {results['max']:.6f}s"
            
            print(f"Timing consistency verified - variance: {timing_variance*1000:.2f}ms")
        ...
        async with async_benchmark_timer() as timer:
            ...
            for cycle in range(cycles):
                ...
                for i in range(verifications_per_cycle):
                    ...
                    # Verify signature
                    is_valid, _ = await verifier.verify_webhook_request(payload_str, headers)
                    assert is_valid, f"Verification failed at cycle {cycle}, iteration {i}"
            ...
            if len(memory_samples) > 2:
                ...
                assert abs(slope) < 0.5, f"Potential memory leak detected - growth rate: {slope:.3f}MB/cycle"
            
            # Total memory growth should be reasonable
            assert memory_growth < 50, f"Excessive memory growth: {memory_growth:.2f}MB"
            
            print(f"Memory leak test passed - growth: {memory_growth:.2f}MB over {cycles} cycles")
    ...
    async def test_concurrent_mixed_valid_invalid_signatures(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            start_time = time.perf_counter()
            tasks = [verify_signature(payload_str, headers) for payload_str, headers in verification_tasks]
            results = await asyncio.gather(*tasks)
            end_time = time.perf_counter()
            ...
            for i, (actual, expected) in enumerate(zip(results, expected_results)):
                assert actual == expected, f"Verification {i} mismatch: expected {expected}, got {actual}"
            ...
            assert verifications_per_second > 30, f"Mixed verification performance too low: {verifications_per_second:.2f} req/s"
            
            # Count valid vs invalid
            valid_count = sum(expected_results)
            invalid_count = len(expected_results) - valid_count
            
            print(f"Mixed verification: {verifications_per_second:.2f} req/s ({valid_count} valid, {invalid_count} invalid)")
    ...
    async def test_extreme_payload_sizes(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            for size_name, content_size in size_tests:
                ...
                assert verification_time < max_time, f"{size_name} verification too slow: {verification_time:.4f}s"
                
                print(f"{size_name} payload ({actual_size} bytes): {verification_time*1000:.2f}ms")
    ...
    async def test_signature_verification_error_handling(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            error_scenarios = [
                ...
                ("invalid_json", {"typeform-signature": "sha256=test"}, "{invalid json"),
                
                # None values
                ("none_payload", {"typeform-signature": "sha256=test"}, None),
                
                # Binary payload
                ("binary_payload", {"typeform-signature": "sha256=test"}, b"binary data"),
            ]
            ...
            for scenario_name, headers, payload in error_scenarios:
                ...
                try:
                    ...
                    is_valid, error_msg = await verifier.verify_webhook_request(payload, headers)
                    
                    # Should be invalid with descriptive error
                    assert not is_valid, f"Error scenario {scenario_name} unexpectedly valid"
                    assert error_msg is not None, f"Error scenario {scenario_name} missing error message"
                ...
                assert verification_time < 0.1, f"Error handling too slow for {scenario_name}: {verification_time:.4f}s"
                
                print(f"Error scenario '{scenario_name}': {verification_time*1000:.2f}ms")
    ...
    async def test_signature_verification_under_cpu_stress(self, async_benchmark_timer):
        ...
        async with async_benchmark_timer() as timer:
            ...
            with ThreadPoolExecutor(max_workers=cpu_cores) as executor:
                ...
                stress_futures = [executor.submit(cpu_stress_worker) for _ in range(cpu_cores)]
            ...
            assert avg_time < 0.1, f"Average verification time too slow under CPU stress: {avg_time:.4f}s"
            assert max_time < 0.5, f"Max verification time too slow under CPU stress: {max_time:.4f}s"
            
            print(f"CPU stress test: avg {avg_time*1000:.2f}ms, max {max_time*1000:.2f}ms")