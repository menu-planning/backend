...
class TransactionalRepository:
    ...
    def __getattr__(self, name):
            ...
            async def buffered_operation(*args, **kwargs):
                if self._committed:
                    ...
                    if name == 'create':
                        ...
                        entity = await self._create_entity_without_storage(attr, args, kwargs)
                        # Buffer a custom add operation with the created entity
                        add_method = getattr(self._shared_repo, 'add')
        ...
        elif name in ['get_by_typeform_id', 'get_by_id', 'get_all']:
                ...
                if not result and name == 'get_by_typeform_id' and args:
                    ...
                    for op_name, op_args, op_kwargs, op_attr in self._operations:
                        if op_name == 'add' and op_args:
                            entity = op_args[0]
                            if hasattr(entity, 'typeform_id') and entity.typeform_id == typeform_id:
                                return entity
...
class FakeUnitOfWork(UnitOfWork):
    ...
    def __init__(self):
        ...
        self.onboarding_forms = TransactionalRepository(FakeUnitOfWork._shared_onboarding_forms)
        self.form_responses = TransactionalRepository(FakeUnitOfWork._shared_form_responses)
    ...
    def _populate_replay_protection_forms(self):
        ...
        if FakeUnitOfWork._shared_onboarding_forms:
            for form in replay_forms:
                ...
                FakeUnitOfWork._shared_onboarding_forms._typeform_lookup[form.typeform_id] = form.id
