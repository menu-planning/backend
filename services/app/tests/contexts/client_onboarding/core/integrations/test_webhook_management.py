...
class TestWebhookManagerIntegration:
    ...
    async def test_setup_onboarding_form_webhook_creates_new_webhook(self):
        ...
        # Act
        onboarding_form, webhook_info = await self.webhook_manager.setup_onboarding_form_webhook(
            uow=self.fake_uow,
            user_id=user_id,
    ...
    async def test_setup_onboarding_form_webhook_updates_existing_webhook(self):
        ...
        self.fake_api.create_webhook(typeform_id, {
            "url": original_url,
    ...
    async def test_update_webhook_url_success(self):
        ...
        updated_form = await self.fake_uow.onboarding_forms.get_by_id(onboarding_form.id)
    ...
    async def test_delete_webhook_configuration(self):
        ...
        retrieved_form = await self.fake_uow.onboarding_forms.get_by_id(onboarding_form.id)
        ...
        client_onboarding_webhooks = [
            w for w in api_webhooks["items"] 
            if w.get("tag") == "client_onboarding"
        ]
