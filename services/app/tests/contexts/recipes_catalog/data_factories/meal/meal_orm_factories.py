        ...
        for meal_idx in range(count):
            for tag_idx in range(tags_per_meal):
                ...
                if tag_key not in unique_tags:
                    tag = create_meal_tag_orm(key=key, value=value, author_id=author_id, type=tag_type)
...
def create_test_meal_dataset_orm(meal_count: int = 100, tags_per_meal: int = 0) -> Dict[str, Any]:
