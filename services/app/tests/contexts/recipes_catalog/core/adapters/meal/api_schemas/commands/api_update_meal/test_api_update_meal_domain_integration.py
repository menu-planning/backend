    ...
    def test_round_trip_consistency_comprehensive(self):
        ...
        try:
            ...
            for scenario_name, scenario_meal in test_scenarios.items():
                ...
                assert domain_update.meal_id == scenario_meal.id, f"Failed for {scenario_name}"
                assert isinstance(domain_update.updates, dict), f"Failed for {scenario_name}"
                
                # Verify key fields are preserved
                assert domain_update.updates["name"] == scenario_meal.name, f"Failed for {scenario_name}"
                assert domain_update.updates["description"] == scenario_meal.description, f"Failed for {scenario_name}"
        ...
        try:
            ...
            for scenario_name, scenario_meal in type_conversion_scenarios.items():
                ...
                if "recipes" in domain_update.updates:
                    assert isinstance(domain_update.updates["recipes"], list), f"Failed for {scenario_name}"
                ...
                if "tags" in domain_update.updates:
                    assert isinstance(domain_update.updates["tags"], set), f"Failed for {scenario_name}"
                ...
                if "name" in domain_update.updates:
                    assert isinstance(domain_update.updates["name"], str), f"Failed for {scenario_name}"
