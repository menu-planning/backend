...
class TestApiMealBasics:
    ...
    def test_from_domain_basic_conversion(self, domain_meal):
        ...
        excluded_fields = {
            'recipes', 'tags', 'nutri_facts',  # Tested in nested objects test
            'weight_in_grams', 'calorie_density', 'carbo_percentage', 'protein_percentage', 'total_fat_percentage'  # Tested in computed properties test
        }
    ...
    def test_from_domain_nested_objects_conversion(self, domain_meal):
            ...
            if field_name == 'recipes':
                ...
                if domain_value:
                    ...
                    for nutri_field in ApiNutriFacts.model_fields.keys():
                        ...
                        if api_nutri_value is not None and domain_nutri_value is not None:
        ...
        assert len(known_nested_objects) == 0, f"All nested objects should be tested, but these were missed: {known_nested_objects}"
    ...
    def test_from_domain_computed_properties(self, complex_meal):
        ...
        for field_name in api_model_fields:
            ...
            if field_name == 'nutri_facts':
                ...
                if api_value is not None and domain_value is not None:
                    ...
                    assert False, f"nutri_facts null: API={api_value}, Domain={domain_value}"
                ...
                if api_value is not None and domain_value is not None:
                    assert abs(api_value - domain_value) < 0.001, \
                        f"{field_name} mismatch: API={api_value}, Domain={domain_value}"
                else:
                    assert False, f"{field_name} null: API={api_value}, Domain={domain_value}"
                ...
                if hasattr(api_value, '__class__') and 'HttpUrl' in str(type(api_value)):
                    assert str(api_value) == str(domain_value), \
                        f"{field_name} mismatch: API={api_value}, Domain={domain_value}"
                else:
                    assert api_value == domain_value, \
                        f"{field_name} mismatch: API={api_value}, Domain={domain_value}"
        ...
        assert len(known_computed_properties) == 0, f"All computed properties should be compared, but"
    ...
    def test_to_domain_basic_conversion(self, simple_api_meal):
        ...
        # Define fields to exclude from basic conversion test (tested in other specific tests)
        excluded_fields = {
            'recipes', 'tags',  # Tested in collection conversion test
            'nutri_facts',  # Tested in nutrition facts conversion test
            'weight_in_grams', 'calorie_density', 'carbo_percentage', 'protein_percentage', 'total_fat_percentage'  # Computed properties
        }
        ...
        for field_name in api_model_fields:
            ...
            if not hasattr(simple_api_meal, field_name) or not hasattr(domain_meal, field_name):
                continue
            ...
            if isinstance(api_value, (str, int, float, bool)) or api_value is None:
                ...
                assert api_value.value == domain_value.value if hasattr(domain_value, 'value') else domain_value, \
                    f"Enum field '{field_name}' mismatch: API({api_value}) != Domain({domain_value})"
            else:
                # For other types, ensure conversion happened properly
                assert domain_value is not None or api_value is None, \
        ...
        assert domain_meal.has_same_content(expected_domain), "Domain meal conversion should be deterministic"
    ...
    def test_to_domain_collection_type_conversion(self, complex_api_meal):
        ...
        for field_name in api_model_fields:
            ...
            if field_name == 'recipes':
                ...
                assert len(domain_value) == len(api_value), f"Recipes count should match"
                
                # Each recipe should be properly converted to domain Recipe
                for i, (api_recipe, domain_recipe) in enumerate(zip(api_value, domain_value)):
                    assert isinstance(api_recipe, ApiRecipe), f"API recipe[{i}] should be ApiRecipe"
                    # Domain recipe type checking depends on the domain implementation
                    assert domain_recipe is not None, f"Domain recipe[{i}] should not be None"
                ...
                domain_tag_contents = {(tag.key, tag.value, tag.type) for tag in domain_value}
                assert api_tag_contents == domain_tag_contents, f"Tag contents should be preserved across conversion"
                
        # Ensure we tested all known collections
        assert len(known_collections) == 0, f"All collections should be tested, but these were missed: {known_collections}"
    ...
    def test_to_domain_nutrition_facts_conversion(self, complex_api_meal):
        ...
        expected_protein = sum(recipe.nutri_facts.protein.value if recipe.nutri_facts else 0 
                              for recipe in complex_api_meal.recipes)
        expected_carbs = sum(recipe.nutri_facts.carbohydrate.value if recipe.nutri_facts else 0 
                            for recipe in complex_api_meal.recipes)
        ...
        for field_name in api_model_fields:
            ...
            if field_name == 'recipes':
                ...
                if orm_value is not None:
                    assert len(api_value) == len(orm_value), f"Recipes count should match ORM"
                    # Each recipe should be converted to ApiRecipe
                    assert all(isinstance(recipe, ApiRecipe) for recipe in api_value), \
                        f"All recipes should be ApiRecipe instances"
                else:
                    assert api_value == [], f"API recipes should be empty list when ORM is None"
                ...
                assert isinstance(api_value, frozenset), f"API tags should be frozenset"
                if orm_value is not None:
                    assert len(api_value) == len(orm_value), f"Tags count should match ORM"
                    # Each tag should be converted to ApiTag
                    assert all(isinstance(tag, ApiTag) for tag in api_value), \
                        f"All tags should be ApiTag instances"
                else:
                    assert api_value == frozenset(), f"API tags should be empty frozenset when ORM is None"
                
            elif field_name == 'nutri_facts':
                known_nested_objects.remove('nutri_facts')
                # Should handle nutrition facts conversion properly
                if orm_value is not None:
                    assert isinstance(api_value, ApiNutriFacts), f"API nutri_facts should be ApiNutriFacts"
                    # Verify nested nutritional values are properly converted
                    for nutri_field in ApiNutriFacts.model_fields.keys():
                        api_nutri_value = getattr(api_value, nutri_field)
                        orm_nutri_value = getattr(orm_value, nutri_field, None)
                        if orm_nutri_value is not None:
                            assert api_nutri_value is not None, \
                    ...
                    assert api_value is None, f"API nutri_facts should be None when ORM is None"
    ...
    def test_from_orm_model_computed_properties(self, real_orm_meal):
        ...
        for field_name in api_model_fields:
            ...
            if field_name == 'nutri_facts':
                ...
                if api_value is not None and orm_value is not None:
                    ...
                    assert False, f"nutri_facts mismatch: API={api_value}, ORM={orm_value}"
                    
            elif field_name in known_computed_properties:
                known_computed_properties.remove(field_name)
                # Special handling for float computed properties - use approximate comparison
                if api_value is not None and orm_value is not None:
                    assert abs(api_value - orm_value) < 0.001, \
                        f"{field_name} mismatch: API={api_value}, ORM={orm_value}"
                elif api_value is None and orm_value is None:
                    # Both None is acceptable
                    pass
                else:
                    assert False, f"{field_name} null mismatch: API={api_value}, ORM={orm_value}"
        ...
        remaining_computed_props = {prop for prop in known_computed_properties if hasattr(real_orm_meal, prop)}
        assert len(remaining_computed_props) == 0, f"All computed properties should be compared, but these were missed: {remaining_computed_props}"
    ...
    def test_to_orm_kwargs_nested_objects_conversion(self, complex_api_meal):
        ...
        for field_name in api_model_fields:
            ...
            if field_name == 'recipes':
                ...
                assert isinstance(kwargs[field_name], list), f"recipes should be list in ORM kwargs"
                assert len(kwargs[field_name]) == len(api_value), f"recipes count should match"
                
                # Each recipe should be converted to dict/kwargs format
                for i, recipe_kwargs in enumerate(kwargs[field_name]):
                    assert isinstance(recipe_kwargs, dict), f"recipe[{i}] should be dict in ORM kwargs"
                    # Basic structure check - should have key fields
                    assert "id" in recipe_kwargs, f"recipe[{i}] should have 'id' field"
                    assert "name" in recipe_kwargs, f"recipe[{i}] should have 'name' field"
                
            elif field_name == 'tags':
                known_nested_objects.remove('tags')
                # Tags should be converted from frozenset to list of kwargs
                assert isinstance(kwargs[field_name], list), f"tags should be list in ORM kwargs"
                assert len(kwargs[field_name]) == len(api_value), f"tags count should match"
                
                # Each tag should be converted to dict/kwargs format
                for i, tag_kwargs in enumerate(kwargs[field_name]):
                    assert isinstance(tag_kwargs, dict), f"tag[{i}] should be dict in ORM kwargs"
                    # Basic structure check - should have key fields
                ...
                if api_value is not None:
                    ...
                    assert isinstance(kwargs[field_name], NutriFactsSaModel), f"nutri_facts should be NutriFactsSaModel in ORM kwargs"
                    ...
                    for nutri_field in ApiNutriFacts.model_fields.keys():
                        ...
                        if api_nutri_value is not None:
                            assert orm_nutri_value == api_nutri_value.value, \
                                f"nutri_facts.{nutri_field} should be extracted from ApiNutriValue"
                else:
                    assert kwargs[field_name] is None, f"nutri_facts should be None in ORM kwargs when API value is None"
    ...
    def test_to_orm_kwargs_computed_properties_conversion(self, complex_api_meal):
        ...
        for field_name in api_model_fields:
            ...
            if field_name in known_computed_properties:
                ...
                assert field_name in kwargs, f"Computed property '{field_name}' should be in ORM kwargs"
                ...
                if field_name == 'nutri_facts':
                    if api_value is not None:
                        from src.contexts.shared_kernel.adapters.ORM.sa_models.nutri_facts_sa_model import NutriFactsSaModel
                        assert isinstance(orm_value, NutriFactsSaModel), f"nutri_facts should be converted to NutriFactsSaModel"
                    else:
                        assert orm_value is None, f"nutri_facts should be None when API value is None"
                        
                # Special handling for float computed properties
                elif field_name in ['weight_in_grams', 'calorie_density', 'carbo_percentage', 'protein_percentage', 'total_fat_percentage']:
                    if api_value is not None:
                        assert abs(api_value - orm_value) < 0.001, \
                            f"Computed property '{field_name}' mismatch: API={api_value}, ORM={orm_value}"
                    else:
                        assert orm_value is None, f"Computed property '{field_name}' should be None when API value is None"
        ...
        assert len(known_computed_properties) == 0, f"All computed properties should be tested, but these were missed: {known_computed_properties}"
...
class TestApiMealRoundTrip:
    ...
    def test_api_to_domain_to_api_round_trip(self, complex_api_meal):
        ...
        if recovered_api.nutri_facts and expected_calories > 0:
            assert abs(recovered_api.nutri_facts.calories.value - expected_calories) < 0.1
    ...
    def test_orm_to_api_to_orm_round_trip(self, real_orm_meal):
        ...
        for field_name in api_model_fields:
            ...
            if isinstance(original_orm_value, (str, int, float, bool)) or original_orm_value is None:
                assert original_orm_value == round_trip_orm_value, \
                ...
                assert original_orm_value.value == round_trip_orm_value, \
                ...
                assert field_name in orm_kwargs, f"Field '{field_name}' should be present in round-trip ORM kwargs"
    ...
    def test_complete_four_layer_round_trip(self, simple_api_meal):
        ...
        assert domain_meal.nutri_facts.saturated_fat.value == original_api.nutri_facts.saturated_fat.value
        
        # Domain → API
        api_from_domain = ApiMeal.from_domain(domain_meal)
        
        # Use comprehensive API equality for the API comparison
        assert api_from_domain == original_api, "Four-layer round-trip should preserve API data"
        ...
        for field_name in api_model_fields:
            ...
            if isinstance(original_api_value, (str, int, float, bool)) or original_api_value is None:
                assert original_api_value == final_orm_value, \
                ...
                assert original_api_value.value == final_orm_value, \
                ...
                assert field_name in orm_kwargs, f"Field '{field_name}' should be present in final ORM kwargs"
    ...
    def test_round_trip_with_edge_cases(self, edge_case_meals, case_name):
        ...
        assert recovered_api == api_meal, f"Round-trip failed for edge case: {case_name}"
    ...
    def test_round_trip_preserves_computed_properties(self, complex_api_meal):
        ...
        assert recovered_api == complex_api_meal, "Round-trip should preserve all computed properties"
    ...
    def test_round_trip_with_nested_nutrition_facts(self, complex_api_meal):
        ...
        if complex_api_meal.nutri_facts:
            ...
            assert recovered_api == complex_api_meal, "Round-trip should preserve nutrition facts"
    ...
    def test_round_trip_with_complex_recipes(self, complex_api_meal):
        ...
        assert recovered_api == complex_api_meal, "Round-trip should preserve recipe collections"
    ...
    def test_round_trip_with_complex_tags(self, complex_api_meal):
        ...
        assert recovered_api == complex_api_meal, "Round-trip should preserve tag collections"
...
class TestApiMealComputedProperties:
    ...
    def test_computed_properties_correction_round_trip(self):
        ...
        original_calories = original_api_meal.nutri_facts.calories.value if original_api_meal.nutri_facts else 0
        ...
        expected_carbs = sum(recipe.nutri_facts.carbohydrate.value if recipe.nutri_facts else 0 
                            for recipe in original_api_meal.recipes)
        expected_fat = sum(recipe.nutri_facts.total_fat.value if recipe.nutri_facts else 0 
                          for recipe in original_api_meal.recipes)
        expected_weight = sum(recipe.weight_in_grams if recipe.weight_in_grams else 0 
                             for recipe in original_api_meal.recipes)
        ...
        assert abs(corrected_api_meal.nutri_facts.calories.value - expected_calories) < 0.1
        assert abs(corrected_api_meal.nutri_facts.protein.value - expected_protein) < 0.1
        assert abs(corrected_api_meal.nutri_facts.carbohydrate.value - expected_carbs) < 0.1
        ...
        if expected_calories != original_calories:
            assert abs(corrected_api_meal.nutri_facts.calories.value - original_calories) > 0.1
    ...
    def test_computed_nutrition_facts_aggregation(self):
        ...
        if corrected_api_meal.nutri_facts and corrected_api_meal.recipes:
            ...
            if expected_calories > 0:
                assert corrected_api_meal.nutri_facts.calories is not None and abs(corrected_api_meal.nutri_facts.calories.value - expected_calories) < 10
            if expected_protein > 0:
                assert corrected_api_meal.nutri_facts.protein is not None and abs(corrected_api_meal.nutri_facts.protein.value - expected_protein) < 5
            if expected_carbs > 0:
                assert corrected_api_meal.nutri_facts.carbohydrate is not None and abs(corrected_api_meal.nutri_facts.carbohydrate.value - expected_carbs) < 5
            if expected_fat > 0:
                assert corrected_api_meal.nutri_facts.total_fat is not None and abs(corrected_api_meal.nutri_facts.total_fat.value - expected_fat) < 5
    ...
    def test_computed_weight_aggregation(self):
        ...
        if corrected_api_meal.recipes:
            ...
            expected_weight = sum(recipe.weight_in_grams or 0 for recipe in corrected_api_meal.recipes)
    ...
    def test_computed_calorie_density_calculation(self):
        ...
        if corrected_api_meal.nutri_facts and corrected_api_meal.weight_in_grams and corrected_api_meal.weight_in_grams > 0 and corrected_api_meal.nutri_facts.calories is not None:
            # Calculate expected calorie density
            expected_calorie_density = (corrected_api_meal.nutri_facts.calories.value / corrected_api_meal.weight_in_grams) * 100
            
            # Verify calorie density matches expectations (within reasonable tolerance)
            if corrected_api_meal.calorie_density is not None:
                assert abs(corrected_api_meal.calorie_density - expected_calorie_density) < 1.0
            ...
            if protein is not None and carbs is not None and fat is not None:
                ...
                if total_macros > 0:
                    ...
                    if corrected_api_meal.protein_percentage is not None:
                        assert abs(corrected_api_meal.protein_percentage - expected_protein_percentage) < 1.0
                    if corrected_api_meal.carbo_percentage is not None:
                        assert abs(corrected_api_meal.carbo_percentage - expected_carbo_percentage) < 1.0
                    if corrected_api_meal.total_fat_percentage is not None:
                        assert abs(corrected_api_meal.total_fat_percentage - expected_fat_percentage) < 1.0
    ...
    def test_computed_properties_with_edge_cases(self, edge_case_meals):
        ...
        assert corrected_max_meal.weight_in_grams is not None and corrected_max_meal.weight_in_grams >= 0
        if corrected_max_meal.nutri_facts:
            assert corrected_max_meal.nutri_facts.calories is not None and corrected_max_meal.nutri_facts.calories.value >= 0
            assert corrected_max_meal.nutri_facts.protein is not None and corrected_max_meal.nutri_facts.protein.value >= 0
            assert corrected_max_meal.nutri_facts.carbohydrate is not None and corrected_max_meal.nutri_facts.carbohydrate.value >= 0
            assert corrected_max_meal.nutri_facts.total_fat is not None and corrected_max_meal.nutri_facts.total_fat.value >= 0
    ...
    def test_json_with_computed_properties_round_trip(self, complex_api_meal):
        ...
        assert restored_meal == complex_api_meal, "JSON round-trip should preserve all meal data"
    ...
    def test_domain_computed_properties_override_api_values(self):
        ...
        original_api_calories = api_meal_with_incorrect.nutri_facts.calories.value if api_meal_with_incorrect.nutri_facts else 0
        ...
        expected_weight = sum(recipe.weight_in_grams if recipe.weight_in_grams else 0 
                             for recipe in api_meal_with_incorrect.recipes)
        expected_calorie_density = (expected_calories / expected_weight) * 100 if expected_weight > 0 else None
        ...
        if expected_calorie_density is not None and domain_meal.calorie_density is not None:
        ...
        if expected_calorie_density is not None and final_api_meal.calorie_density is not None:
        ...
        if expected_calories > 0 and original_api_calories is not None and abs(expected_calories - original_api_calories) > 0.1:
            assert abs(final_api_meal.nutri_facts.calories.value - original_api_calories) > 0.1, "Domain should have overridden incorrect API calories"
        
        if expected_weight != original_api_weight:
            assert final_api_meal.weight_in_grams != original_api_weight, "Domain should have overridden incorrect API weight"
