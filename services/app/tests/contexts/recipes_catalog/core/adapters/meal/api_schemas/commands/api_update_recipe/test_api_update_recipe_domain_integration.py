...
class TestApiUpdateRecipeDomainIntegration:
    ...
    def test_end_to_end_update_flow_complex_recipe(self):
        ...
        assert len(domain_update.updates["ingredients"]) == len(original_recipe.ingredients)
    ...
    @pytest.mark.parametrize("test_case", [
        {"name": "Updated Recipe Name", "description": "Updated Description", "instructions": "Updated Instructions"},
        {"name": "Recipe with special chars: !@#$%", "description": "Description with unicode: 🍕", "instructions": "Cook with ♥"},
        {"name": ("Very long recipe name " * 10).strip(), "description": ("Very long description " * 20).strip(), "instructions": ("Very long instructions " * 50).strip()},
    ])
    ...
    @pytest.mark.parametrize("privacy_value", [
        "public",
        "private",
    ])
    ...
    def test_business_rules_preservation_computed_properties(self):
        ...
        try:
            ...
            expected_fields = ["name", "description", "instructions", "ingredients", "tags"]
    ...
    @pytest.mark.parametrize("scenario_name,recipe_factory", [
    ...
    def test_event_generation_verification_simulation(self, scenario_name, recipe_factory):
        ...
        event_trigger_fields = ["name", "description", "instructions", "ingredients", "tags"]
    ...
    def test_comprehensive_integration_scenarios(self, scenario_index):
        ...
        assert domain_update.recipe_id == test_recipe.id, f"Failed for scenario {scenario_index}"
        ...
        assert "instructions" in domain_update.updates, f"Failed for scenario {scenario_index}"
        assert domain_update.updates["name"] == test_recipe.name, f"Failed for scenario {scenario_index}"
        assert domain_update.updates["instructions"] == test_recipe.instructions, f"Failed for scenario {scenario_index}"
