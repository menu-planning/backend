...
REALISTIC_RATING_SCENARIOS = [
    ...
    {"taste": 3, "convenience": 5, "comment": "Quick and easy but flavor could be better"},
    {"taste": 4, "convenience": 4, "comment": "Good all-around recipe"},
    {"taste": 2, "convenience": 4, "comment": "Easy to make but not very tasty"},
    {"taste": 5, "convenience": 2, "comment": "Incredible taste but very time-consuming"},
...
def create_api_rating_from_json(json_data: Optional[str] = None, **kwargs) -> ApiRating:
    """
...
def create_excellent_rating(**kwargs) -> ApiRating:
    ...
    final_kwargs = {
        ...
        "comment": kwargs.get("comment", "Absolutely perfect! This recipe exceeded all expectations. The flavors were incredible and it was so easy to make. Will definitely make again!"),
        **{k: v for k, v in kwargs.items() if k not in ["taste", "convenience", "comment"]}
    }
...
def create_poor_rating(**kwargs) -> ApiRating:
    ...
    final_kwargs = {
        ...
        "comment": kwargs.get("comment", "Very disappointed. The taste was bland and the recipe was confusing. Too much effort for such poor results."),
        **{k: v for k, v in kwargs.items() if k not in ["taste", "convenience", "comment"]}
    }
    return create_api_rating(**final_kwargs)
...
def create_mixed_rating(**kwargs) -> ApiRating:
    ...
    final_kwargs = {
        ...
        "comment": kwargs.get("comment", "Amazing flavors but very time-consuming and complex. Worth it for special occasions but not for weeknight dinners."),
        **{k: v for k, v in kwargs.items() if k not in ["taste", "convenience", "comment"]}
    }
    return create_api_rating(**final_kwargs)
...
def create_rating_with_max_comment(**kwargs) -> ApiRating:
    ...
    """
    max_comment = "This is an extremely detailed review that tests the maximum comment length limit. " * 12  # Creates ~996 chars
...
def create_quick_easy_rating(**kwargs) -> ApiRating:
    ...
    final_kwargs = {
        ...
        "convenience": kwargs.get("convenience", 5),
        "comment": kwargs.get("comment", "Super quick and easy! Perfect for busy weeknights. Taste is decent but the convenience makes up for it."),
        **{k: v for k, v in kwargs.items() if k not in ["taste", "convenience", "comment"]}
    }
    return create_api_rating(**final_kwargs)
...
def create_gourmet_rating(**kwargs) -> ApiRating:
    ...
    final_kwargs = {
        "taste": kwargs.get("taste", 5),
        "convenience": kwargs.get("convenience", 2),
        "comment": kwargs.get("comment", "Exceptional flavor profile! This recipe produces restaurant-quality results but requires time and skill. Worth every minute."),
        **{k: v for k, v in kwargs.items() if k not in ["taste", "convenience", "comment"]}
    }
    return create_api_rating(**final_kwargs)
...
def create_rating_range_dataset() -> List[ApiRating]:
    ...
    for taste in range(0, 6):
        for convenience in range(0, 6):
            ...
            rating_kwargs["comment"] = f"Rating with taste={taste}, convenience={convenience}"
            rating = create_api_rating(**rating_kwargs)
            ratings.append(rating)
