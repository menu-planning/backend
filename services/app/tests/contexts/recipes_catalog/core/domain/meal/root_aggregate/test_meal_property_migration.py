...
class TestMealCurrentPropertyBehavior:
    ...
    @pytest.mark.xfail(reason="Direct property setters removed in migration to protected setter pattern")
    ...
    def test_menu_id_setter_no_event_generation(self, sample_meal):
        ...
        assert sample_meal.version == initial_version + 1
        assert len(sample_meal.events) == initial_events  # No events for menu_id changes
    ...
    def test_event_generation_on_property_changes(self, sample_meal):
        ...
        update_events = [e for e in sample_meal.events if isinstance(e, UpdatedAttrOnMealThatReflectOnMenu)]
    ...
    def test_recipe_management_methods(self, sample_meal, sample_recipe):
        ...
        sample_meal.create_recipe(
            ...
            name="New Recipe",
            ingredients=[
                Ingredient(name="New Ingredient", quantity=50, unit=MeasureUnit.GRAM, position=0)
            ],
...
class TestMealProtectedSetterMigrationRequirements:
    ...
    def test_should_have_protected_setters_after_migration(self, sample_meal):
        ...
        for setter_name in expected_protected_setters:
            assert hasattr(sample_meal, setter_name), f"Missing protected setter: {setter_name}"
            setter_method = getattr(sample_meal, setter_name)
            assert callable(setter_method), f"Protected setter {setter_name} is not callable"
    ...
    def test_direct_property_setters_removed_after_migration(self, sample_meal):
        ...
        with pytest.raises(AttributeError, match="property 'name' of 'Meal' object has no setter"):
            sample_meal.name = "Direct Name"
            
        with pytest.raises(AttributeError, match="property 'description' of 'Meal' object has no setter"):
            sample_meal.description = "Direct Description"
            
        with pytest.raises(AttributeError, match="property 'notes' of 'Meal' object has no setter"):
            sample_meal.notes = "Direct Notes"
            
        with pytest.raises(AttributeError, match="property 'like' of 'Meal' object has no setter"):
            sample_meal.like = False
            
        with pytest.raises(AttributeError, match="property 'image_url' of 'Meal' object has no setter"):
    ...
    def test_invalid_property_update_raises_error(self, sample_meal):
        ...
        with pytest.raises(AttributeError, match="Meal has no property 'invalid_property' or it cannot be updated"):
