...
class TestUserRepositoryCore:
    ...
    async def test_get_sa_instance(self, user_repository: UserRepo, test_session):
        ...
        admin_role = create_role_orm(name="administrator", context="IAM", permissions="read, write, delete, admin")
    ...
    async def test_persist_user_fails_without_add(self, user_repository: UserRepo):
        ...
        with pytest.raises(Exception):  # Should fail because user not in repository.seen
    ...
    async def test_persist_all_users_fails_without_add(self, user_repository: UserRepo):
        ...
        users = [
            create_user(id="not_added_1"),
            create_user(id="not_added_2")
        ]
        
        with pytest.raises(Exception):  # Should fail because users not in repository.seen
    ...
    async def test_roles_are_persisted_after_user_creation(self, user_repository: UserRepo, test_session):
    ...
    async def test_role_update_through_user_modification(self, user_repository: UserRepo, test_session):
    ...
    async def test_relationship_queries_no_duplicate_rows(self, user_repository: UserRepo, test_session):
...
class TestUserRepositoryRoles:
    ...
    async def test_role_relationship_scenarios_with_real_data(self, user_repository: UserRepo, test_session, scenario):
        ...
        if scenario["should_match"]:
            assert len(results) >= 1, f"Scenario {scenario['scenario_id']} should match but didn't"
            assert user_orm.id in [user.id for user in results]
        else:
            user_ids = [user.id for user in results]
            assert user_orm.id not in user_ids, f"Scenario {scenario['scenario_id']} should not match but did"

    @pytest.mark.parametrize("scenario", get_permission_scenarios_orm())
    async def test_permission_validation_scenarios_with_real_data(self, user_repository: UserRepo, test_session, scenario):
        ...
        if "expected_permissions" in scenario:
            ...
            for expected_perm in scenario["expected_permissions"]:
                assert expected_perm in all_permissions, f"Missing permission {expected_perm} in scenario {scenario['scenario_id']}"

    async def test_role_context_filtering_with_real_data(self, user_repository: UserRepo, test_session):
        ...
        recipes_user = create_user_orm(
            ...
            roles=[create_role_orm(name="user", context="recipes_catalog", permissions="read")]
        )
    ...
    async def test_role_name_filtering_with_real_roles(self, user_repository: UserRepo, test_session):
    ...
    async def test_multiple_role_combinations_with_database(self, user_repository: UserRepo, test_session):
        ...
        multi_context_user = create_user_orm(
            ...
            roles=[
                ...
                create_role_orm(name="administrator", context="recipes_catalog", permissions="read, write, delete"),
                create_role_orm(name="auditor", context="products_catalog", permissions="read, audit")
            ]
        )
    ...
    async def test_role_validation_with_orm_models(self, user_repository: UserRepo, test_session):
...
class TestUserRepositoryFiltering:
    ...
    async def test_user_filter_scenarios_with_real_data(self, user_repository: UserRepo, test_session, scenario):
    ...
    async def test_discarded_user_filtering(self, user_repository: UserRepo, test_session):
    ...
    async def test_version_filtering_with_real_data(self, user_repository: UserRepo, test_session):
    ...
    async def test_combined_filtering_scenarios(self, user_repository: UserRepo, test_session):
        ...
        admin_discarded = create_user_orm(id="admin_discarded", discarded=True, version=2)
        ...
        assert any(role.name == "administrator" for role in active_admin_results[0].roles)
...
class TestUserRepositoryErrorHandling:
    ...
    async def test_database_constraint_violations_with_users(self, user_repository: UserRepo, test_session):
    ...
    async def test_role_constraint_validation_with_database(self, user_repository: UserRepo, test_session):
        """Test role constraint validation"""
        # Test role with valid composite primary key
        role1 = create_role_orm(name="test_role", context="test_context", permissions="read")
        role2 = create_role_orm(name="test_role", context="test_context", permissions="write")  # Same composite key
...
class TestUserRepositoryPerformance:
    ...
    async def test_query_performance_scenarios(self, user_repository: UserRepo, scenario: Dict[str, Any]):
        ...
        admin_role = create_role_orm(name="administrator", context="IAM", permissions="read, write, delete, admin")
        manager_role = create_role_orm(name="user_manager", context="IAM", permissions="read, write, manage_users")
        auditor_role = create_role_orm(name="auditor", context="IAM", permissions="read, audit")
        ...
        assert elapsed_time <= expected_time, f"Query took {elapsed_time:.3f}s, expected ≤{expected_time}s"
        assert isinstance(results, list)
        assert all(isinstance(user, UserSaModel) for user in results)

    async def test_bulk_persist_performance(self, user_repository: UserRepo, test_session):
        ...
        assert time_per_user <= expected_time_per_user, f"Bulk creation took {time_per_user:.4f}s per user, expected ≤{expected_time_per_user}s"
    ...
    async def test_complex_query_performance(self, user_repository: UserRepo):
        ...
        assert elapsed_time <= 1.0, f"Complex query took {elapsed_time:.3f}s, expected ≤1.0s"
    ...
    async def test_bulk_user_query_performance(self, user_repository: UserRepo, test_session):
        ...
        assert elapsed_time <= 2.0, f"Bulk query took {elapsed_time:.3f}s for {user_count} users"
        assert len(results) == user_count
        assert all(isinstance(user, UserSaModel) for user in results)
...
class TestSpecializedUserFactories:
    ...
    async def test_admin_user_creation_with_database(self, user_repository: UserRepo, test_session):
    ...
    async def test_user_manager_creation_with_database(self, user_repository: UserRepo, test_session):
    ...
    async def test_multi_role_user_creation_with_database(self, user_repository: UserRepo, test_session):
    ...
    async def test_discarded_user_creation_with_database(self, user_repository: UserRepo, test_session):
        ...
        result = await user_repository.get_sa_instance("test_discarded", _return_discarded=True)
        
        assert result.id == "test_discarded"
        assert result.discarded is True  # Key characteristic
        assert result.version == 1

    async def test_user_orm_factory_persistence_patterns(self, user_repository: UserRepo, test_session):
