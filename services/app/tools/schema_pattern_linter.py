...
class SchemaPatternLinter:
    ...
    def _analyze_class(self, node: ast.ClassDef, file_path: Path) -> Optional[SchemaAnalysis]:
    ...
    def _is_api_schema_class(self, node: ast.ClassDef, file_path: Path) -> bool:
        ...
        base_classes = [self._get_base_name(base) for base in node.bases]
        api_base_classes = {"BaseModel", "BaseApiModel", "BaseValueObject", "BaseEntity", "BaseCommand"}
    ...
    def _determine_schema_type(self, class_name: str, base_classes: List[str], file_path: Path) -> SchemaType:
        ...
        elif "filters" in path_str:
            return SchemaType.FILTER
            
        # Check class naming patterns
        if "command" in class_lower or class_name.startswith("ApiCreate") or class_name.startswith("ApiUpdate"):
            return SchemaType.COMMAND
        elif "filter" in class_lower:
            return SchemaType.FILTER
        elif any(base in base_classes for base in ["BaseEntity"]):
            return SchemaType.ENTITY
        elif any(base in base_classes for base in ["BaseValueObject"]):
            return SchemaType.VALUE_OBJECT
            
        return SchemaType.UNKNOWN
    
    def _check_inheritance_violations(self, analysis: SchemaAnalysis, node: ast.ClassDef):
        ...
        if expected_base and expected_base not in analysis.base_classes:
            ...
            if "BaseModel" in analysis.base_classes:
                ...
                violation = Violation(
                    ...
                    expected=expected_base,
                    actual=", ".join(analysis.base_classes) if analysis.base_classes else "None",
                    context={
                        "schema_type": analysis.schema_type.value,
                        "current_bases": analysis.base_classes
                    }
    ...
    def _check_missing_methods(self, analysis: SchemaAnalysis, node: ast.ClassDef):
        ...
        for method_name, method_info in self.REQUIRED_METHODS.items():
            ...
            if not method_node:
                ...
                severity = "critical" if method_name in ["from_orm_model", "to_orm_kwargs"] else "high"
                ...
                self._validate_method_signature(analysis, method_node, method_name, method_info)
    
    def _find_method_in_class(self, class_node: ast.ClassDef, method_name: str) -> Optional[ast.FunctionDef]:
    ...
    def _validate_method_signature(self, analysis: SchemaAnalysis, method_node: ast.FunctionDef, 
        ...
        if method_info["signature"] == "classmethod":
            ...
            if method_name in ["from_domain", "from_orm_model"]:
                param_names.append("domain_obj" if "domain" in method_name else "orm_model")
        else:
    ...
    def _check_config_compliance(self, analysis: SchemaAnalysis, node: ast.ClassDef):
        ...
        elif not any(base in analysis.base_classes 
    ...
    def _find_collection_fields(self, node: ast.ClassDef) -> List[str]:
        ...
        for item in node.body:
            if isinstance(item, ast.AnnAssign) and item.annotation:
                ...
                if any(pattern in annotation_str.lower() for pattern in collection_patterns):
    ...
    def _involves_typeadapter(self, value_node: ast.expr) -> bool:
        ...
        if isinstance(value_node, ast.Call):
            ...
            if isinstance(func, ast.Name) and func.id == "TypeAdapter":
                return True
            elif isinstance(func, ast.Attribute) and func.attr == "TypeAdapter":
                return True
        return False
    
    def analyze_directory(self, directory: Path, recursive: bool = True) -> List[SchemaAnalysis]:
    ...
    def _generate_text_report(self) -> str:
        ...
        for violation_type, violations in violations_by_type.items():
            ...
            if len(violations) > 10:
                report.append(f"     ... and {len(violations) - 10} more violations of this type")
                report.append("")
...
Examples:
  ...
  poetry run python tools/schema_pattern_linter.py src/contexts/ --report-format json
    ...
    parser.add_argument("--severity-filter", choices=["critical", "high", "medium", "low"],
    ...
    if args.context:
        linter.violations = [v for v in linter.violations if args.context in v.file_path]
    ...
    if args.severity_filter:
        ...
        linter.violations = [v for v in linter.violations 
