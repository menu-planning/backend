...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('users', sa.Column('username', sa.VARCHAR(), autoincrement=False, nullable=False), schema='iam')
    op.add_column('users', sa.Column('email', sa.VARCHAR(), autoincrement=False, nullable=False), schema='iam')
    op.add_column('users', sa.Column('confirmed', sa.BOOLEAN(), autoincrement=False, nullable=False), schema='iam')
    op.add_column('users', sa.Column('active', sa.BOOLEAN(), autoincrement=False, nullable=False), schema='iam')
    op.add_column('users', sa.Column('hashed_password', sa.VARCHAR(), autoincrement=False, nullable=False), schema='iam')
    op.create_index('ix_iam_users_username', 'users', ['username'], unique=True, schema='iam')
