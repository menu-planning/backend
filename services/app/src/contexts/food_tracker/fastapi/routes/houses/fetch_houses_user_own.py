...
@timeout_after()
async def read_houses_user_own(
    ...
    """
    Retrieve houses the user own.
    """
    uow: UnitOfWork
    with bus.uow as uow:
        houses_user_own = await uow.houses.query(filter={"owner": current_user.id})
    return [ApiHouse.from_domain(house) for house in houses_user_own]
