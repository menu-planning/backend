...
@timeout_after()
async def read_houses_user_is_member(
    ...
    """
    Retrieve houses the user is a member of.
    """
    uow: UnitOfWork
    with bus.uow as uow:
        houses_user_is_member = await uow.houses.query(
            filter={"members": current_user.id}
        )
    return [ApiHouse.from_domain(house) for house in houses_user_is_member]
