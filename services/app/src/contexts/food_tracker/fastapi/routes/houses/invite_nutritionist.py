...
@router.post(
    "/houses/{house_id}/invite-nutritionist", status_code=status.HTTP_201_CREATED
)
async def invite_nutritionist(
    house_id: Annotated[int, Path(title="The ID of the house to update")],
    api: ApiInviteNutritionist,
    ...
    """
    Invite a nutritionist to a house.
    """
    uow: UnitOfWork
    with bus.uow as uow:
