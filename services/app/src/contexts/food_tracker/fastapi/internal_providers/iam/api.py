...
from src.contexts.food_tracker.fastapi.internal_providers.iam.api_schemas.user import (
    IAMUser,
)
...
class IAMProvider:
    ...
    async def get(id: str) -> SeedUser:
        user = await iam_api.get(id=id, caller_context="food_tracker")
        return IAMUser(**json.loads(user)).to_domain()

    @staticmethod
    def current_active_user(
        user_data: Annotated[
            SeedUser, Depends(iam_api.CurrentActiveUserWithContext("food_tracker"))
        ]
    ) -> ApiSeedUser:
