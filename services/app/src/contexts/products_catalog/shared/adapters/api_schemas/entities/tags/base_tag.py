...
class ApiTag(BaseModel):
    """
    A Pydantic model representing and validating a recipe tag.
    ...
    Methods:
        from_domain(domain_obj: Tag) -> "ApiTag":
    ...
    @classmethod
    def from_domain(cls, domain_obj: Tag, entity_type: type[Tag]) -> "ApiTag":
        """Creates an instance of `ApiTag` from a domain model object."""
        try:
            return entity_type(
                id=domain_obj.id,
    ...
    def to_domain(self, entity_type: type[Tag]) -> Tag:
        """Converts the instance to a domain model object."""
        try:
            return entity_type(
                id=self.id,
            ...
            raise ValueError(f"Failed to convert ApiTag to domain model: {e}")
