...
@router.get(
    "/product-classifications/{classification_type}/{id}",
    response_model=ApiClassification,
    status_code=status.HTTP_200_OK,
)
@timeout_after()
async def get_by_id(
    id: Annotated[int, Path(title="The ID of the classification to fetch")],
    ...
    """
    Retrieve specific classification by type.
    """
    ...
    return await utils.read_classification(
        id,
        current_user,
        uow_repo_name,
        api_schema_class,
