...
from src.contexts.seedwork.shared.adapters.api_schemas.base_api_model import MODEL_CONFIG
from src.contexts.seedwork.shared.adapters.api_schemas.validators import validate_permissions_collection
from src.contexts.client_onboarding.core.domain.enums import Permission as ClientOnboardingPermission
...
class UserContext(BaseModel):
    ...
    permissions: Annotated[
        ...
        Field(
            ...
            description="Set of client onboarding permissions associated with the user"
        )
    ]
    roles: frozenset[str] = Field(
        default_factory=frozenset,
        description="Set of roles associated with the user in client onboarding context"
    )
    context: str = Field(
...
class PermissionValidationRequest(BaseModel):
    ...
    resource_context: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Additional context about the resource being accessed"
    )
...
class FormAccessRequest(BaseModel):
    ...
    form_id: Optional[str] = Field(
        default=None,
        description="TypeForm form ID (external identifier)"
    )
    onboarding_form_id: Optional[int] = Field(
        default=None,
        description="Internal onboarding form ID",
        gt=0
    )
    ...
    def __init__(self, **data):
        ...
        if not self.form_id and not self.onboarding_form_id:
            raise ValueError("Either form_id or onboarding_form_id must be provided")
...
class FormAccessResponse(BaseModel):
    ...
    onboarding_form_id: Optional[int] = Field(
        default=None,
        description="Internal onboarding form ID that was accessed"
    )
    ...
    has_permission: bool = Field(..., description="Whether the user has required permission")
    reason: Optional[str] = Field(
        default=None,
...
class MultiPermissionValidationRequest(BaseModel):
    ...
    required_permissions: List[str] = Field(
        ...,
        description="List of permissions required for the operation",
        min_length=1
    )
    operation: str = Field(..., description="Operation being validated")
...
class MultiPermissionValidationResponse(BaseModel):
    ...
    missing_permissions: List[str] = Field(..., description="Permissions that the user lacks")
