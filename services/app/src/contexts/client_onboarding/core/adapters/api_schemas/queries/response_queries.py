...
Pydantic models for querying stored responses for form creators with proper authorization.
Provides request/response validation for various query operations.
"""
...
class ResponseQueryRequest(BaseModel):
    ...
    def validate_form_id_for_form_queries(cls, v, info):
        ...
        if info.data.get('query_type') == QueryType.RESPONSES_BY_FORM and v is None:
            raise ValueError("form_id is required for responses_by_form queries")
        return v
    ...
    def validate_response_id_for_response_queries(cls, v, info):
        ...
        if info.data.get('query_type') == QueryType.RESPONSE_BY_ID and v is None:
            raise ValueError("response_id is required for response_by_id queries")
        return v
...
class FormSummary(BaseModel):
    ...
    response_count: Optional[int] = Field(None, description="Number of responses")
...
class ResponseSummary(BaseModel):
    ...
    client_identifiers: Optional[ClientIdentifierSet] = Field(None, description="Extracted client identifiers")
...
class ResponseQueryResponse(BaseModel):
    ...
    forms: Optional[List[FormSummary]] = Field(None, description="Forms matching the query")
    responses: Optional[List[ResponseSummary]] = Field(None, description="Responses matching the query")
    ...
    def validate_forms_for_form_queries(cls, v, info):
        ...
        if query_type == QueryType.FORMS_BY_USER and v is None:
            raise ValueError("forms must be provided for forms_by_user queries")
        return v
    ...
    def validate_responses_for_response_queries(cls, v, info):
        ...
        query_type = info.data.get('query_type')
        if query_type in [QueryType.RESPONSES_BY_FORM, QueryType.RESPONSE_BY_ID, QueryType.RESPONSES_BY_USER] and v is None:
            raise ValueError(f"responses must be provided for {query_type} queries")
        return v
...
class BulkResponseQueryResponse(BaseModel):
    ...
    results: List[ResponseQueryResponse] = Field(..., description="Results for each query")
