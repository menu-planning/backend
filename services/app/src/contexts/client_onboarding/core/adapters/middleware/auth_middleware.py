...
This middleware provides specialized authentication and authorization for client onboarding
features by extending the shared_kernel auth patterns with form ownership validation.
...
class ClientOnboardingAuthContext(AuthContext):
    ...
    def can_access_form(self, form_owner_id: str) -> bool:
        ...
        return (self.user_id == form_owner_id or 
                self.has_permission("client_onboarding:form:read_all"))
    ...
    def can_manage_form(self, form_owner_id: str) -> bool:
        ...
        return (self.user_id == form_owner_id or 
                self.has_permission("client_onboarding:form:manage_all"))
    ...
    def can_configure_webhooks(self, form_owner_id: str) -> bool:
        ...
        return (self.user_id == form_owner_id or 
                self.has_permission("client_onboarding:webhook:configure"))
    ...
    def can_view_responses(self, form_owner_id: str) -> bool:
        ...
        return (self.user_id == form_owner_id or 
                self.has_permission("client_onboarding:response:read_all"))
...
class ClientOnboardingAuthMiddleware(SharedAuthMiddleware):
    ...
    async def _authenticate_user(self, event: Dict[str, Any]) -> ClientOnboardingAuthContext:
        """
        ...
        try:
            ...
            if not user_id:
                self.structured_logger.warning(
                    ...
                    event_headers=list(event.get("headers", {}).keys()) if event.get("headers") else []
                )
            ...
            iam_response = await self.iam_provider.get_user(user_id, "client_onboarding")
            
            if iam_response.get("statusCode") == 200:
    ...
    def get_current_auth_context(self) -> Optional[ClientOnboardingAuthContext]:
    ...
    async def require_form_ownership(self, form_owner_id: str) -> None:
        """
        Require that the current user owns the specified form or has management permissions.
        ...
        if not auth_context:
            raise AuthorizationError("No authentication context available")
        ...
        if not auth_context.can_manage_form(form_owner_id):
            ...
            raise AuthorizationError(f"User does not have permission to manage form owned by {form_owner_id}")
    ...
    async def require_form_access(self, form_owner_id: str) -> None:
        ...
        if not auth_context.can_access_form(form_owner_id):
            ...
            raise AuthorizationError(f"User does not have permission to access form owned by {form_owner_id}")
...
def create_client_onboarding_auth_middleware(
    ...
    """
    return ClientOnboardingAuthMiddleware(
        require_authentication=require_authentication
    ) 