...
class WebhookSecurityVerifier:
    ...
    async def verify_webhook_request(self, payload: str, headers: Dict[str, str], timestamp_tolerance_minutes: int = 5) -> Tuple[bool, Optional[str]]:
        try:
            ...
            if not is_valid:
                ...
                return False, error_msg
            timestamp_valid = await self._verify_timestamp(headers, timestamp_tolerance_minutes)
    ...
    async def _verify_signature(self, payload: str, expected_signature: str) -> bool:
        try:
            ...
            computed_hmac = hmac.new(secret_bytes, payload_bytes, hashlib.sha256).digest()
    ...
    async def _verify_timestamp(self, headers: Dict[str, str], tolerance_minutes: int) -> bool:
        timestamp_header = (headers.get('x-typeform-timestamp') or headers.get('timestamp') or headers.get('x-timestamp'))
...
async def verify_typeform_webhook(payload: str, headers: Dict[str, str], webhook_secret: Optional[str] = None, timestamp_tolerance_minutes: int = 5) -> Tuple[bool, Optional[str]]:
    verifier = WebhookSecurityVerifier(webhook_secret)
    return await verifier.verify_webhook_request(payload, headers, timestamp_tolerance_minutes)
