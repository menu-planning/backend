...
from src.logging.logger import correlation_id_ctx, logger as standard_logger, StructlogFactory
...
class ErrorMiddleware:
    ...
    def __init__(
        ...
        Args:
            ...
            include_stack_trace: Whether to include stack traces in error responses (dev only)
    ...
    def _categorize_exception(self, exception: Exception) -> ErrorType:
        ...
        elif any(keyword in error_message for keyword in ["unauthorized", "not authenticated", "invalid token"]):
            return ErrorType.AUTHENTICATION_ERROR
        elif any(keyword in error_message for keyword in ["forbidden", "permission", "not allowed", "access denied"]):
            return ErrorType.AUTHORIZATION_ERROR
        elif any(keyword in error_message for keyword in ["already exists", "duplicate", "conflict"]):
            return ErrorType.CONFLICT_ERROR
        elif any(keyword in error_message for keyword in ["timeout", "timed out", "connection"]):
    ...
    async def _create_error_response(
        ...
        if isinstance(exception, ValidationError):
            return await self._create_validation_error_response(exception, correlation_id)
