...
class ApiContactInfo(BaseApiValueObject[ContactInfo, SaBase]):
    ...
    def from_orm_model(cls, orm_model: ContactInfoSaModel) -> "ApiContactInfo":
        ...
        return cls(
            ...
            all_phones=frozenset(orm_model.all_phones) if orm_model.all_phones is not None else frozenset(),
            all_emails=frozenset(orm_model.all_emails) if orm_model.all_emails is not None else frozenset(),
        )
