...
#     def __init__(self, recipe_repo: RecipeRepo):
#         self.recipe_repo = recipe_repo

#     async def map_domain_to_sa(self, domain_obj: Recipe) -> RecipeSaModel:
#         async def relationship(sa_model_type: SaBase, filter: dict):
#             stmt = select(sa_model_type).filter_by(**filter)
#             try:
#                 query = await self.recipe_repo._session.execute(stmt)
