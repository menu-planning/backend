...
from src.contexts.recipes_catalog.core.adapters.meal.api_schemas.commands.api_copy_meal import ApiCopyMeal
...
async def async_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    """
    Lambda function handler to copy a meal.
    """
    ...
    try:
        ...
        return {
            "statusCode": 400,
            "headers": CORS_headers,
            "body": json.dumps({"message": f"Invalid meal copy data: {str(e)}"}),
        }
    ...
    if not (
        ...
        logger.warning(f"User {current_user.id} does not have permission to copy meal for user {api.id_of_user_coping_meal}")
        return {
            "statusCode": 403,
            "headers": CORS_headers,
            "body": json.dumps(
    ...
    # Business context: Meal copying through message bus
    try:
        bus: MessageBus = container.bootstrap()
        await bus.handle(cmd)
        logger.debug(f"Meal copied successfully")
    except Exception as e:
        logger.error(f"Failed to copy meal: {str(e)}")
        return {
            "statusCode": 500,
            "headers": CORS_headers,
            "body": json.dumps({"message": "Internal server error during meal copying"}),
        }
    
    return {
        "statusCode": 201,
        "headers": CORS_headers,
        "body": json.dumps({"message": "Meal copied successfully"}),
    }
