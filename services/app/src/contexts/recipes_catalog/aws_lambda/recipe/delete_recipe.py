...
async def async_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    if not recipe_id:
        logger.error("Recipe ID not provided in path parameters")
        return {
            "statusCode": 400,
            "headers": CORS_headers,
            "body": json.dumps({"message": "Recipe ID is required"}),
        } 
    ...
    try:
        ...
        async with bus.uow as uow:
            ...
            logger.debug(f"Recipe found: {recipe_id}, author: {recipe.author_id}")
        ...
        logger.error(f"Unexpected error retrieving recipe {recipe_id}: {str(e)}")
        return {
            "statusCode": 500,
            "headers": CORS_headers,
            "body": json.dumps({"message": "Internal server error during recipe retrieval"}),
        }
    ...
    if not (
        ...
        logger.warning(f"User {current_user.id} does not have permission to delete recipe {recipe_id} (author: {recipe.author_id})")
        return {
            "statusCode": 403,
            "headers": CORS_headers,
            "body": json.dumps(
    ...
    try:
        ...
        logger.debug(f"Recipe deleted successfully: {recipe_id}")
    except Exception as e:
        logger.error(f"Failed to delete recipe {recipe_id}: {str(e)}")
        return {
            "statusCode": 500,
            "headers": CORS_headers,
            "body": json.dumps({"message": "Internal server error during recipe deletion"}),
        }
