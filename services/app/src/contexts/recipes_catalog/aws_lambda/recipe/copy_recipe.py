...
from src.contexts.recipes_catalog.core.adapters.internal_providers.iam.api import \
    IAMProvider
from src.contexts.recipes_catalog.core.adapters.meal.api_schemas.commands.api_copy_recipe import ApiCopyRecipe
...
async def async_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "statusCode": 400,
            "headers": CORS_headers,
            "body": json.dumps({"message": f"Invalid recipe copy data: {str(e)}"}),
        }
    ...
    if not (
        ...
        logger.warning(f"User {current_user.id} does not have permission to copy recipe for user {api.user_id}")
        return {
            "statusCode": 403,
            "headers": CORS_headers,
            "body": json.dumps(
    ...
    try:
        ...
        logger.debug(f"Recipe copied successfully")
    except Exception as e:
        logger.error(f"Failed to copy recipe: {str(e)}")
        return {
            "statusCode": 500,
            "headers": CORS_headers,
            "body": json.dumps({"message": "Internal server error during recipe copying"}),
        }
