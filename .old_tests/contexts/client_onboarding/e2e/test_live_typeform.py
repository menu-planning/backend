...
skip_if_no_credentials = pytest.mark.skipif(
    not TYPEFORM_API_KEY or not TYPEFORM_WEBHOOK_SECRET or not TYPEFORM_WEBHOOK_URL,
    reason="TYPEFORM_API_KEY, TYPEFORM_WEBHOOK_SECRET, and TYPEFORM_WEBHOOK_URL environment variables required for live API tests",
)
...
class TestLiveTypeformAPI:
    ...
    async def test_create_webhook_with_live_api(self):
        ...
        typeform_id = (
            TYPEFORM_TEST_FORM_ID  # Use the actual test form ID from environment
        )
        ...
        webhook_url = f"{TYPEFORM_WEBHOOK_URL}/webhook"
            ...
            if webhook_info and webhook_info.id:
                ...
                delete_result = (
                    await self.webhook_manager.typeform_client.delete_webhook(
                        form_id=typeform_id, tag="client_onboarding"
                    )
                )
    ...
    async def test_rate_limiting_compliance(self):
        ...
        try:
            ...
            if webhook_info and webhook_info.id:
                ...
                await self.webhook_manager.typeform_client.update_webhook(
                    form_id=typeform_id, tag="client_onboarding", webhook_url=new_url
                )
    ...
    async def test_concurrent_webhook_operations(self):
        ...
        for i in range(3):
            ...
            onboarding_form = create_onboarding_form(
                id=form_id, typeform_id=typeform_id
            )
            await self.fake_uow.onboarding_forms.add(onboarding_form)
    ...
    async def test_webhook_status_synchronization(self):
        ...
        try:
            await self.webhook_manager.typeform_client.delete_webhook(
                form_id=TYPEFORM_TEST_FORM_ID, tag="client_onboarding"
            )
        except:
