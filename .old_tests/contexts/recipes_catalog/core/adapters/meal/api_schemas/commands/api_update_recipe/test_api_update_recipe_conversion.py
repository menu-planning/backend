...
class TestApiUpdateRecipeConversion:
    ...
    def test_update_recipe_command_validation(self, recipe_factory):
        ...
        assert isinstance(domain_command.recipe_id, str)
        assert len(domain_command.recipe_id.replace('-', '')) == 32  # UUID without hyphens
    ...
    def test_conversion_with_realistic_recipe_scenarios(self, scenario_data):
        ...
        assert isinstance(domain_command, UpdateRecipe), f"Conversion failed for scenario: {scenario_data['name']}"
        assert domain_command.recipe_id == api_recipe.id, f"Recipe ID mismatch for scenario: {scenario_data['name']}"
        ...
        assert domain_command.updates["name"] == api_recipe.name, f"Name mismatch for scenario: {scenario_data['name']}"
        
        # Verify scenario-specific characteristics
        if api_recipe.ingredients:
            if "ingredients" in domain_command.updates:
                assert len(domain_command.updates["ingredients"]) == len(api_recipe.ingredients), \
                    f"Ingredient count mismatch for scenario: {scenario_data['name']}"
        
        if api_recipe.tags:
            if "tags" in domain_command.updates:
                assert len(domain_command.updates["tags"]) == len(api_recipe.tags), \
                    f"Tag count mismatch for scenario: {scenario_data['name']}"

    @pytest.mark.parametrize("variant_name,recipe_factory", [
    ...
    def test_conversion_with_recipe_type_variants(self, variant_name, recipe_factory):
        ...
        assert isinstance(domain_command, UpdateRecipe), f"Conversion failed for variant: {variant_name}"
        assert domain_command.recipe_id == api_recipe.id, f"Recipe ID mismatch for variant: {variant_name}"
        ...
        assert domain_command.updates["name"] == api_recipe.name, f"Name mismatch for variant: {variant_name}"
        ...
        if variant_name == "minimal":
            ...
            has_collections = "ingredients" in domain_command.updates or "tags" in domain_command.updates
            ...
            update_fields = set(domain_command.updates.keys())
            assert expected_complex_fields.issubset(update_fields), \
    ...
    def test_recipe_configuration_field_consistency(self, recipe_factory):
        ...
        if "tags" in updates:
            ...
            for tag in updates["tags"]:
                ...
                assert tag.author_id == api_recipe.author_id  # Tag should belong to recipe author 
    ...
    def test_conversion_with_empty_minimal_updates(self):
        ...
        for field_name in updates_dict.keys():
            if field_name not in expected_minimal_fields:
                ...
                assert field_value is not None or field_name in ["description", "notes", "image_url", "utensils", "total_time", "weight_in_grams"]
    ...
    def test_conversion_with_truly_empty_optional_fields(self):
        ...
        unset_fields = {"description", "notes", "image_url", "utensils", "total_time", "weight_in_grams"}
    ...
    def test_complex_nested_object_conversion_comprehensive(self):
        ...
        if complex_recipe.ingredients and "ingredients" in updates:
            ...
            api_ingredients_list = sorted(list(complex_recipe.ingredients), key=lambda x: x.position)
            domain_ingredients_list = sorted(ingredients, key=lambda x: x.position)
            
            for i, (domain_ingredient, api_ingredient) in enumerate(zip(domain_ingredients_list, api_ingredients_list)):
    ...
    def test_frozen_set_handling_for_ingredients_and_tags(self):
        ...
        if complex_recipe.ingredients or complex_recipe.tags:
            ...
            if "ingredients" in updates:
                ingredients = updates["ingredients"]
                assert isinstance(ingredients, list)  # Should be list in domain, not frozenset
                
                # Extract ingredient names
                ingredient_names = [ingredient.name for ingredient in ingredients]
                api_ingredient_names = [ingredient.name for ingredient in complex_recipe.ingredients] if complex_recipe.ingredients else []
            ...
            if "tags" in updates:
                ...
                api_tag_keys = [tag.key for tag in complex_recipe.tags] if complex_recipe.tags else []
    ...
    @pytest.mark.parametrize("factory_name,recipe_factory", [
    ...
    def test_command_structure_with_different_recipe_factories(self, factory_name, recipe_factory):
        ...
        assert isinstance(domain_command, UpdateRecipe), f"Command structure inconsistent for {factory_name}"
        assert isinstance(domain_command.recipe_id, str), f"recipe_id structure inconsistent for {factory_name}"
        ...
        assert len(domain_command.recipe_id.replace('-', '')) == 32, f"recipe_id format inconsistent for {factory_name}"
    ...
    def test_command_field_types_comprehensive(self):
        ...
        for field in url_fields:
            if field in updates and updates[field] is not None:
                ...
                assert isinstance(updates[field], (str, HttpUrl)), f"URL field {field} has wrong type: {type(updates[field])}"
        
        # Test numeric field types
        numeric_fields = ["total_time", "weight_in_grams"]
        for field in numeric_fields:
            if field in updates and updates[field] is not None:
                assert isinstance(updates[field], int), f"Numeric field {field} has wrong type: {type(updates[field])}"
                assert updates[field] >= 0, f"Numeric field {field} has negative value: {updates[field]}"
        ...
        for field in enum_fields:
            if field in updates and updates[field] is not None:
                ...
                assert updates[field] in [Privacy.PRIVATE, Privacy.PUBLIC], f"Enum field {field} has invalid value: {updates[field]}"
        
        # Test collection field types
        if "ingredients" in updates:
            assert isinstance(updates["ingredients"], list), f"ingredients field is not list: {type(updates['ingredients'])}"
            for ingredient in updates["ingredients"]:
                ...
                assert hasattr(ingredient, 'quantity'), "Ingredient missing quantity attribute"
                assert hasattr(ingredient, 'position'), "Ingredient missing position attribute"
                assert isinstance(ingredient.name, str), f"Ingredient name wrong type: {type(ingredient.name)}"
                assert isinstance(ingredient.quantity, (int, float)), f"Ingredient quantity wrong type: {type(ingredient.quantity)}"
                assert isinstance(ingredient.position, int), f"Ingredient position wrong type: {type(ingredient.position)}"
        
        if "tags" in updates:
            assert isinstance(updates["tags"], frozenset), f"tags field is not frozenset: {type(updates['tags'])}"
            for tag in updates["tags"]:
        ...
        if "nutri_facts" in updates and updates["nutri_facts"] is not None:
            ...
            assert hasattr(nutri_facts, 'calories'), "NutriFacts missing calories attribute"
            assert hasattr(nutri_facts, 'protein'), "NutriFacts missing protein attribute"
            assert hasattr(nutri_facts, 'carbohydrate'), "NutriFacts missing carbohydrate attribute"
    ...
    def test_frozen_set_handling_comprehensive(self):
        ...
        if "ingredients" in updates:
            ...
            assert isinstance(api_ingredients, frozenset), f"API ingredients should be frozenset, got {type(api_ingredients)}"
            assert isinstance(domain_ingredients, list), f"Domain ingredients should be list, got {type(domain_ingredients)}"
            assert len(domain_ingredients) == len(api_ingredients), "Ingredient count should be preserved"
            
            # Verify ingredient positions are unique (what matters for frozenset property)
            ingredient_positions = [ingredient.position for ingredient in domain_ingredients]
            assert len(set(ingredient_positions)) == len(ingredient_positions), "Ingredient positions should be unique (frozenset property)"
        ...
        if "tags" in updates:
            ...
            assert isinstance(domain_tags, frozenset), f"Domain tags should remain frozenset, got {type(domain_tags)}"
            ...
            tag_identities = {(tag.key, tag.value, tag.author_id, tag.type) for tag in domain_tags}
            assert len(tag_identities) == len(domain_tags), "Tag identities should be unique (frozenset property)"
    ...
    def test_frozen_set_handling_edge_cases(self):
        ...
        if "ingredients" in updates:
            assert isinstance(updates["ingredients"], list), "Empty ingredients should still be list"
            assert len(updates["ingredients"]) == 0, "Empty ingredients should have length 0"
        
        if "tags" in updates:
            assert isinstance(updates["tags"], frozenset), "Empty tags should still be frozenset"
        ...
        if simple_recipe.ingredients or simple_recipe.tags:
            ...
            if "ingredients" in simple_updates and simple_updates["ingredients"]:
                assert isinstance(simple_updates["ingredients"], list), "Single-item ingredients should be list"
                assert len(simple_updates["ingredients"]) >= 1, "Should have at least one ingredient"
            
            if "tags" in simple_updates and simple_updates["tags"]:
                assert isinstance(simple_updates["tags"], frozenset), "Single-item tags should be frozenset"
    ...
    def test_frozen_set_ingredient_uniqueness(self):
        ...
        if max_recipe.ingredients:
            ...
            api_ingredient_positions = [ingredient.position for ingredient in max_recipe.ingredients]
            assert len(set(api_ingredient_positions)) == len(api_ingredient_positions), "API ingredient positions should be unique"
            
            if "ingredients" in updates:
                domain_ingredients = updates["ingredients"]
                domain_ingredient_positions = [ingredient.position for ingredient in domain_ingredients]
                
                # Verify uniqueness is maintained through conversion
                assert len(set(domain_ingredient_positions)) == len(domain_ingredient_positions), "Domain ingredient positions should remain unique"
                assert len(domain_ingredient_positions) == len(api_ingredient_positions), "Ingredient count should be preserved"
                
                # Verify same positions are preserved
                assert set(domain_ingredient_positions) == set(api_ingredient_positions), "Ingredient positions should be exactly preserved"
        ...
        if max_recipe.tags and "tags" in updates:
            ...
            for tag in domain_tags:
                ...
                assert hasattr(tag, 'author_id'), "Domain tag should have author_id attribute"
