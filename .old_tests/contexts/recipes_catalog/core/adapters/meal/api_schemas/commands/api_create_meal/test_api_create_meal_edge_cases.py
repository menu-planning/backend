...
class TestApiCreateMealBoundaryValues:
    ...
    @pytest.mark.parametrize("test_case_name,test_case_data", _generate_boundary_test_cases())
    ...
    @pytest.mark.parametrize("scenario", [
        ...
        {'field': 'name', 'value': 'A' * 255, 'should_pass': True},   # name has max_length=255
        {'field': 'name', 'value': 'A' * 256, 'should_pass': False},  # over the limit
        ...
        {'field': 'name', 'value': 'A', 'should_pass': True},         # Single character name
        {'field': 'name', 'value': '', 'should_pass': False},         # Empty name should fail
    ])
...
class TestApiCreateMealErrorHandling:
    ...
    @pytest.mark.parametrize("test_case_name,invalid_data", _generate_field_validation_test_cases())
    def test_error_handling_with_field_validation_suite(self, test_case_name, invalid_data):
    ...
    @pytest.mark.parametrize("test_case_name,scenario", _generate_missing_fields_test_cases())
    def test_error_handling_with_missing_required_fields(self, test_case_name, scenario):
    ...
    @pytest.mark.parametrize("test_case_name,invalid_data", _generate_nested_object_test_cases())
    ...
    @pytest.mark.parametrize("scenario", [
        ...
        {'field': 'image_url', 'value': 'ftp://invalid', 'error_contains': 'image_url'},
...
class TestApiCreateMealComplexScenarios:
    ...
    @pytest.mark.parametrize("test_case_name,scenario", _generate_comprehensive_validation_test_cases())
    ...
    @pytest.mark.parametrize("test_case_name,scenario", _generate_systematic_error_test_cases())
    ...
    def test_complex_recipes_scenarios(self):
        ...
        single_recipe = [
            create_simple_api_recipe(name='Simple Recipe', author_id=author_id, meal_id=meal_id)
        ]
        kwargs = create_api_create_meal_kwargs(author_id=author_id, recipes=single_recipe)
        ...
        multiple_recipes = [
            create_simple_api_recipe(name='Recipe 1', author_id=author_id, meal_id=meal_id),
            create_complex_api_recipe(name='Recipe 2', author_id=author_id, meal_id=meal_id)
        ]
        kwargs = create_api_create_meal_kwargs(author_id=author_id, recipes=multiple_recipes)
    ...
    @pytest.mark.parametrize("name,author_id,menu_id", [
    ...
    def test_comprehensive_field_validation_matrix(self, name, author_id, menu_id):
        ...
        kwargs = create_api_create_meal_kwargs(
            name=name,
            author_id=author_id,
            menu_id=menu_id if menu_id is not None else str(uuid4())  # Ensure menu_id is never None for ApiCreateMeal
        )
...
class TestApiCreateMealRealisticScenarios:
    ...
    @pytest.mark.parametrize("scenario_data", REALISTIC_MEAL_SCENARIOS[:10])  # Test first 10 scenarios
    def test_realistic_scenarios_from_factory(self, scenario_data):
        ...
        tags = []
        if "tags" in scenario_copy and scenario_copy["tags"]:
            for tag_data in scenario_copy["tags"]:
                if isinstance(tag_data, dict) and "key" in tag_data and "value" in tag_data:
        ...
        recipes = []
        if "recipes" in scenario_copy and scenario_copy["recipes"]:
...
class TestApiCreateMealPerformance:
    ...
    def test_performance_with_large_datasets(self):
        ...
        large_recipes = [
            create_simple_api_recipe(name=f'Recipe {i}', author_id=author_id, meal_id=meal_id)
            for i in range(20)  # 20 recipes per meal
        ]
        
        large_tags = frozenset([
            create_api_meal_tag(key=f'tag{i}', value=f'value{i}', author_id=author_id)
    ...
    def test_performance_with_minimal_data(self, meal_index):
        ...
        kwargs = create_minimal_api_create_meal_kwargs(
            name=f'Minimal Meal {meal_index}',
            author_id=str(uuid4())
        )
...
class TestApiCreateMealComplexRelationships:
    ...
    def test_author_id_consistency_validation(self):
        ...
        recipes = [
            create_simple_api_recipe(name='Recipe 1', author_id=author_id, meal_id=meal_id),
            create_simple_api_recipe(name='Recipe 2', author_id=author_id, meal_id=meal_id)
        ]
