...
import pytest
from old_tests_v0.contexts.client_onboarding.fakes.fake_typeform_api import (
    FakeTypeFormAPI,
...
class PerformanceRateLimitTester:
    ...
    async def make_requests(
        self, client: TypeFormClient, request_count: int
    ) -> dict[str, Any]:
    ...
    def analyze_rate_compliance(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        compliance = actual_rate <= (
            self.target_rate_limit * 1.15
        )  # 15% tolerance for measurement overhead
        ...
        duration_compliance = results["total_duration"] >= (
            expected_min_duration * 0.9
        )  # 10% tolerance
        ...
        return {
            ...
            "rate_deviation_percent": (
                (actual_rate - self.target_rate_limit) / self.target_rate_limit
            )
            * 100,
...
class TestRateLimitingPerformance:
    ...
    def setup_method(self):
        ...
        assert (
            self.client.rate_limit_validator.requests_per_second
    ...
    async def test_rate_limiting_baseline_performance(self):
        ...
        assert (
            results["successful_operations"] == request_count
        ), f"All {request_count} requests should succeed"

        # Rate compliance assertions
        assert compliance[
            "overall_compliant"
        ], f"Should be rate compliant: {compliance}"
        assert (
            results["actual_rate"] <= self.target_rate_limit * 1.2
        ), f"Actual rate {results['actual_rate']:.2f} should not exceed {self.target_rate_limit * 1.2:.2f}"
        ...
        assert (
            results["total_duration"] >= expected_duration * 0.9
        ), f"Duration {results['total_duration']:.2f}s should be >= {expected_duration * 0.9:.2f}s"

        print(f"✅ Baseline rate limiting performance:")
        print(
            f"   {request_count} requests in {results['total_duration']:.2f}s ({results['actual_rate']:.2f} req/s)"
        )
    ...
    async def test_sustained_load_rate_limiting_compliance(self):
        ...
        assert (
            results["successful_operations"] == request_count
        ), f"All {request_count} requests should succeed under sustained load"
        assert compliance[
            "overall_compliant"
        ], f"Should maintain compliance under sustained load: {compliance}"

        # Rate should be consistently limited
        assert (
            results["actual_rate"] <= self.target_rate_limit * 1.15
        ), f"Sustained actual rate {results['actual_rate']:.2f} should not exceed limit"
        ...
        if len(request_times) > 1:
            ...
            assert (
                timing_variance < 0.1
            ), f"Request timing variance {timing_variance:.3f} should be low (consistent rate limiting)"

        # Get rate limit status from client
        rate_status = await self.client.get_rate_limit_status()
        assert rate_status[
            "is_compliant"
        ], f"Client should report compliance: {rate_status}"
    ...
    async def test_concurrent_requests_rate_limiting(self):
        ...
        total_successful = sum(
            result["successful_requests"] for result in batch_results
        )
        ...
        ), f"All {total_requests} concurrent requests should succeed"

        # Rate limiting should still be enforced even with concurrency
        assert (
            actual_rate <= self.target_rate_limit * 1.2
        ), f"Concurrent rate {actual_rate:.2f} should not exceed limit"

        # Total duration should respect rate limiting
        expected_min_duration = (total_requests - 1) / self.target_rate_limit
        assert (
            total_duration >= expected_min_duration * 0.9
        ), f"Concurrent duration {total_duration:.2f}s should respect rate limiting"

        print(f"✅ Concurrent requests rate limiting:")
        print(
            f"   {total_requests} requests ({concurrent_batches} batches) in {total_duration:.2f}s"
        )
        print(
            f"   Actual rate: {actual_rate:.2f} req/s (target: {self.target_rate_limit} req/s)"
        )
    ...
    async def test_rate_limiting_memory_efficiency_under_load(self):
        ...
        for round_num in range(rounds):
            ...
            results = await self.rate_limit_tester.make_requests(
                self.client, requests_per_round
            )
            ...
            assert compliance[
                "overall_compliant"
            ], f"Round {round_num + 1} should be rate compliant"

            print(
                f"   Round {round_num + 1}: {requests_per_round} requests, "
        ...
        ), f"Memory grew by {memory_growth_mb:.1f}MB, expected < {max_acceptable_growth_mb}MB"
        ...
        if len(memory_samples) > 1:
            ...
            assert (
                memory_variance < 100
            ), f"Memory variance {memory_variance:.1f} should be stable"

        print(f"✅ Rate limiting memory efficiency:")
        print(f"   {rounds * requests_per_round} total requests across {rounds} rounds")
        print(
            f"   Memory: {initial_memory_mb:.1f}MB → {final_memory_mb:.1f}MB (+{memory_growth_mb:.1f}MB)"
        )
    ...
    async def test_rate_limiting_accuracy_under_timing_pressure(self):
        ...
        for i in range(1, len(request_timestamps)):
            interval = (
                request_timestamps[i]["start"] - request_timestamps[i - 1]["start"]
            )
        ...
        if inter_request_intervals:
            ...
            assert (
                abs(avg_interval - expected_interval) < 0.1
            ), f"Average interval {avg_interval:.3f}s should be ~{expected_interval:.3f}s"

            # Interval variance should be low (consistent timing)
            assert (
                interval_variance < 0.05
            ), f"Interval variance {interval_variance:.4f} should be low (consistent timing)"
            ...
            compliance_percentage = (
                compliant_intervals / len(inter_request_intervals)
            ) * 100
            assert (
                compliance_percentage >= 80
            ), f"At least 80% of intervals should be compliant, got {compliance_percentage:.1f}%"

        print(f"✅ Rate limiting timing accuracy:")
        print(f"   {request_count} requests with precise timing measurement")
        print(
            f"   Average interval: {avg_interval:.3f}s (target: {expected_interval:.3f}s)"
        )
        print(f"   Interval variance: {interval_variance:.4f}")
        print(
            f"   Compliance: {compliance_percentage:.1f}% of intervals within tolerance"
        )
...
class TestRateLimitingPerformanceStress:
    ...
    async def test_high_volume_rate_limiting_stress(self):
        ...
        assert (
            results["successful_operations"] == request_count
        ), f"All {request_count} requests should succeed under stress"
        assert compliance[
            "overall_compliant"
        ], f"Should maintain compliance under high volume stress: {compliance}"

        # Rate limiting should remain effective under stress
        assert (
            results["actual_rate"] <= self.target_rate_limit * 1.15
        ), f"Stress rate {results['actual_rate']:.2f} should not exceed limit"

        # Check rate limit status after stress test
        final_status = await self.client.get_rate_limit_status()
        assert final_status[
            "is_compliant"
        ], f"Should report compliance after stress: {final_status}"

        print(f"✅ High volume rate limiting stress test:")
        print(
            f"   {request_count} requests in {stress_duration:.2f}s ({results['actual_rate']:.2f} req/s)"
        )
    ...
    async def test_burst_request_rate_limiting_resilience(self):
        ...
        for i, interval in enumerate(burst_intervals):
            ...
            print(
                f"   Burst {i + 1}: {burst_count} requests in {burst_duration:.2f}s ({burst_rate:.2f} req/s)"
            )
        ...
        assert (
            total_requests == len(burst_intervals) * burst_count
        ), "All burst requests should eventually succeed"

        # Overall rate should still comply with limits despite bursts
        assert (
            overall_rate <= self.target_rate_limit * 1.2
        ), f"Overall burst rate {overall_rate:.2f} should be rate-limited"

        # Rate limiter should recover properly after bursts
        final_status = await self.client.get_rate_limit_status()
        assert final_status[
            "is_compliant"
        ], f"Should recover compliance after bursts: {final_status}"
...
class TestRateLimitingValidatorPerformance:
    ...
    async def test_rate_limit_enforcement_performance(self):
        ...
        assert (
            fast_percentage >= 0.5
        ), f"At least 0.5% of enforcements should be fast, got {fast_percentage:.1f}%"

        # Maximum enforcement time should not be excessive
        assert (
            max_enforcement_time < 1.0
        ), f"Max enforcement time {max_enforcement_time:.3f}s should be < 1.0s"

        print(f"✅ Rate limit enforcement performance:")
        print(
            f"   {enforcement_count} enforcements: avg {avg_enforcement_time*1000:.2f}ms, max {max_enforcement_time*1000:.2f}ms"
        )
    ...
    async def test_rate_limit_status_monitoring_performance(self):
        ...
        assert (
            avg_status_time < 0.001
        ), f"Average status check time {avg_status_time*1000:.2f}ms should be < 1ms"
        assert (
            max_status_time < 0.005
        ), f"Max status check time {max_status_time*1000:.2f}ms should be < 5ms"

        print(f"✅ Rate limit status monitoring performance:")
        print(
            f"   {monitoring_count} status checks: avg {avg_status_time*1000:.3f}ms, max {max_status_time*1000:.3f}ms"
        )
    ...
    async def test_concurrent_rate_limit_validator_performance(self):
        ...
        total_status_checks = sum(
            result["status_check_count"] for result in task_results
        )
        avg_task_duration = statistics.mean(
            result["duration"] for result in task_results
        )

        # Concurrent performance assertions
        assert (
            len(task_results) == concurrent_operations
        ...
        print(
            f"   {total_enforcements} enforcements, {total_status_checks} status checks"
        )
