...
class TestReplayAttackProtection:
    ...
    async def test_webhook_replay_attack_same_signature_blocked(
        ...
        async with async_benchmark_timer() as timer:
            ...
            is_valid_1, err_1 = await verifier.verify_webhook_request(
                payload=payload_str, headers=headers
            )
    ...
    async def test_webhook_timestamp_validation_expired_request(
        self, async_benchmark_timer
    ):
        ...
        async with async_benchmark_timer() as timer:
            ...
            payload_str = json.dumps(
                webhook_payload, ensure_ascii=False, separators=(",", ":")
            )
            ...
            is_valid, error_msg = await verifier.verify_webhook_request(
                payload=payload_str, headers=headers, timestamp_tolerance_minutes=1
            )
    ...
    async def test_webhook_timestamp_validation_future_request(
        self, async_benchmark_timer
    ):
        ...
        future_timestamp = datetime.now(UTC) + timedelta(minutes=10)
        webhook_payload = create_webhook_payload_kwargs(
            form_response={
                "form_id": f"form_{get_next_webhook_counter()}",
                "token": f"token_{get_next_webhook_counter()}",
        ...
        async with async_benchmark_timer() as timer:
            ...
            is_valid, error_msg = await verifier.verify_webhook_request(
                payload=payload_str, headers=headers, timestamp_tolerance_minutes=5
            )
    ...
    async def test_webhook_replay_protection_with_nonce_simulation(
        self, async_benchmark_timer
    ):
    ...
    async def test_webhook_signature_replay_with_different_payload(
        self, async_benchmark_timer
    ):
        ...
        async with async_benchmark_timer() as timer:
            ...
            verifier = WebhookSecurityVerifier(webhook_secret)
            is_valid_1, _ = await verifier.verify_webhook_request(
                payload=payload_str_1, headers=headers_1
            )
            assert is_valid_1 is True
            ...
            is_valid_2, err_2 = await verifier.verify_webhook_request(
                payload=payload_str_2, headers=malicious_headers
            )

            # Should be rejected due to signature mismatch
            assert is_valid_2 is False

    async def test_webhook_security_timing_attack_prevention(
        self, async_benchmark_timer
    ):
        ...
        async with async_benchmark_timer() as timer:
            ...
            valid_result = await verifier._verify_signature(
                payload_str, signature_value
            )
            ...
            for invalid_sig in invalid_signatures:
                if invalid_sig.startswith("sha256="):
                    ...
                    invalid_result = await verifier._verify_signature(
                        payload_str, sig_value
                    )
                    assert invalid_result is False

    async def test_webhook_replay_protection_comprehensive_scenarios(
        self, async_benchmark_timer
    ):
        ...
        async with async_benchmark_timer() as timer:
            ...
            is_valid_2, err_2 = await verifier.verify_webhook_request(
                payload=payload_str, headers=modified_headers
            )
            assert is_valid_2 is False
            assert err_2 is not None and (
                "timestamp" in err_2.lower() or "replay" in err_2.lower()
            )
