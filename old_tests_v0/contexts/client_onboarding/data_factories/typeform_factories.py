...
def create_form_info_kwargs(**kwargs) -> Dict[str, Any]:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get(
            "description", f"Test form description for testing - Form {counter}"
        ),
        ...
        "public_url": kwargs.get(
            "public_url", f"https://test-form.typeform.com/to/test_form_{counter:03d}"
        ),
        ...
        "theme": kwargs.get(
            "theme", {"href": f"https://api.typeform.com/themes/test_theme_{counter}"}
        ),
        "settings": kwargs.get(
            "settings",
            {
        ...
        "thankyou_screens": kwargs.get(
            "thankyou_screens",
            [{"id": f"thankyou_{counter}", "title": "Thank you!", "properties": {}}],
        ),
        "fields": kwargs.get(
            "fields",
            [
...
def create_webhook_info_kwargs(**kwargs) -> Dict[str, Any]:
    ...
    final_kwargs = {
        ...
        "url": kwargs.get(
            "url", f"https://test-webhook.example.com/hook/{webhook_counter}"
        ),
...
def create_client_onboarding_form_response_kwargs(**kwargs) -> Dict[str, Any]:
    ...
    default_answers = [
        ...
        create_form_answer_kwargs(
            ...
            type="choice",
            choice={"label": "Restaurant", "other": None},
        ),
        create_form_answer_kwargs(
            field={
                "id": f"monthly_budget_{counter}",
    ...
    final_kwargs = {
        "form_id": kwargs.get(
            "form_id", default_definition["id"]
        ),  # Add direct form_id field
        ...
        "landed_at": kwargs.get(
            "landed_at", (datetime.now(UTC) - timedelta(minutes=5)).isoformat()
        ),
        "submitted_at": kwargs.get("submitted_at", datetime.now(UTC).isoformat()),
...
def create_webhook_payload_kwargs(**kwargs) -> Dict[str, Any]:
    ...
    default_form_response = create_client_onboarding_form_response_kwargs(
        **(provided_form_response or {})
    )
...
def create_typeform_signature_headers(**kwargs) -> Dict[str, str]:
    ...
    final_kwargs = {
        "Typeform-Signature": kwargs.get(
            "Typeform-Signature", f"sha256=test_signature_{counter}"
        ),
...
def create_not_found_error_kwargs(**kwargs) -> Dict[str, Any]:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get(
            "description", "The requested form or webhook does not exist"
        ),
    }
...
def create_realistic_onboarding_scenario(**kwargs) -> Dict[str, Any]:
    ...
    default_signature_headers = create_typeform_signature_headers(
        **kwargs.get("signature_headers", {})
    )
