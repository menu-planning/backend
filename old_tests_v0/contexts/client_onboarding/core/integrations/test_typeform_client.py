...
class TestTypeFormClientCRUDOperations:
    ...
    async def test_get_form_success(self, mock_client_factory, async_benchmark_timer):
        ...
        assert form_dict.get("public_url", "").startswith(
            "https://fake-form.typeform.com"
        )
    ...
    async def test_list_webhooks_success(
        self, mock_client_factory, async_benchmark_timer
    ):
    ...
    async def test_create_webhook_success(
        ...
        async with async_benchmark_timer() as timer:
            webhook_info = await client.create_webhook(
                form_id, webhook_url, "client_onboarding"
            )
    ...
    async def test_update_webhook_success(
        ...
        async with async_benchmark_timer() as timer:
            webhook_info = await client.update_webhook(
                form_id,
                "client_onboarding",
                "https://updated-webhook.example.com/hook",
                False,
            )
        ...
        assert (
            timer.elapsed < 1.5
        ), f"Webhook update should be reasonably fast, took {timer.elapsed:.3f}s"
...
class TestTypeFormClientAuthentication:
    ...
    def mock_authentication_client(self):
        ...
        def _create_client(api_key="valid_key", simulate_auth_error=False):
            ...
            mock_client.delete = lambda url, **kwargs: mock_request(
                "DELETE", url, **kwargs
            )
    ...
    async def test_authentication_success(
        self, mock_authentication_client, async_benchmark_timer
    ):
    ...
    async def test_authentication_failure_missing_key(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "src.contexts.client_onboarding.config.config.typeform_api_key", ""
        )
    ...
    async def test_authentication_error_scenario_comprehensive(
        self, mock_authentication_client
    ):
        ...
        for api_key, scenario_name in scenarios:
            with pytest.raises((TypeFormAuthenticationError, TypeFormAPIError)):
                client = mock_authentication_client(
                    api_key=api_key, simulate_auth_error=True
                )
                form_id = f"test_form_{get_next_typeform_api_counter():03d}"
                await client.get_form(form_id)
...
class TestTypeFormClientRateLimiting:
    ...
    async def test_rate_limiting_retry_success(
        self, rate_limit_client, async_benchmark_timer
    ):
    ...
    async def test_retry_logic_exponential_backoff(
        ...
        async with async_benchmark_timer() as timer:
            ...
            form_info = await client.get_form_with_exponential_backoff(
                form_id, max_retries=2, base_delay=0.1
            )
    ...
    async def test_rate_limiting_performance_degradation(
        self, rate_limit_client, async_benchmark_timer
    ):
        ...
        form_ids = [
            f"test_form_{get_next_typeform_api_counter():03d}" for _ in range(3)
        ]
...
class TestTypeFormClientRateLimitingCompliance:
    ...
    async def test_rate_limit_configuration_compliance(self, compliance_client):
        ...
        assert (
            validator.requests_per_second == 2
        ), "Rate limit should be 2 req/sec for TypeForm compliance"
    ...
    async def test_rate_limit_enforcement_timing(
        self, compliance_client, async_benchmark_timer
    ):
        ...
        assert (
            timer.elapsed >= 1.0
        ), f"3 requests should take at least 1s with 2 req/sec limit, took {timer.elapsed:.3f}s"
    ...
    async def test_rate_limit_status_monitoring(self, compliance_client):
        ...
        assert (
            status_after["compliance_percentage"] >= 90
    ...
    async def test_configuration_validation_warnings(self):
        ...
        for rate_limit, expected_warning in test_cases:
            ...
            warning_found = any(
                expected_warning in w for w in validation_result["warnings"]
            )
            assert (
                warning_found
            ), f"Expected warning '{expected_warning}' for rate limit {rate_limit}"
    ...
    async def test_concurrent_request_rate_limiting(
        ...
        assert (
            timer.elapsed >= 1.5
        ), f"4 concurrent requests should take at least 1.5s, took {timer.elapsed:.3f}s"
    ...
    async def test_compliance_under_sustained_load(
        self, compliance_client, async_benchmark_timer
    ):
        ...
        assert (
            timer.elapsed >= expected_min_time
        ), f"Sustained load test: expected >= {expected_min_time}s, got {timer.elapsed:.3f}s"

        # Verify compliance was maintained
        final_status = await client.rate_limit_validator.get_rate_limit_status()
        assert (
            final_status["is_compliant"] == True
        ), "Should maintain compliance under sustained load"

    async def test_rate_limit_integration_with_webhook_operations(
