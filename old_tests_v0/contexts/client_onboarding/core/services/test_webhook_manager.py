...
class TestWebhookManager:
    ...
    def create_webhook_manager(
        self,
    ) -> tuple[WebhookManager, TypeFormClient, FakeUnitOfWork]:
        """Create webhook manager with fake dependencies."""
        fake_api = FakeTypeFormAPI()
        fake_uow = (
            FakeUnitOfWork()
        )  # No arguments - repositories are initialized internally
    ...
    async def test_setup_onboarding_form_webhook_complete_lifecycle(self):
        ...
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Test Onboarding Form"
        )
    ...
    async def test_setup_webhook_for_existing_form(self):
        ...
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Existing Form"
        )
        # Note: We don't need to manually add forms anymore as FakeTypeFormAPI creates them on-demand
        ...
        assert (
            webhook_info.url.startswith("https://fake-webhook.example.com")
            or webhook_info.url == webhook_url
        )
        assert webhook_info.enabled is True
    ...
    async def test_update_webhook_url_lifecycle(self):
        ...
        new_url = (
            f"https://api.example.com/webhooks/updated_{get_next_webhook_counter()}"
        )
        ...
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Update Test Form"
        )
        # Note: We don't need to manually add forms anymore as FakeTypeFormAPI creates them on-demand

        # Create initial webhook
        initial_webhook = await fake_client.create_webhook(
            form_id=typeform_id, webhook_url=initial_url, tag="client_onboarding"
        )

        # Update webhook URL
        updated_webhook = await webhook_manager.update_webhook_url(
            uow=fake_uow, onboarding_form_id=form.id, new_webhook_url=new_url
        )
    ...
    async def test_disable_webhook_lifecycle(self):
        ...
        # Setup fake TypeForm data using proper factory
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Disable Test Form"
        )
        # Note: We don't need to manually add forms anymore as FakeTypeFormAPI creates them on-demand

        # Create active webhook
        webhook = await fake_client.create_webhook(
            form_id=typeform_id, webhook_url=webhook_url, tag="client_onboarding"
        )
        ...
        disabled_webhook = await fake_client.get_webhook(
            typeform_id, "client_onboarding"
        )
    ...
    async def test_enable_webhook_lifecycle(self):
        ...
        # Setup fake TypeForm data using proper factory
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Enable Test Form"
        )
        # Note: We don't need to manually add forms anymore as FakeTypeFormAPI creates them on-demand
        ...
        enabled_webhook = await fake_client.get_webhook(
            typeform_id, "client_onboarding"
        )
    ...
    async def test_delete_webhook_configuration_lifecycle(self):
        ...
        # Setup fake TypeForm data using proper factory
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Delete Test Form"
        )
        # Note: We don't need to manually add forms anymore as FakeTypeFormAPI creates them on-demand
    ...
    async def test_get_comprehensive_webhook_status(self):
        ...
        # Setup fake TypeForm data using proper factory
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Status Test Form"
        )
        # Note: We don't need to manually add forms anymore as FakeTypeFormAPI creates them on-demand
    ...
    async def test_synchronize_webhook_status_drift_detection(self):
        ...
        # Setup fake TypeForm data using proper factory
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Sync Test Form"
        )
        # Note: We don't need to manually add forms anymore as FakeTypeFormAPI creates them on-demand
    ...
    async def test_bulk_webhook_status_check(self):
        ...
        for i in range(3):
            typeform_id = f"form_{get_next_onboarding_form_id()}"
            webhook_url = (
                f"https://api.example.com/webhooks/{get_next_webhook_counter()}"
            )
            ...
            form = OnboardingForm(
                ...
                webhook_url=webhook_url,
                status=(
                    OnboardingFormStatus.ACTIVE
                    if i % 2 == 0
                    else OnboardingFormStatus.PAUSED
                ),
                created_at=datetime.now(UTC),
                updated_at=datetime.now(UTC),
            )
            ...
            # Setup fake TypeForm data using proper factory
            fake_form_kwargs = create_form_info_kwargs(
                id=typeform_id, title=f"Bulk Test Form {i}"
            )
        ...
        assert expected_form_ids.issubset(
            result_form_ids
        ), f"Missing forms: {expected_form_ids - result_form_ids}"
    ...
    async def test_error_handling_and_rollback(self, monkeypatch):
        ...
        # Setup fake form in TypeForm API using proper factory
        fake_form_kwargs = create_form_info_kwargs(
            id=typeform_id, title="Error Test Form"
        )
        # Note: We don't need to manually add forms anymore as FakeTypeFormAPI creates them on-demand

        # Force a webhook creation error by mocking config to have no webhook endpoint
        monkeypatch.setattr(
            "src.contexts.client_onboarding.config.config.webhook_endpoint_url", ""
        )

        # Execute webhook setup and expect error due to missing webhook URL
        with pytest.raises(
            ValueError, match="Webhook URL must be provided or configured"
        ):
            await webhook_manager.setup_onboarding_form_webhook(
                uow=fake_uow,
                user_id=user_id,
        ...
        assert (
            stored_form is None
        ), f"Form {typeform_id} should not be persisted when validation fails early"
