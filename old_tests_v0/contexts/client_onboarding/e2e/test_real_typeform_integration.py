...
class TestRealTypeformIntegration:
    ...
    async def setup(self):
        ...
        self.test_user_id = (
            get_next_user_id() + 1000
        )  # Start from 1001 to avoid conflicts
    ...
    async def test_complete_onboarding_form_lifecycle(self):
        ...
        if webhook_info and webhook_info.id:
            ...
            stored_responses = cast(
                "list[FormResponse]", await self.fake_uow.form_responses.get_all()
            )
    ...
    async def test_real_webhook_creation_and_validation(self):
        ...
        onboarding_form = create_onboarding_form(
            id=form_id, typeform_id=TYPEFORM_FORM_ID_STR, user_id=self.test_user_id
        )
        await self.fake_uow.onboarding_forms.add(onboarding_form)
        self.created_form_ids.append(form_id)
        ...
        if webhook_info and webhook_info.id:
            ...
            status_info = await self.webhook_manager.get_comprehensive_webhook_status(
                uow=self.fake_uow, onboarding_form_id=form.id
            )
            ...
            try:
                ...
                print(
                    "ℹ️  Webhook update skipped - API key lacks update permissions (this is normal)"
                )
    ...
    async def test_real_webhook_error_scenarios(self):
        ...
        try:
            invalid_url_result = (
                await self.webhook_manager.typeform_client.create_webhook(
                    form_id=TYPEFORM_FORM_ID_STR,
    ...
    async def test_concurrent_real_webhook_operations(self):
        ...
        async def create_webhook_for_form(form_data):
            ...
            webhook_url = (
                f"https://concurrent-{get_next_webhook_counter()}.ngrok.io/webhook"
            )
    ...
    async def test_webhook_signature_validation_with_real_secret(self):
        ...
        verifier = WebhookSecurityVerifier(TYPEFORM_WEBHOOK_SECRET)
        is_valid, _ = await verifier.verify_webhook_request(
            payload=payload_json, headers=valid_headers
        )
        assert is_valid is True
        success, error, response_id = await process_typeform_webhook(
            payload=payload_json, headers=valid_headers, uow_factory=self.uow_factory
        )
        ...
        is_valid, _ = await verifier.verify_webhook_request(
            payload=payload_json, headers=invalid_headers
        )
    ...
    async def test_end_to_end_user_creation_flow(self):
        ...
        if (
            "form_response" in webhook_payload
            and "answers" in webhook_payload["form_response"]
        ):
        ...
        if stored_response.client_identifiers:
            user_identifiers = [
                identifier
                for identifier in stored_response.client_identifiers
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    assert response.status_code in [
                        200,
                        404,
                    ]  # 404 is fine if no health endpoint
