...
from old_tests_v0.contexts.client_onboarding.e2e.conftest import (
...
class TestTypeformFeatures:
    ...
    async def test_complete_onboarding_form_management(
        ...
        try:
            ...
            if webhook_info and webhook_info.id:
                ...
                new_webhook_url = get_test_webhook_url_with_path(
                    test_webhook_url, "/updated-webhook"
                )
    ...
    async def test_form_response_processing_features(
        self, fake_uow, typeform_config, unique_form_id
    ):
        ...
        onboarding_form = create_onboarding_form(id=form_id, typeform_id=test_form_id)
        await fake_uow.onboarding_forms.add(onboarding_form)
        ...
        webhook_payload = create_typeform_webhook_payload(
            form_response={"form_id": test_form_id, "token": response_token}
        )
        ...
        client_webhook_payload = create_typeform_webhook_payload(
            form_response={"form_id": test_form_id, "token": client_response_token},
        ...
        all_responses = cast(
            "list[FormResponse]", await fake_uow.form_responses.get_all()
        )
        assert len(all_responses) == 2

        client_response = next(
            r for r in all_responses if r.response_id == client_response_token
        )
        ...
        for i in range(5):
            ...
            payload = create_typeform_webhook_payload(
                form_response={"form_id": test_form_id, "token": token}
            )
            rapid_payloads.append(payload)

        # Process concurrently
        tasks = [
            process_webhook_with_signature(fake_uow, payload)
            for payload in rapid_payloads
        ]
    ...
    async def test_webhook_security_features(
        ...
        status_code, response = await process_webhook_with_signature(
            fake_uow, webhook_payload, typeform_config["webhook_secret"]
        )
    ...
    async def test_error_handling_features(
        ...
        status_code, response = await process_webhook_with_signature(
            fake_uow, nonexistent_payload
        )

        assert status_code == 404
        assert (
            response["error"] == "form_not_found"
        )  # Corrected based on actual error message
        ...
        with patch.object(
            fake_uow, "commit", side_effect=Exception("Database connection lost")
        ):
            status_code, response = await process_webhook_with_signature(
                fake_uow, error_payload
            )
    ...
    async def test_webhook_management_features(
        ...
        try:
            ...
            webhook_url = get_test_webhook_url_with_path(
                test_webhook_url, "/management-test"
            )
            ...
            if webhook_info and webhook_info.id:
                ...
                status_info = await webhook_manager.get_comprehensive_webhook_status(
                    uow=fake_uow, onboarding_form_id=form.id
                )
                ...
                updated_url = get_test_webhook_url_with_path(
                    test_webhook_url, "/updated-management"
                )
                ...
                delete_result = await webhook_manager.typeform_client.delete_webhook(
                    form_id=typeform_config["form_id"], tag="client_onboarding"
                )
                ...
                try:
                    deleted_status = (
                        await webhook_manager.get_comprehensive_webhook_status(
                            uow=fake_uow, onboarding_form_id=form.id
                        )
                    )
    ...
    async def test_performance_features(
        self, fake_uow, webhook_handler, typeform_config, unique_form_id
    ):
        ...
        batch_tasks = [
            process_webhook_with_signature(webhook_handler, payload)
        ...
        successful_load = sum(
            1 for status_code, _ in load_results if status_code == 200
        )
        ...
        assert (
            len(final_responses) == 1 + batch_size + 20
        )  # 1 (perf test) + 10 (batch) + 20 (load)
