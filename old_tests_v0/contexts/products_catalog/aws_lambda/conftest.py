...
def test_user_domain_object(mock_user_id):
    ...
    return User(id=mock_user_id, roles=frozenset([basic_role]))
...
def lambda_event_fetch_products(lambda_event_with_user):
    ...
    event["queryStringParameters"] = {"limit": "10", "is_food": "true"}
    event["multiValueQueryStringParameters"] = {"limit": ["10"], "is_food": ["true"]}
    return event
...
def mock_iam_provider_success(test_user_domain_object):
    ...
    with patch(
        ...
        mock_get.return_value = {"statusCode": 200, "body": test_user_domain_object}
        yield mock_get
...
def mock_iam_provider_unauthorized():
    ...
    with patch(
        ...
        mock_get.return_value = {"statusCode": 401, "body": {"message": "Unauthorized"}}
        yield mock_get
...
def mock_iam_provider_forbidden():
    ...
    with patch(
        ...
        mock_get.return_value = {"statusCode": 403, "body": {"message": "Forbidden"}}
        yield mock_get
...
def mock_business_logic():
    ...
    with (
        ...
        ) as mock_list_adapter,
        patch(
            "src.contexts.products_catalog.core.adapters.api_schemas.root_aggregate.api_product.ApiProduct"
        ) as mock_api_product,
        patch(
            "src.contexts.products_catalog.aws_lambda.fetch_product.container"
        ) as mock_fetch_container,
        patch(
            "src.contexts.products_catalog.aws_lambda.get_product_by_id.container"
        ...
        mock_api_product.model_dump_json.return_value = (
            b'{"id": "test", "name": "test"}'
        )
