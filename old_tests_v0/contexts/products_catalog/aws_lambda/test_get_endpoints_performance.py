...
class TestGetEndpointsPerformance:
    ...
    async def test_get_product_by_id_performance(
        self,
        mock_single_product_event,
        ...
        async with PerformanceTimer() as timer:
            result = await get_product_by_id.async_handler(
                mock_single_product_event, {}
            )
        ...
        assert (
            avg_time < 0.1
        ), f"Average time per request {avg_time:.6f}s exceeds 100ms limit"

    async def test_get_product_source_name_by_id_performance(
        self,
        mock_single_product_event,
        mock_localstack_environment,
        performance_test_data,
        ...
        # Performance test - single request
        async with PerformanceTimer() as timer:
            result = await get_product_source_name_by_id.async_handler(
                mock_single_product_event, {}
            )
    ...
    async def test_fetch_product_collection_performance(
        self, mock_collection_event, mock_successful_auth_response
    ):
        """Test fetch_product collection endpoint performance meets <200ms threshold."""

        with (
            patch(
                "src.contexts.products_catalog.aws_lambda.fetch_product.IAMProvider.get"
            ) as mock_auth,
            patch(
                "src.contexts.products_catalog.aws_lambda.fetch_product.Container"
            ) as mock_container,
        ):
            ...
            mock_type_adapter.dump_json.return_value = (
                b'[{"id": "product-1", "name": "Product 1"}]'
            )

            with (
                patch(
                    "src.contexts.products_catalog.aws_lambda.fetch_product.ProductListTypeAdapter",
                    mock_type_adapter,
                ),
                patch(
                    "src.contexts.products_catalog.aws_lambda.fetch_product.ApiProduct.from_domain"
                ) as mock_from_domain,
            ):
                ...
                async with PerformanceTimer() as timer:
                    result = await fetch_product.async_handler(
                        mock_collection_event, {}
                    )
                ...
                assert (
                    avg_time < 0.15
                ), f"Average collection time {avg_time:.6f}s exceeds 150ms limit"

                throughput = iterations / bulk_timer.get_execution_time()
                assert (
                    throughput > 5
                ), f"Collection throughput {throughput:.1f} req/sec is below 5 req/sec minimum"

    async def test_search_product_similar_name_performance(
        self, mock_search_event, mock_successful_auth_response
    ):
        """Test search_product_similar_name endpoint performance meets <300ms threshold."""

        with (
            patch(
                "src.contexts.products_catalog.aws_lambda.search_product_similar_name.IAMProvider.get"
            ) as mock_auth,
            patch(
                "src.contexts.products_catalog.aws_lambda.search_product_similar_name.Container"
            ) as mock_container,
        ):
            ...
            mock_type_adapter = MagicMock()
            mock_type_adapter.dump_json.return_value = (
                b'[{"id": "product-1", "name": "Chicken Product 1"}]'
            )

            with (
                patch(
                    "src.contexts.products_catalog.aws_lambda.search_product_similar_name.ProductListTypeAdapter",
                    mock_type_adapter,
                ),
                patch(
                    "src.contexts.products_catalog.aws_lambda.search_product_similar_name.ApiProduct.from_domain"
                ) as mock_from_domain,
            ):
                ...
                async with PerformanceTimer() as timer:
                    result = await search_product_similar_name.async_handler(
                        mock_search_event, {}
                    )
                ...
                assert (
                    avg_time < 0.25
                ), f"Average search time {avg_time:.6f}s exceeds 250ms limit"

                throughput = iterations / bulk_timer.get_execution_time()
                assert (
                    throughput > 3
                ), f"Search throughput {throughput:.1f} req/sec is below 3 req/sec minimum"

    async def test_fetch_product_source_name_collection_performance(
        self, mock_collection_event, mock_successful_auth_response
    ):
        ...
        with (
            patch(
                "src.contexts.products_catalog.aws_lambda.fetch_product_source_name.IAMProvider.get"
            ) as mock_auth,
            patch(
                "src.contexts.products_catalog.aws_lambda.fetch_product_source_name.Container"
            ) as mock_container,
        ):
            ...
            with patch(
                "src.contexts.products_catalog.aws_lambda.fetch_product_source_name.ApiSource.from_domain"
            ) as mock_from_domain:
                ...
                # Performance test - single collection request
                async with PerformanceTimer() as timer:
                    result = await fetch_product_source_name.async_handler(
                        mock_collection_event, {}
                    )
                ...
                timer.assert_faster_than(
                    0.2, "fetch_product_source_name collection request"
                )
    ...
    async def test_lambda_helpers_overhead_benchmark(
        self,
        mock_single_product_event,
        mock_localstack_environment,
        performance_test_data,
        ...
        async with PerformanceTimer() as lambda_helpers_timer:
            for _ in range(iterations):
                param = LambdaHelpers.extract_path_parameter(
                    mock_single_product_event, "id"
                )
                assert param == "product-123"
        ...
        assert (
            overhead_ratio < 5.0
        ), f"LambdaHelpers overhead {overhead_ratio:.2f}x is too high"

        # Both should be very fast (under 5ms per operation)
        assert (
            lambda_helpers_avg < 0.005
        ), f"LambdaHelpers too slow: {lambda_helpers_avg:.6f}s per operation"
        assert (
            direct_avg < 0.005
        ), f"Direct parsing unexpectedly slow: {direct_avg:.6f}s per operation"

    def test_lambda_helpers_utilities_performance_suite(
        self, mock_collection_event, mock_single_product_event
    ):
        ...
        performance_results["extract_path_parameter"] = (
            timer.get_execution_time() / iterations
        )
        ...
        performance_results["extract_query_parameters"] = (
            timer.get_execution_time() / iterations
        )
        ...
        with PerformanceTimer() as timer:
            for _ in range(iterations):
                params = LambdaHelpers.extract_multi_value_query_parameters(
                    mock_collection_event
                )
        performance_results["extract_multi_value_query_parameters"] = (
            timer.get_execution_time() / iterations
        )
        ...
        performance_results["is_localstack_environment"] = (
            timer.get_execution_time() / iterations
        )
        ...
        for utility_name, avg_time in performance_results.items():
            assert (
                avg_time < 0.0005
            ), f"{utility_name} too slow: {avg_time:.6f}s per operation"
        ...
        assert (
            total_utility_time < 0.003
        ), f"Combined utility overhead {total_utility_time:.6f}s too high"
