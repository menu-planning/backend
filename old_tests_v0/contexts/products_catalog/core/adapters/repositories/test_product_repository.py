...
class TestProductRepositoryCore:
    ...
    async def test_add_and_get_product(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
        """Test adding a product and retrieving it by ID"""
        # Given: A new ORM product with valid source_id
        product = create_ORM_product(
            name="Test Product for Add/Get", source_id="orm_test_source_1"
        )
    ...
    async def test_query_all_products(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
        product_count: int,
        ...
        assert (
            len(result) >= product_count
        )  # >= because other tests might have added products
    ...
    async def test_update_product(
        ...
        product = create_ORM_product(
            name="Original Name", shopping_name="Original Shopping Name"
        )
        test_session_with_sources.add(product)
        await test_session_with_sources.commit()
    ...
    async def test_get_sa_instance(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
    ...
    async def test_persist_all_products(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
...
class TestProductRepositoryFiltering:
    ...
    async def test_product_filtering_scenarios(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
        ...
        if scenario.get("skip_reason"):
            pytest.skip(
                f"SKIPPED: {scenario['skip_reason']} - Scenario: {scenario['scenario_id']}"
            )
        ...
        result = await product_repository_orm.query(
        ...
        if scenario["should_match"]:
            ...
            ), f"Scenario '{scenario['scenario_id']}' failed: {scenario['description']}"
        else:
            assert (
                product.id not in product_ids
            ), f"Scenario '{scenario['scenario_id']}' failed: {scenario['description']}"

    async def test_is_food_filter(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
        ...
        food_results = await product_repository_orm.query(
            filters={"is_food": True}, _return_sa_instance=True
        )
        non_food_results = await product_repository_orm.query(
            filters={"is_food": False}, _return_sa_instance=True
        )
    ...
    async def test_name_filter(
        ...
        apple_results = await product_repository_orm.query(
            filters={"name": "Fresh Apple"}, _return_sa_instance=True
        )
    ...
    async def test_barcode_filter(
        ...
        without_barcode = create_ORM_product(
            name="Product without Barcode", barcode=None
        )
        ...
        barcode_results = await product_repository_orm.query(
            filters={"barcode": with_barcode.barcode}, _return_sa_instance=True
        )
    ...
    async def test_combined_filters(
        ...
        processed_snack = create_ORM_processed_product(
            name="Processed Snack", is_food=True
        )
...
class TestProductRepositorySimilaritySearch:
    ...
    async def test_similarity_search_scenarios(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
        scenario: dict[str, Any],
        ...
        for expected_name in scenario["expected_matches"]:
            assert any(
                expected_name in name for name in result_names
            ), f"Expected '{expected_name}' to be found in similarity search results: {result_names}"

    async def test_similarity_search_first_word_partial_match(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
        ...
        products = [
            ...
            create_ORM_product(
                name="maca fuji"
            ),  # Should NOT match "banana" search when first_word=True
            create_ORM_product(
                name="maca verde"
            ),  # Should NOT match "banana" search when first_word=True
        ]
        ...
        results_no_filter = await product_repository_orm.list_top_similar_names(
            description="banana", filter_by_first_word_partial_match=False
        )
        ...
        results_with_filter = await product_repository_orm.list_top_similar_names(
            description="maca", filter_by_first_word_partial_match=True
        )
    ...
    async def test_similarity_search_with_barcodes(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
        ...
        results_no_barcode = await product_repository_orm.list_top_similar_names(
            description="Apple Juice", include_product_with_barcode=False
        )
        ...
        results_with_barcode = await product_repository_orm.list_top_similar_names(
            description="Apple Juice", include_product_with_barcode=True
        )
    ...
    async def test_similarity_search_limit(
        ...
        results = await product_repository_orm.list_top_similar_names(
            description="Apple", limit=5
        )

        # Then: Should return no more than limit
        assert len(results) <= 5

    async def test_similarity_search_food_only(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
...
class TestProductRepositoryHierarchicalFiltering:
    ...
    async def test_hierarchical_filtering_scenarios(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
        scenario: dict[str, Any],
        ...
        results = await product_repository_orm.query(
            filters=scenario["filter"], _return_sa_instance=True
        )
    ...
    async def test_source_filtering(
        ...
        manual_product = create_ORM_product(
            name="ORM Manual Product", source_id="orm_test_source_1"
        )
        tbca_product = create_ORM_product(
            name="ORM TBCA Product", source_id="orm_test_source_2"
        )
    ...
    @pytest.mark.skip(
        reason="Requires Brand entities and repository - Task 4.4.2-4.4.3"
    )
    async def test_brand_filtering_full(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
    ...
    @pytest.mark.skip(
        reason="Requires Category hierarchy entities and repository - Task 4.4.2-4.4.3"
    )
    async def test_category_hierarchy_filtering_full(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
    ...
    async def test_category_hierarchy_filtering(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
        ...
        products = [
            create_ORM_product(
                name=f"ORM Product {i}", category_id=None, parent_category_id=None
            )
            for i in range(3)
        ]
...
class TestProductRepositoryCustomMethods:
    ...
    async def test_list_filter_options(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
    ...
    async def test_list_filter_options_with_existing_filter(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
    ...
    async def test_list_all_brand_names(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
    ...
    @pytest.mark.skip(
        reason="Requires Brand entities with actual names - Task 4.4.2-4.4.3"
    )
    async def test_list_all_brand_names_full(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
    ...
    async def test_source_priority_sorting_behavior(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
        ...
        result_source_ids = [
            p.source_id for p in results if p.source_id.startswith("orm_test_source_")
        ]
        expected_source_ids = [
            f"orm_test_source_{suffix}" for suffix in ["1", "2", "3", "1", "2"]
        ]

        for expected_id in expected_source_ids:
            assert any(
                expected_id == result_id for result_id in result_source_ids
            ), f"Expected source_id '{expected_id}' not found in results"

    async def test_custom_sorting_with_source_priority(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
        ...
        products = [
            ...
            create_ORM_product(
                name="ORM Manual Product", source_id="orm_test_source_2"
            ),
        ...
        for product in products:
            assert any(
                product.source_id == source_id for source_id in result_source_ids
            )
...
class TestProductRepositoryErrorHandling:
    ...
    async def test_cannot_persist_product_not_added(
        self, product_repository_orm: ProductRepo
    ):
    ...
    async def test_duplicate_id_constraint(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
    ...
    async def test_invalid_filter_parameters(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
        ...
        with pytest.raises(FilterValidationError, match="Invalid filter keys"):
            await product_repository_orm.query(
    ...
    async def test_null_handling_in_filters(
        ...
        results_with_barcode = await product_repository_orm.query(
            filters={"barcode": "987654321123"}, _return_sa_instance=True
        )
        all_results = await product_repository_orm.query(_return_sa_instance=True)
    ...
    async def test_concurrent_access(
        ...
        products = [
            create_ORM_product(name=f"ORM Concurrent Product {i}") for i in range(5)
        ]
...
class TestProductRepositoryPerformance:
    ...
    async def test_query_performance_scenarios(
        ...
        if scenario.get("create_dataset"):
            dataset_info = create_ORM_test_dataset(
                product_count=scenario["dataset_size"]
            )
        ...
        async with async_benchmark_timer() as timer:
            if scenario["operation"] == "query_all":
                ...
                results = await product_repository_orm.query(
                    filters=scenario.get("filter", {}), _return_sa_instance=True
                )
    ...
    async def test_bulk_insert_performance(
        ...
        ), f"Throughput {throughput:.2f} products/sec, expected >= {min_throughput}"
    ...
    async def test_complex_query_performance(
        ...
        for i in range(50):
            product = create_ORM_product(
                name=f"ORM Complex Query Product {i}", is_food=True
            )
            products.append(product)
        ...
        async with async_benchmark_timer() as timer:
            ...
            similarity_results = await product_repository_orm.list_top_similar_names(
                "ORM Apple", limit=10
            )
            filter_options = await product_repository_orm.list_filter_options()
    ...
    async def test_memory_usage_with_large_resultset(
        self,
        product_repository_orm: ProductRepo,
        test_session_with_sources: AsyncSession,
    ):
