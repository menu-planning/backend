...
class TestMenuEdgeCases:
    ...
    def test_menu_with_varying_tag_counts(self, tag_count):
        ...
        menu = create_menu(description="Tag Count Test Menu", tags=tags)
    ...
    def test_menu_with_varying_week_ranges(self, week_range):
        ...
        menu = create_menu(
            description=f"Week Range Test Menu ({start_week}-{end_week})"
        )
        ...
        expected_meal_count = (
            (end_week - start_week + 1) * 7 * 3
        )  # weeks * days * meal_types
    ...
    def test_menu_with_extreme_nutrition_scenarios(self, nutrition_scenario):
        ...
        if nutrition_scenario == "zero_nutrition":
            ...
            for i in range(3):
                meal = create_menu_meal(
                    ...
                    nutri_facts=NutriFacts(
                        calories=NutriValue(
                            value=5000.0 + i * 1000, unit=MeasureUnit.ENERGY
                        ),
                        carbohydrate=NutriValue(
                            value=600.0 + i * 100, unit=MeasureUnit.GRAM
                        ),
                        protein=NutriValue(value=200.0 + i * 50, unit=MeasureUnit.GRAM),
                        total_fat=NutriValue(
                            value=300.0 + i * 50, unit=MeasureUnit.GRAM
                        ),
                    ),
                )
                meals.add(meal)
            ...
            for i in range(4):
                meal = create_menu_meal(
                    ...
                    nutri_facts=NutriFacts(
                        calories=NutriValue(
                            value=0.01 + i * 0.01, unit=MeasureUnit.ENERGY
                        ),
                        carbohydrate=NutriValue(
                            value=0.001 + i * 0.001, unit=MeasureUnit.GRAM
                        ),
                        protein=NutriValue(
                            value=0.001 + i * 0.001, unit=MeasureUnit.GRAM
                        ),
        ...
        for meal in menu.meals:
            ...
            _ = (
                meal.nutri_facts.carbohydrate.value
    ...
    @pytest.mark.parametrize(
        "menu_type", ["weekly", "special_event", "dietary_restriction"]
    )
    def test_specialized_menu_edge_cases(self, menu_type):
        ...
        if menu_type == "weekly":
            ...
            menu = create_dietary_restriction_menu(
                description="Dietary Restriction Edge Case Menu"
            )
    ...
    def test_menu_large_dataset_performance(self):
        ...
        sample_ids = {
            f"large_meal_{i:04d}" for i in range(1, 101, 10)
        }  # Every 10th meal
        retrieved_meals = menu.get_meals_by_ids(sample_ids)
