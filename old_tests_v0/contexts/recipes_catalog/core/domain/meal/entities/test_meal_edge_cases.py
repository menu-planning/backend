...
class TestMealEdgeCases:
    ...
    def _create_recipe_for_meal(self, meal_id: str, author_id: str, **recipe_kwargs):
        ...
        kwargs = create_recipe_kwargs(
            meal_id=meal_id, author_id=author_id, **recipe_kwargs
        )
    ...
    def test_meal_with_varying_tag_counts(self, tag_count):
        ...
        meal = create_meal(name="Tag Count Test Meal", tags=tags)
    ...
    def test_specialized_meal_edge_cases(self, meal_type_nutrition):
        ...
        tags = {
            create_meal_tag(key=k, value=v, type="meal", author_id="test_author")
        ...
        fat_grams = max(
            0.0, total_macros - protein_grams - carb_grams
        )  # Ensure non-negative
        ...
        if meal.protein_percentage is not None:
            ...
            assert (
                abs(meal.protein_percentage - expected_protein_pct) < 5.0
            )  # 5% tolerance

        # Verify tags
        for expected_key, expected_value in tag_dict.items():
            found_tag = any(
                tag.key == expected_key and tag.value == expected_value
                for tag in meal.tags
            )
    ...
    @pytest.mark.parametrize(
        "recipe_modification",
        ["add_recipe", "remove_recipe", "update_recipe", "clear_all_recipes"],
    )
    def test_meal_recipe_modification_edge_cases(self, recipe_modification):
        ...
        if recipe_modification == "add_recipe":
            ...
            initial_recipe._set_nutri_facts(
                NutriFacts(
    ...
    def test_meal_with_varying_string_lengths(self, string_length):
        ...
        meal = create_meal(
            name=long_name, description=long_description, notes=long_notes
        )
            ...
            recipe = self._create_recipe_for_meal(
                ...
                nutri_facts=NutriFacts(
                    ...
                    carbohydrate=NutriValue(
                        value=target_calories * 0.4 / 4, unit=MeasureUnit.GRAM
                    ),
                    protein=NutriValue(
                        value=target_calories * protein_ratio / 4, unit=MeasureUnit.GRAM
                    ),
                    total_fat=NutriValue(
                        value=target_calories * 0.3 / 9, unit=MeasureUnit.GRAM
                    ),
                ),
            )
            meal.update_properties(recipes=[recipe])
        ...
        for i in range(5):
            new_recipe = self._create_recipe_for_meal(
                ...
                nutri_facts=NutriFacts(
                    ...
                    carbohydrate=NutriValue(
                        value=10.0 * (i + 1), unit=MeasureUnit.GRAM
                    ),
