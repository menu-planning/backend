...
from old_tests_v0.contexts.recipes_catalog.core.adapters.client.repositories.data_factories.menu.menu_orm_factories import (
    create_menu_orm,
    create_menus_with_tags_orm,
)
from old_tests_v0.contexts.recipes_catalog.core.adapters.client.repositories.data_factories.menu.parametrized_menu_scenarios import (
...
class TestMenuRepositoryCore:
    ...
    async def test_add_and_get_menu(
        self, menu_repository: MenuRepo, test_session, test_clients
    ):
        # Given
    ...
    async def test_query_all_menus(
        self, menu_repository: MenuRepo, test_session, test_clients, menu_count: int
    ):
        # Given
        menus = [
            create_menu_orm(description=f"Query Menu {i}") for i in range(menu_count)
        ]
        for m in menus:
            test_session.add(m)
        await test_session.commit()
    ...
    async def test_soft_delete_menu(
        self, menu_repository: MenuRepo, test_session, test_clients
    ):
        # Given
    ...
    async def test_query_by_list_of_ids(
        ...
        # When
        result = await menu_repository.query(
            filters={"id": [m1.id, m2.id]}, _return_sa_instance=True
        )
...
class TestMenuRepositoryFiltering:
    ...
    async def test_filtering_scenarios(
        self,
        ...
        if "client_id" in scenario["menu_kwargs"] or "client_id" in scenario["filter"]:
            ...
            client_id = scenario["menu_kwargs"].get("client_id") or scenario[
                "filter"
            ].get("client_id")
            if client_id:
        ...
        if scenario["should_match"]:
            assert (
                menu.id in ids
            ), f"Scenario {scenario['scenario_id']} failed – expected match"
        else:
            assert (
                menu.id not in ids
            ), f"Scenario {scenario['scenario_id']} failed – expected no match"

    async def test_author_and_client_filter(
        self, menu_repository: MenuRepo, test_session, test_clients
    ):
        # Given
        ...
        # When
        result = await menu_repository.query(
            filters={"author_id": "author_target", "client_id": client_x_id},
...
class TestMenuRepositoryTagFiltering:
    ...
    async def test_tags_not_exists(
        self, menu_repository: MenuRepo, test_session, test_clients
    ):
        # Given
        exclude_tag = create_menu_tag_orm(
            key="event", value="wedding", author_id="auth1", type="menu"
        )
        ...
        # When
        result = await menu_repository.query(
            filters={"tags_not_exists": [("event", "wedding", "auth1")]},
            _return_sa_instance=True,
        )
    ...
    async def test_tag_dissociation_and_re_add(
        self, menu_repository: MenuRepo, test_session, test_clients
    ):
        # Given
        tag = create_menu_tag_orm(
            key="season", value="summer", author_id="auth1", type="menu"
        )
        ...
        initial = await menu_repository.query(
            filters={"tags": [("season", "summer", "auth1")]}, _return_sa_instance=True
        )
        ...
        after_removal = await menu_repository.query(
            filters={"tags": [("season", "summer", "auth1")]}, _return_sa_instance=True
        )
        ...
        # When – add new tag
        new_tag = create_menu_tag_orm(
            key="season", value="winter", author_id="auth1", type="menu"
        )
        still_exists.tags.append(new_tag)
        await test_session.commit()

        # Then – new tag filter should find it
        final = await menu_repository.query(
            filters={"tags": [("season", "winter", "auth1")]}, _return_sa_instance=True
        )
...
class TestMenuRepositoryErrorHandling:
    ...
    async def test_invalid_filter_parameters(self, menu_repository: MenuRepo):
        with pytest.raises(FilterValidationError):
            await menu_repository.query(filters={"unknown_field": "value"})

    async def test_constraint_violation_on_duplicate_tag(
        self, menu_repository: MenuRepo, test_session, test_clients
    ):
        # Given
        tag = create_menu_tag_orm(
            key="type", value="weekly", author_id="auth1", type="menu"
        )
        m1 = create_menu_orm(description="Menu 1", tags=[tag])
        test_session.add(m1)
        await test_session.commit()

        duplicate_tag = create_menu_tag_orm(
            key="type", value="weekly", author_id="auth1", type="menu"
        )
...
class TestMenuRepositoryPerformance:
    ...
    async def test_query_performance(
        ...
        if scenario["operation"] == "basic_query":
            ...
            filter_kwargs = (
                {"tags": [(first_tag.key, first_tag.value, first_tag.author_id)]}
                if first_tag
                else {}
            )
            await menu_repository.query(filters=filter_kwargs, _return_sa_instance=True)
        elif scenario["operation"] == "complex_query":
            await menu_repository.query(
                filters={"author_id": "author_1", "client_id": test_clients[0].id},
                _return_sa_instance=True,
            )
        else:
    ...
    async def test_bulk_insert_performance(
        ...
        start = time.perf_counter()
        menus = [
            create_menu_orm(description=f"Bulk Menu {i}") for i in range(menu_count)
        ]
        test_session.add_all(menus)
        await test_session.commit()
        duration = time.perf_counter() - start
        ...
        assert (
            duration <= max_total
        ), f"Bulk insert took {duration:.3f}s – expected <= {max_total}s"
        assert (
            per_entity_ms <= 60
        ), f"Per-entity time {per_entity_ms:.2f}ms – expected <= 60ms"
    ...
    async def test_complex_query_performance(
        ...
        start = time.perf_counter()
        await menu_repository.query(
            filters={"author_id": "author_1", "tags": [("type", "weekly", "author_1")]},
            _return_sa_instance=True,
        )
        duration = time.perf_counter() - start
