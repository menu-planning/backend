...
import pytest
from old_tests_v0.contexts.recipes_catalog.core.adapters.client.repositories.data_factories.client.client_domain_factories import (
...
from old_tests_v0.contexts.recipes_catalog.core.adapters.client.repositories.data_factories.client.parametrized_client_scenarios import (
    get_client_filter_scenarios,
    get_client_tag_filtering_scenarios,
)
from old_tests_v0.contexts.recipes_catalog.core.adapters.client.repositories.data_factories.shared_orm_factories import (
    create_client_tag_orm,
)
...
class TestClientRepositoryCore:
    ...
    async def test_add_and_get_client(
        self, client_repository: ClientRepo, test_session
    ):
        """Test adding a client and retrieving it by ID"""
        # Given: A new ORM client
        client = create_client_orm(
            name="Test Client for Add/Get", author_id="test_author"
        )
    ...
    async def test_query_all_clients(
        self, client_repository: ClientRepo, test_session, client_count: int
    ):
        ...
        assert (
            len(result) >= client_count
        )  # >= because other tests might have added clients
    ...
    async def test_update_client(self, client_repository: ClientRepo, test_session):
        ...
        client = create_client_orm(name="Original Name", notes="Original Notes")
        test_session.add(client)
        await test_session.commit()
    ...
    async def test_query_by_list_of_ids(
        self, client_repository: ClientRepo, test_session
    ):
        ...
        result = await client_repository.query(
            filters={"id": [client1.id, client2.id]}, _return_sa_instance=True
        )
    ...
    async def test_client_menus_relationship(
        self, client_repository: ClientRepo, test_session
    ):
        ...
        client = create_client(name="Client with Menus", author_id="test_author")
...
class TestClientRepositoryFiltering:
    ...
    async def test_client_filtering_scenarios(
        self, client_repository: ClientRepo, test_session, scenario: dict[str, Any]
    ):
    ...
    async def test_author_id_filter(self, client_repository: ClientRepo, test_session):
        ...
        result = await client_repository.query(
            filters={"author_id": "author_1"}, _return_sa_instance=True
        )
    ...
    async def test_created_at_gte_filter(
        self, client_repository: ClientRepo, test_session
    ):
        ...
        early_client = create_client_orm(
            name="Early Client", created_at=datetime(2024, 1, 1)
        )
        late_client = create_client_orm(
            name="Late Client", created_at=datetime(2024, 6, 1)
        )
        ...
        result = await client_repository.query(
            filters={"created_at_gte": datetime(2024, 3, 1)}, _return_sa_instance=True
        )
    ...
    async def test_created_at_lte_filter(
        self, client_repository: ClientRepo, test_session
    ):
        ...
        result = await client_repository.query(
            filters={"created_at_lte": datetime(2024, 3, 1)}, _return_sa_instance=True
        )
...
class TestClientRepositoryTagFiltering:
    ...
    async def test_tag_filtering_scenarios(
        self, client_repository: ClientRepo, test_session, scenario: dict[str, Any]
    ):
        ...
        client = create_client_orm(
            name=f"Client for {scenario['scenario_id']}", tags=tags
        )
        test_session.add(client)
        await test_session.commit()
        ...
        result = await client_repository.query(
            filters={"tags": scenario["filter_tags"]}, _return_sa_instance=True
        )
        ...
        if scenario["should_match"]:
            assert (
                client.id in result_ids
            ), f"Tag scenario '{scenario['scenario_id']}' failed: {scenario['description']}"
        else:
            assert (
                client.id not in result_ids
            ), f"Tag scenario '{scenario['scenario_id']}' failed: {scenario['description']}"

    async def test_single_client_tag_exact_match(
        self, client_repository: ClientRepo, test_session
    ):
        """Test single tag exact matching"""
        # Given: ORM client with specific tag
        tag = create_client_tag_orm(
            key="category", value="restaurant", author_id="test_author", type="client"
        )
        ...
        result = await client_repository.query(
            filters={"tags": [("category", "restaurant", "test_author")]},
            _return_sa_instance=True,
        )
    ...
    async def test_multiple_client_tags_and_logic(
        self, client_repository: ClientRepo, test_session
    ):
        """Test AND logic between different tag keys"""
        # Given: ORM client with multiple tags (different keys)
        category_tag = create_client_tag_orm(
            key="category", value="restaurant", author_id="test_author", type="client"
        )
        size_tag = create_client_tag_orm(
            key="size", value="large", author_id="test_author", type="client"
        )
        client = create_client_orm(
            name="Large Restaurant Client", tags=[category_tag, size_tag]
        )
        test_session.add(client)
        await test_session.commit()
    ...
    async def test_multiple_values_same_key_or_logic(
        self, client_repository: ClientRepo, test_session
    ):
            ...
            filters={
                "tags": [
                    ...
                    (
                        ...
                        "hotel",
                        "test_author",
                    ),  # This doesn't match, but OR logic
                ]
            },
            _return_sa_instance=True,
        )
    ...
    async def test_client_tags_not_exists_filtering(
        self, client_repository: ClientRepo, test_session
    ):
        """Test tag exclusion with tags_not_exists"""
        # Given: Two ORM clients - one with priority tag, one without
        priority_tag = create_client_tag_orm(
            key="priority", value="urgent", author_id="test_author", type="client"
        )
        ...
        result = await client_repository.query(
            filters={"tags_not_exists": [("priority", "urgent", "test_author")]},
            _return_sa_instance=True,
        )
    ...
    async def test_complex_client_tag_combination(
        self, client_repository: ClientRepo, test_session
    ):
        ...
        industry_tag = create_client_tag_orm(
        ...
        client = create_client_orm(
            name="Complex Tag Client", tags=[category_tag, industry_tag, size_tag]
        )
        test_session.add(client)
        await test_session.commit()
    ...
    async def test_client_tag_dissociation_and_removal(
        self, client_repository: ClientRepo, test_session
    ):
        ...
        industry_tag = create_client_tag_orm(
            key="industry", value="hospitality", author_id="test_author", type="client"
        )

        client_with_tags = create_client_orm(
            name="Client with Tags to Remove", tags=[category_tag, industry_tag]
        )
        test_session.add(client_with_tags)
        await test_session.commit()

        # Verify tags are initially present with ORM instances
        initial_result = await client_repository.query(
            filters={"tags": [("category", "restaurant", "test_author")]},
            _return_sa_instance=True,
        )
        ...
        after_removal_result = await client_repository.query(
            filters={"tags": [("category", "restaurant", "test_author")]},
            _return_sa_instance=True,
        )
        ...
        client_still_exists = await client_repository.get_sa_instance(
            client_with_tags.id
        )
        ...
        new_tag = create_client_tag_orm(
            key="region", value="west", author_id="test_author", type="client"
        )
        retrieved_client_again = await client_repository.get_sa_instance(
            client_with_tags.id
        )
        ...
        final_result = await client_repository.query(
            filters={"tags": [("region", "west", "test_author")]},
            _return_sa_instance=True,
        )
...
class TestClientRepositoryErrorHandling:
    ...
    async def test_invalid_filter_parameters(self, client_repository: ClientRepo):
        ...
        with pytest.raises(FilterValidationError):
            await client_repository.query(filters={"invalid_field": "some_value"})
    ...
    async def test_null_handling_in_filters(
        ...
        client_with_nulls = create_client_orm(name="Client with Nulls", notes=None)
...
class TestClientRepositoryPerformance:
    ...
    async def test_bulk_insert_performance(
        ...
        ), f"Bulk insert took {duration:.3f}s, expected <= {max_duration}s"
        assert (
            per_entity_ms <= 100
        ), f"Per-entity time {per_entity_ms:.2f}ms, expected <= 100ms"  # Relaxed for CI
    ...
    async def test_tag_filtering_performance(
        self, client_repository: ClientRepo, test_session
    ):
        ...
        result = await client_repository.query(
            filters={"tags": [("category", "restaurant", "author_1")]},
            _return_sa_instance=True,
        )
    ...
    async def test_specialized_client_factories_performance(
        self, client_repository: ClientRepo
    ):
        ...
        assert (
            duration <= 1.0
        ), f"Specialized client operations took {duration:.3f}s, expected <= 1.0s"
        ...
        assert (
            len(retrieved_restaurant.tags) >= 3
        )  # Should have category, industry, size tags
        assert retrieved_catering.profile.name == "Elite Catering Services"
        assert (
            len(retrieved_catering.tags) >= 3
        )  # Should have category, industry, size tags
