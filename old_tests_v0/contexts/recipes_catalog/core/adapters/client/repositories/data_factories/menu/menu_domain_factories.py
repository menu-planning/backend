...
from old_tests_v0.utils.counter_manager import get_next_menu_id, get_next_menu_meal_id
...
def create_menu_kwargs(**kwargs) -> dict[str, Any]:
    ...
    final_kwargs = {
        ...
        "author_id": kwargs.get(
            "author_id", f"author_{(menu_counter % 5) + 1}"
        ),  # Cycle through 5 authors
        "client_id": kwargs.get(
            "client_id", f"client_{((menu_counter - 1) % 5) + 1:03d}"
        ),  # Cycle through 5 clients (client_001 to client_005)
        "description": kwargs.get(
            "description", f"Test menu description {menu_counter}"
        ),
        ...
        "created_at": kwargs.get(
            "created_at", base_time + timedelta(hours=menu_counter)
        ),
        "updated_at": kwargs.get(
            "updated_at", base_time + timedelta(hours=menu_counter, minutes=30)
        ),
...
def create_menu_meal_kwargs(**kwargs) -> dict[str, Any]:
    ...
    final_kwargs = {
        ...
        "hour": kwargs.get(
            "hour", time(hour=8 + (meal_type_idx * 3), minute=0)
        ),  # 8am, 11am, 2pm, 5pm
    }
...
def create_weekly_menu(**kwargs) -> Menu:
    ...
    final_kwargs = {
        "description": kwargs.get(
            "description", "Complete weekly menu with breakfast, lunch, and dinner"
        ),
...
def create_special_event_menu(**kwargs) -> Menu:
    ...
    final_kwargs = {
        "description": kwargs.get(
            "description", "Elegant menu for wedding reception with gourmet options"
        ),
        "meals": kwargs.get("meals", meals),
        "tags": kwargs.get(
            "tags",
...
def create_dietary_restriction_menu(**kwargs) -> Menu:
    ...
    final_kwargs = {
        "description": kwargs.get(
            "description",
            "Carefully crafted menu accommodating various dietary restrictions",
        ),
        "meals": kwargs.get("meals", meals),
        "tags": kwargs.get(
            "tags",
...
def create_menus_with_tags(
    count: int = 3, tags_per_menu: int = 2, meals_per_menu: int = 3
) -> list[Menu]:
        ...
        for menu_idx in range(count):
            for tag_idx in range(tags_per_menu):
                ...
                if tag_key not in unique_tags:
                    tag = create_menu_tag(
                        key=key, value=value, author_id=author_id, type="menu"
                    )
...
def create_test_dataset(
    menu_count: int = 100, tags_per_menu: int = 0, meals_per_menu: int = 0
) -> dict[str, Any]:
    ...
    return {"menus": menus, "all_tags": all_tags, "all_meals": all_meals}
