...
from old_tests_v0.utils.counter_manager import get_next_client_id
...
def create_client_orm_kwargs(**kwargs) -> dict[str, Any]:
    ...
    default_contact_info = ContactInfoSaModel(
        ...
        all_phones=[f"+1555{client_counter:04d}"],
        all_emails=[f"client{client_counter}@example.com"],
    )
    ...
    final_kwargs = {
        ...
        "author_id": kwargs.get("author_id", f"author_{(client_counter % 5) + 1}"),
...
def create_clients_with_tags_orm(
    count: int = 3, tags_per_client: int = 2
) -> list[ClientSaModel]:
