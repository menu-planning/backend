...
import pytest
from old_tests_v0.contexts.products_catalog.core.adapters.repositories.product_data_factories import (
    create_ORM_product,
)
from old_tests_v0.contexts.recipes_catalog.data_factories.meal.meal_orm_factories import (
    create_meal_orm,
)
from old_tests_v0.contexts.recipes_catalog.data_factories.meal.recipe.parametrized_recipe_scenarios import (
...
class TestRecipeRepositoryCore:
    ...
    async def test_get_recipe_by_id(self, recipe_repository: RecipeRepo, test_session):
        ...
        recipe = create_recipe_orm(name="Test Recipe for Get", meal_id=meal.id)
    ...
    async def test_get_sa_instance(self, recipe_repository: RecipeRepo, test_session):
        ...
        recipe = create_recipe_orm(name="Test Recipe for SA Instance", meal_id=meal.id)
        test_session.add(recipe)
        await test_session.commit()
    ...
    async def test_persist_recipe_fails_without_add(
        self, recipe_repository: RecipeRepo
    ):
        ...
        with pytest.raises(
            AssertionError, match="Cannon persist entity which is unknown to the repo"
        ):
            await recipe_repository.persist(recipe)

    async def test_persist_all_recipes_fails_without_add(
        self, recipe_repository: RecipeRepo
    ):
    ...
    async def test_query_all_recipes(
        self, recipe_repository: RecipeRepo, recipe_count: int
    ):
    ...
    async def test_add_method_raises_not_implemented(
        self, recipe_repository: RecipeRepo
    ):
        ...
        with pytest.raises(
            NotImplementedError, match="Recipes must be added through the meal repo"
        ):
            await recipe_repository.add(recipe)

    async def test_tags_are_persisted_after_meal_creation(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        # Create tags
        tag1 = create_recipe_tag_orm(
            key="cuisine", value="italian", author_id=shared_author_id, type="recipe"
        )
        tag2 = create_recipe_tag_orm(
            key="difficulty", value="easy", author_id=shared_author_id, type="recipe"
        )
    ...
    async def test_ingredient_replacement_through_recipe_update(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        new_ingredient = create_ingredient_orm(
            name="New Single Ingredient", position=0, recipe_id=recipe.id
        )
    ...
    async def test_relationship_queries_no_duplicate_rows(
        ...
        matching_recipes = [
            r for r in all_recipes if r.name == "Recipe with Product Relationships"
        ]
        ...
        product_filtered_recipes = await recipe_repository.query(
            filters={"products": ["product_001", "product_002"]},
            _return_sa_instance=True,
        )
        matching_by_products = [
            r
            for r in product_filtered_recipes
            if r.name == "Recipe with Product Relationships"
        ]

        # Then: Should still return exactly one recipe
        assert (
            len(matching_by_products) <= 1
        )  # May be 0 if products don't exist in DB, but never > 1
...
class TestRecipeRepositoryIngredients:
    ...
    async def test_ingredient_relationship_scenarios_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        complex_recipe = create_recipe_orm(
            name="Complex Recipe", author_id=shared_author_id, meal_id=meal.id
        )
        ...
        simple_recipe = create_recipe_orm(
            name="Simple Recipe", author_id=shared_author_id, meal_id=meal.id
        )
        ...
        all_results = await recipe_repository.query(
            filters={}, _return_sa_instance=True
        )
        ...
        complex_recipe_result = await recipe_repository.get_sa_instance(
            complex_recipe.id
        )
        ...
        flour_ingredient = next(
            ing for ing in complex_recipe_result.ingredients if ing.name == "Flour"
        )
        ...
        no_ingredients_result = await recipe_repository.get_sa_instance(
            no_ingredients_recipe.id
        )

        # Then: Should have empty ingredients list
        assert len(no_ingredients_result.ingredients) == 0

    async def test_product_filtering_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        ingredients_with_products = [
            create_ingredient_orm(
                ...
                product_id="product_001",
                position=0,
                recipe_id="recipe_product_filter",
            ),
            create_ingredient_orm(
                name="Sugar",
                product_id="product_002",
        ...
        result_single = await recipe_repository.query(
            filters={"products": ["product_001"]}, _return_sa_instance=True
        )
        ...
        result_multiple = await recipe_repository.query(
            filters={"products": ["product_001", "product_002"]},
            _return_sa_instance=True,
        )

        # Then: Should find the recipe containing either product
        matching_recipes_multi = [
            r for r in result_multiple if r.id == "recipe_product_filter"
        ]
        assert len(matching_recipes_multi) == 1

        # When: Filtering by non-existent product
        result_none = await recipe_repository.query(
            filters={"products": ["nonexistent_product"]}, _return_sa_instance=True
        )

        # Then: Should return empty list or not include our test recipe
        matching_nonexistent = [
            r for r in result_none if r.id == "recipe_product_filter"
        ]
        assert len(matching_nonexistent) == 0

    async def test_product_name_filtering_with_real_products(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        from old_tests_v0.contexts.products_catalog.core.adapters.repositories.product_data_factories import (
            get_or_create_ORM_source,
        )

        flour_source = await get_or_create_ORM_source(
            test_session,
            id="flour_source",
            name="Flour Source",
            author_id=shared_author_id,
        )
        sugar_source = await get_or_create_ORM_source(
            test_session,
            id="sugar_source",
            name="Sugar Source",
            author_id=shared_author_id,
        ...
        flour_product = create_ORM_product(
            id="flour_product_001", name="All-Purpose Flour", source_id="flour_source"
        )
        sugar_product = create_ORM_product(
            id="sugar_product_001", name="White Sugar", source_id="sugar_source"
        )
        ...
        generic_recipe = create_recipe_orm(
            ...
            ingredients=[
                create_ingredient_orm(
                    name="Salt", product_id=None, position=0, recipe_id="generic_recipe"
                )
            ],
        )
        ...
        flour_results = await recipe_repository.query(
            filters={"product_name": "flour"}, _return_sa_instance=True
        )
        ...
        sugar_results = await recipe_repository.query(
            filters={"product_name": "sugar"}, _return_sa_instance=True
        )
        ...
        nonexistent_results = await recipe_repository.query(
            filters={"product_name": "nonexistent_product"}, _return_sa_instance=True
        )
    ...
    async def test_ingredient_validation_with_orm_models(
        self, recipe_repository: RecipeRepo, test_session
    ):
    ...
    async def test_ingredient_product_relationship_with_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        # When: Retrieving the recipe through repository
        retrieved_recipe = await recipe_repository.get_sa_instance(
            "recipe_product_relations"
        )
        ...
        sorted_ingredients = sorted(
            retrieved_recipe.ingredients, key=lambda x: x.position
        )
    ...
    async def test_ingredient_without_product_relationship(
        self, recipe_repository: RecipeRepo, test_session
    ):
    ...
    async def test_ingredient_filtering_by_name(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        flour_recipe = create_recipe_orm(
            ...
            ingredients=[
                create_ingredient_orm(
                    name="Flour", position=0, recipe_id="flour_recipe"
                )
            ],
        )
        ...
        sugar_recipe = create_recipe_orm(
            ...
            ingredients=[
                create_ingredient_orm(
                    name="Sugar", position=0, recipe_id="sugar_recipe"
                )
            ],
        )
...
class TestRecipeRepositoryRatings:
    ...
    async def test_rating_aggregation_scenarios_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        high_recipe_result = await recipe_repository.get_sa_instance(
            "high_rated_recipe"
        )
        medium_recipe_result = await recipe_repository.get_sa_instance(
            "medium_rated_recipe"
        )
        ...
        assert set(high_taste_scores) == {4, 5}
        assert set(high_convenience_scores) == {4, 5}

        # Verify medium rated recipe ratings
        medium_taste_scores = [r.taste for r in medium_recipe_result.ratings]
        medium_convenience_scores = [
            r.convenience for r in medium_recipe_result.ratings
        ]
        assert set(medium_taste_scores) == {2, 3}
        assert set(medium_convenience_scores) == {3, 4}
        ...
        assert set(low_taste_scores) == {1, 2}
        assert set(low_convenience_scores) == {1, 2}

    async def test_average_taste_rating_filter_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        high_taste_results = await recipe_repository.query(
            filters={"average_taste_rating_gte": 4.0}, _return_sa_instance=True
        )
        ...
        medium_taste_results = await recipe_repository.query(
            filters={"average_taste_rating_gte": 3.5}, _return_sa_instance=True
        )
        ...
        all_taste_results = await recipe_repository.query(
            filters={"average_taste_rating_gte": 0.0}, _return_sa_instance=True
        )

        # Then: Should find our test recipes
        all_result_names = {r.name for r in all_taste_results}
        assert (
            "High Taste Recipe" in all_result_names
            or "Medium Taste Recipe" in all_result_names
            or "Low Taste Recipe" in all_result_names
        )

    async def test_average_convenience_rating_filter_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        high_convenience_recipe = create_recipe_orm(
            ...
            ratings=[
                create_rating_orm(
                    user_id="user_1",
                    recipe_id="high_convenience_recipe",
                    taste=3,
                    convenience=5,
                ),
                create_rating_orm(
        ...
        low_convenience_recipe = create_recipe_orm(
            ...
            ratings=[
                create_rating_orm(
                    user_id="user_1",
                    recipe_id="low_convenience_recipe",
                    taste=5,
                    convenience=2,
                ),
                create_rating_orm(
        ...
        high_convenience_results = await recipe_repository.query(
            filters={"average_convenience_rating_gte": 4.5}, _return_sa_instance=True
        )
        ...
        all_convenience_results = await recipe_repository.query(
            filters={"average_convenience_rating_gte": 1.0}, _return_sa_instance=True
        )

        # Then: Should find our test recipes
        all_result_names = {r.name for r in all_convenience_results}
        assert (
            "High Convenience Recipe" in all_result_names
            or "Low Convenience Recipe" in all_result_names
        )

    async def test_rating_validation_with_orm_models(
        self, recipe_repository: RecipeRepo, test_session
    ):
    ...
    async def test_rating_range_validation_with_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
    ...
    async def test_recipe_with_multiple_ratings_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        multiple_ratings = [
            create_rating_orm(
                recipe_id="recipe_multiple_ratings",
                user_id="user_1",
                taste=5,
                convenience=4,
            ),
    ...
    async def test_recipe_without_ratings_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
...
class TestRecipeRepositoryTagFiltering:
    ...
    async def test_tag_filtering_scenarios_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        cuisine_italian = create_recipe_tag_orm(
            key="cuisine", value="italian", type="recipe", author_id=shared_author_id
        )
        cuisine_mexican = create_recipe_tag_orm(
            key="cuisine", value="mexican", type="recipe", author_id=shared_author_id
        )
        difficulty_easy = create_recipe_tag_orm(
            key="difficulty", value="easy", type="recipe", author_id=shared_author_id
        )
        difficulty_hard = create_recipe_tag_orm(
            key="difficulty", value="hard", type="recipe", author_id=shared_author_id
        )
        diet_vegan = create_recipe_tag_orm(
            key="diet", value="vegan", type="recipe", author_id=shared_author_id
        )
        diet_vegetarian = create_recipe_tag_orm(
            key="diet", value="vegetarian", type="recipe", author_id=shared_author_id
        )
        ...
        no_tags_recipe = create_recipe_orm(
            name="Recipe with No Tags", author_id=shared_author_id, meal_id=meal.id
        )
        ...
        italian_results = await recipe_repository.query(
            filters={"tags": [("cuisine", "italian", shared_author_id)]},
            _return_sa_instance=True,
        )
    ...
    async def test_single_tag_filtering_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        cuisine_tag = create_recipe_tag_orm(
            key="cuisine", value="italian", type="recipe", author_id=shared_author_id
        )
        ...
        untagged_recipe = create_recipe_orm(
            name="Generic Recipe", author_id=shared_author_id, meal_id=meal.id
        )
        ...
        tag_filter = {
            "tags": [("cuisine", "italian", shared_author_id)]
        }  # Use generated author_id
        results = await recipe_repository.query(
            filters=tag_filter, _return_sa_instance=True
        )
    ...
    async def test_multiple_tags_and_logic_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        difficulty_tag = create_recipe_tag_orm(
            key="difficulty", value="easy", type="recipe", author_id=shared_author_id
        )
        other_tag = create_recipe_tag_orm(
            key="diet", value="vegan", type="recipe", author_id=shared_author_id
        )
        ...
        results = await recipe_repository.query(
            filters=and_filter, _return_sa_instance=True
        )
    ...
    async def test_tags_not_exists_filtering_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        # Create tags - let factory generate unique author_ids
        diet_tag = create_recipe_tag_orm(
            key="diet", value="vegan", type="recipe", author_id=shared_author_id
        )
        cuisine_tag = create_recipe_tag_orm(
            key="cuisine", value="italian", type="recipe", author_id=shared_author_id
        ...
        results = await recipe_repository.query(
            filters=exclusion_filter, _return_sa_instance=True
        )
    ...
    async def test_complex_tag_combination_with_real_data(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        diet_vegan_tag = create_recipe_tag_orm(
            key="diet", value="vegan", type="recipe", author_id=shared_author_id
        )
        diet_vegetarian_tag = create_recipe_tag_orm(
            key="diet", value="vegetarian", type="recipe", author_id=shared_author_id
        )
        ...
        complex_filter = {
            "tags": [
                ("cuisine", "italian", shared_author_id),
                ("diet", "vegan", shared_author_id),
            ],
            "tags_not_exists": [("diet", "vegetarian", shared_author_id)],
        }
        results = await recipe_repository.query(
            filters=complex_filter, _return_sa_instance=True
        )
    ...
    async def test_recipe_with_multiple_tags_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
        """Test recipe with multiple tags using real database persistence"""
        # Create meal first
        meal = create_meal_orm(
            name="Meal for Multiple Tags"
        )  # Remove hardcoded author_id and id
        test_session.add(meal)
        await test_session.flush()
        ...
        cuisine_tag = create_recipe_tag_orm(
            key="cuisine", value="italian", type="recipe"
        )  # Remove hardcoded author_id
        difficulty_tag = create_recipe_tag_orm(
            key="difficulty", value="easy", type="recipe"
        )  # Remove hardcoded author_id
        diet_tag = create_recipe_tag_orm(
            key="diet", value="vegetarian", type="recipe"
        )  # Remove hardcoded author_id
...
class TestRecipeRepositoryErrorHandling:
    ...
    async def test_invalid_filter_parameters(self, recipe_repository: RecipeRepo):
        ...
        with pytest.raises(FilterValidationError, match="Invalid filter keys"):
            await recipe_repository.query(
                filters={"invalid_field": "value"}, _return_sa_instance=True
            )

        # When/Then: Should raise appropriate errors for invalid values
        with pytest.raises((ValueError, TypeError, FilterValidationError)):
            await recipe_repository.query(
    ...
    async def test_null_handling_in_filters(self, recipe_repository: RecipeRepo):
        ...
        result = await recipe_repository.query(
            filters={"name": None}, _return_sa_instance=True
        )
        assert isinstance(result, list)

        # But operator filters should reject None values
        with pytest.raises(RepositoryQueryError, match="Query building failed"):
            await recipe_repository.query(
                filters={"total_time_gte": None}, _return_sa_instance=True
            )
    ...
    async def test_empty_filter_dict(self, recipe_repository: RecipeRepo):
        ...
        result = await recipe_repository.query(
            filters=empty_filter, _return_sa_instance=True
        )
    ...
    async def test_none_filter(self, recipe_repository: RecipeRepo):
        ...
        result = await recipe_repository.query(
            filters=none_filter, _return_sa_instance=True
        )

        # Then: Should return all recipes (may be empty list)
        assert isinstance(result, list)

    async def test_rating_validation_errors_with_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
    ...
    async def test_ingredient_validation_errors_with_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
    ...
    async def test_database_constraint_violations_with_recipes(
        self, recipe_repository: RecipeRepo, test_session
    ):
...
class TestRecipeRepositoryPerformance:
    ...
    async def test_query_performance_scenarios(
        self, recipe_repository: RecipeRepo, scenario: dict[str, Any]
    ):
        ...
        if "expected_query_time" in scenario:
            ...
            print(
                f"Query duration: {duration:.3f}s (expected: ≤{scenario['expected_query_time']}s)"
            )
    ...
    async def test_bulk_persist_performance(
        ...
        for i in range(recipe_count):
            # Create meal first
            meal = create_meal_orm(
                name=f"Performance Test Meal {i}", author_id="perf_test_author"
            )
    ...
    async def test_memory_usage_bulk_operations(self, recipe_repository: RecipeRepo):
        ...
        print(
            f"Successfully executed {query_count * len(filters)} queries with various filters"
        )

    async def test_bulk_recipe_query_performance(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        # Query with filters
        public_results = await recipe_repository.query(
            filters={"privacy": "public"}, _return_sa_instance=True
        )
        time_filtered_results = await recipe_repository.query(
            filters={"total_time_gte": 60}, _return_sa_instance=True
        )
...
class TestSpecializedRecipeFactories:
    ...
    async def test_quick_recipe_creation_with_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        quick_recipe = create_quick_recipe_orm(
            name="Super Quick Meal", meal_id=meal.id, author_id=shared_author_id
        )
    ...
    async def test_high_protein_recipe_creation_with_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        protein_recipe = create_high_protein_recipe_orm(
            name="Protein Power", meal_id=meal.id, author_id=shared_author_id
        )
        ...
        assert (
            retrieved_recipe.protein_percentage >= 25.0
        )  # High protein characteristic
    ...
    async def test_vegetarian_recipe_creation_with_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        veg_recipe = create_vegetarian_recipe_orm(
            name="Veggie Delight", meal_id=meal.id, author_id=shared_author_id
        )
    ...
    async def test_public_private_recipe_creation_with_database(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        public_recipe = create_public_recipe_orm(
            name="Public Recipe", meal_id=meal.id, author_id=shared_author_id
        )
        private_recipe = create_private_recipe_orm(
            name="Private Recipe", meal_id=meal.id, author_id=shared_author_id
        )
        ...
        assert (
            "private recipe only visible to the author" in retrieved_private.description
        )

    async def test_recipe_orm_factory_persistence_patterns(
        self, recipe_repository: RecipeRepo, test_session
    ):
        ...
        all_factory_recipes = await recipe_repository.query(
            filters={"meal_id": meal.id}, _return_sa_instance=True
        )
