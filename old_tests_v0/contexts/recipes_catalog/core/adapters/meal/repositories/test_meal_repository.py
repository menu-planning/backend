...
import pytest
from old_tests_v0.contexts.recipes_catalog.data_factories.meal.meal_domain_factories import (
    create_meal,
)
from old_tests_v0.contexts.recipes_catalog.data_factories.meal.meal_orm_factories import (
    create_meal_orm,
    create_meals_with_tags_orm,
    create_test_meal_dataset_orm,
)
from old_tests_v0.contexts.recipes_catalog.data_factories.meal.parametrized_meal_scenarios import (
...
class TestMealRepositoryCore:
    ...
    async def test_query_all_meals(
        self, meal_repository: MealRepo, test_session, meal_count: int
    ):
        ...
        assert (
            len(result) >= meal_count
        )  # >= because other tests might have added meals
    ...
    async def test_persist_all_with_discarded_meal(
        self, meal_repository: MealRepo, test_session
    ):
    ...
    async def test_add_meal_with_reused_recipe_tags(
        self, meal_repository: MealRepo, test_session
    ):
        ...
        meal_tag = create_meal_tag_orm(
            key="meal_type", value="dinner", author_id="test_author", type="meal"
        )

        meal1 = create_meal_orm(
            name="First Meal", author_id="test_author", tags=[meal_tag]
        )
        ...
        meal2 = create_meal_orm(
            name="Second Meal", author_id="test_author", tags=[duplicate_tag]
        )
    ...
    async def test_query_by_list_of_ids(self, meal_repository: MealRepo, test_session):
        ...
        result = await meal_repository.query(
            filters={"id": [meal1.id, meal2.id]}, _return_sa_instance=True
        )
    ...
    async def test_meal_recipe_copying_persistence(self, meal_repository: MealRepo):
        ...
        meal = create_meal(name="Meal with Recipes", author_id="test_author")
        ...
        if hasattr(retrieved_meal, "recipes") and retrieved_meal.recipes:
            ...
            for recipe in retrieved_meal.recipes:
                ...
                assert (
                    recipe.author_id == "test_author"
                )  # Should inherit author_id when copied
    ...
    async def test_meal_copying_author_id_inheritance(self, meal_repository: MealRepo):
        ...
        original_meal = create_meal(name="Original Meal", author_id="original_author")
        ...
        copied_meal = create_meal(
            name="Original Meal", author_id="new_author"  # Same name, different author
        )
...
class TestMealRepositoryFiltering:
    ...
    async def test_meal_filtering_scenarios(
        self, meal_repository: MealRepo, test_session, scenario: dict[str, Any]
    ):
    ...
    async def test_total_time_gte_filter(self, meal_repository: MealRepo, test_session):
        ...
        result = await meal_repository.query(
            filters={"total_time_gte": 30}, _return_sa_instance=True
        )
        ...
        assert (
            len(result) == 2
        ), f"Expected 2 meals to match total_time_gte filter, got: {meal_names}"
    ...
    async def test_total_time_lte_filter(self, meal_repository: MealRepo, test_session):
        ...
        result = await meal_repository.query(
            filters={"total_time_lte": 60}, _return_sa_instance=True
        )
        ...
        assert (
            len(result) == 2
        ), f"Expected 2 meals to match total_time_lte filter, got: {meal_names}"
        result_names = {meal.name for meal in result}
    ...
    async def test_like_filter(self, meal_repository: MealRepo, test_session):
        ...
        result = await meal_repository.query(
            filters={"like": True}, _return_sa_instance=True
        )
    ...
    async def test_combined_filters(self, meal_repository: MealRepo, test_session):
        ...
        target_meal = create_meal_orm(
            name="Target Meal", author_id="target_author", like=True, total_time=45
        )
        wrong_author = create_meal_orm(
            name="Wrong Author", author_id="other_author", like=True, total_time=45
        )
        ...
        result = await meal_repository.query(
            filters={"author_id": "target_author", "total_time_lte": 60, "like": True},
...
class TestMealRepositoryTagFiltering:
    ...
    async def test_single_tag_exact_match(
        ...
        result = await meal_repository.query(
            filters={"tags": [("diet", "vegetarian", "test_author")]},
            _return_sa_instance=True,
        )
    ...
    async def test_multiple_tags_and_logic(
        ...
        cuisine_tag = await get_or_create_meal_tag_orm(
            test_session,
            key="cuisine",
            value="italian",
            author_id="test_author",
        ...
        meal = create_meal_orm(
            name="Italian Vegetarian Meal", tags=[diet_tag, cuisine_tag]
        )
        test_session.add(meal)
        await test_session.commit()
    ...
    async def test_tags_not_exists_filtering(
        self, meal_repository: MealRepo, test_session
    ):
        """Test tag exclusion with tags_not_exists"""
        # Given: Two ORM meals - one with spicy tag, one without
        spicy_tag = await get_or_create_meal_tag_orm(
            test_session, key="spice", value="hot", author_id="test_author", type="meal"
        )
        ...
        result = await meal_repository.query(
            filters={"tags_not_exists": [("spice", "hot", "test_author")]},
            _return_sa_instance=True,
        )
    ...
    async def test_complex_tag_combination(
        self, meal_repository: MealRepo, test_session
    ):
        ...
        difficulty_tag = await get_or_create_meal_tag_orm(
            test_session,
            key="difficulty",
            value="easy",
            author_id="author_1",
        ...
        meal = create_meal_orm(
            name="Complex Tag Meal", tags=[diet_tag, cuisine_tag, difficulty_tag]
        )
        test_session.add(meal)
        await test_session.commit()
    ...
    async def test_tag_dissociation_and_removal(
        self, meal_repository: MealRepo, test_session
    ):
        """Test removing tags from meals and verifying persistence"""
        # Given: ORM meal with multiple tags
        category_tag = await get_or_create_meal_tag_orm(
            test_session,
            key="category",
            value="breakfast",
            author_id="test_author",
        ...
        meal_with_tags = create_meal_orm(
            name="Meal with Tags to Remove", tags=[category_tag, diet_tag]
        )
        test_session.add(meal_with_tags)
        await test_session.commit()

        # Verify tags are initially present
        initial_result = await meal_repository.query(
            filters={"tags": [("category", "breakfast", "test_author")]},
            _return_sa_instance=True,
        )
        ...
        after_removal_result = await meal_repository.query(
            filters={"tags": [("category", "breakfast", "test_author")]},
            _return_sa_instance=True,
        )
        ...
        new_tag = await get_or_create_meal_tag_orm(
            test_session,
            key="season",
            value="winter",
            author_id="test_author",
        ...
        final_result = await meal_repository.query(
            filters={"tags": [("season", "winter", "test_author")]},
            _return_sa_instance=True,
        )
...
class TestMealRepositoryErrorHandling:
    ...
    async def test_invalid_filter_parameters(self, meal_repository: MealRepo):
        ...
        with pytest.raises(FilterValidationError):
            await meal_repository.query(
                filters={"invalid_field": "some_value"}, _return_sa_instance=True
            )
    ...
    async def test_null_handling_in_filters(
        ...
        meal_with_nulls = create_meal_orm(
            name="Meal with Nulls", description=None, notes=None, like=None
        )
...
class TestMealRepositoryPerformance:
    ...
    async def test_query_performance_scenarios(
        ...
        if scenario["operation"] not in [
            "basic_query",
            "tag_filtering",
            "complex_query",
        ]:
            pytest.skip(
                f"Operation {scenario['operation']} not implemented in this test"
            )
        ...
        if scenario["operation"] == "basic_query":
            ...
            if dataset["all_tags"]:
                ...
                result = await meal_repository.query(
                    filters={"tags": tag_filter}, _return_sa_instance=True
                )
            else:
                result = await meal_repository.query(_return_sa_instance=True)
        elif scenario["operation"] == "complex_query":
            result = await meal_repository.query(
                filters={"total_time_gte": 30, "total_time_lte": 90, "like": True},
                _return_sa_instance=True,
            )
    ...
    async def test_bulk_insert_performance(
        ...
        assert (
            per_entity_ms <= 50
        ), f"Per-entity time {per_entity_ms:.2f}ms, expected <= 50ms"  # Relaxed for CI
    ...
    async def test_complex_query_performance(
        ...
        result = await meal_repository.query(
            filters={"total_time_gte": 30, "like": True, "author_id": "author_1"},
            _return_sa_instance=True,
        )
