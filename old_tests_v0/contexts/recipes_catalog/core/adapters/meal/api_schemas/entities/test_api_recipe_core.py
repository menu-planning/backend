...
class TestApiRecipeBasics:
    ...
    def test_from_domain_computed_properties(self, domain_recipe):
        ...
        assert (
            api_recipe.average_convenience_rating
            == domain_recipe.average_convenience_rating
        )
    ...
    def test_from_domain_with_nutrition_facts(self, domain_recipe_with_nutri_facts):
        ...
        for name in api_recipe.nutri_facts.__class__.model_fields:
            value = getattr(api_recipe.nutri_facts, name)
            assert (
                value.value
                == getattr(domain_recipe_with_nutri_facts.nutri_facts, name).value
            )
            assert (
                value.unit
                == getattr(domain_recipe_with_nutri_facts.nutri_facts, name).unit.value
            )
...
class TestApiRecipeRoundTrip:
    ...
    def test_api_to_domain_to_api_round_trip(self, complex_recipe):
        ...
        assert (
            len(recovered_api.tags) == len(complex_recipe.tags)
            or len(recovered_api.tags) == 0
        )
    ...
    def test_round_trip_with_edge_cases(self, edge_case_recipes, case_name):
        ...
        assert (
            len(recovered_api.ratings) == len(edge_case_recipe.ratings)
            or len(recovered_api.ratings) == 0
        )
        assert (
            len(recovered_api.tags) == len(edge_case_recipe.tags)
            or len(recovered_api.tags) == 0
        )
...
class TestApiRecipeComputedProperties:
    ...
    def test_average_rating_correction_round_trip(self):
        ...
        success, details = validate_average_rating_correction_roundtrip(
            incorrect_recipe
        )
    ...
    def test_computed_properties_with_multiple_ratings(self, complex_recipe):
        ...
        if len(complex_recipe.ratings) > 1:
            ...
            total_convenience = sum(
                rating.convenience for rating in complex_recipe.ratings
            )
            ...
            assert (
                abs(
                    complex_recipe.average_convenience_rating - expected_convenience_avg
                )
                < 0.01
            )
    ...
    def test_json_with_incorrect_averages_correction(self):
        ...
        assert (
            restored_recipe.average_taste_rating
            == corrected_recipe.average_taste_rating
        )
        assert (
            restored_recipe.average_convenience_rating
            == corrected_recipe.average_convenience_rating
        )
