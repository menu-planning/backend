...
class TestApiMealFieldValidation:
    ...
    def test_required_field_validation(self):
        ...
        error_fields = {
            str(error["loc"][0]) if error["loc"] else "" for error in errors
        }

        # Should have errors for all required fields
        assert required_fields.issubset(
            error_fields
        ), f"Missing required field validation for: {required_fields - error_fields}"
    ...
    def test_uuid_field_validation(self, invalid_uuid):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ApiMeal.model_validate_json(
                json.dumps({**base_data, "id": invalid_uuid, "author_id": str(uuid4())})
            )
        ...
        with pytest.raises(ValidationError) as exc_info:
            ApiMeal.model_validate_json(
                json.dumps({**base_data, "id": str(uuid4()), "author_id": invalid_uuid})
            )
        assert "Invalid UUID4 format" in str(exc_info.value)
    ...
    def test_name_field_validation(self, invalid_name):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ApiMeal.model_validate_json(json.dumps({**base_data, "name": invalid_name}))
        errors = exc_info.value.errors()
    ...
    def test_url_field_validation(self, invalid_url):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ApiMeal.model_validate_json(
                json.dumps({**base_data, "image_url": invalid_url})
            )
        errors = exc_info.value.errors()
    ...
    def test_percentage_field_validation(self, field, invalid_percentage):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ApiMeal.model_validate_json(
                json.dumps({**base_data, field: invalid_percentage})
            )
    ...
    def test_non_negative_field_validation(self, field, invalid_value):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ApiMeal.model_validate_json(json.dumps({**base_data, field: invalid_value}))
    ...
    def test_version_field_validation(self, invalid_version):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ApiMeal.model_validate_json(
                json.dumps({**base_data, "version": invalid_version})
            )
        errors = exc_info.value.errors()
...
class TestApiMealSerialization:
    ...
    def test_invalid_json_deserialization(self, case_data):
        ...
        assert error_fields.intersection(
    ...
    def test_json_serialization_completeness(self, required_field):
        ...
        ), f"Field '{required_field}' missing from JSON serialization"
...
class TestApiMealValidationBehavior:
    ...
    def test_validation_error_quality(self):
        ...
        assert any(
            "ensure this value is greater than or equal to 1" in msg
            or "greater than or equal to 1" in msg
