...
class TestApiMealValidationEdgeCases:
    ...
    def test_name_field_validation_edge_cases(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            base_kwargs["name"] = ""
            ApiMeal(**base_kwargs)
        assert "string_type" in str(exc_info.value) or "min_length" in str(
            exc_info.value
        )
    ...
    def test_uuid_field_validation_edge_cases(self):
        ...
        assert (
            "uuid" in str(exc_info.value).lower()
        ...
        assert (
            "uuid" in str(exc_info.value).lower()
            or "value_error" in str(exc_info.value).lower()
        )
    ...
    def test_percentage_field_validation_edge_cases(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ...
            ApiMeal(**base_kwargs)
        assert "must be between 0 and 100" in str(
            exc_info.value
        ) or "non-negative" in str(exc_info.value)
        ...
        assert "must be between 0 and 100" in str(
            exc_info.value
        ) or "percentage" in str(exc_info.value)
    ...
    def test_non_negative_field_validation_edge_cases(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ...
            ApiMeal(**base_kwargs)
        assert "must be non-negative" in str(exc_info.value) or "negative" in str(
            exc_info.value
        )
    ...
    def test_very_large_collections_edge_cases(self):
        ...
        large_tags = frozenset(
            create_api_tag(
                key=f"tag_{i}", value=f"value_{i}", author_id=base_kwargs["author_id"]
            )
...
class TestApiMealConversionEdgeCases:
    ...
    def test_from_domain_with_large_collections(self, domain_meal):
        ...
        # Create large recipe collection
        large_recipes = [
            create_recipe(author_id=domain_meal.author_id, meal_id=domain_meal.id)
            for _ in range(50)
        ]
        ...
        counter = 0
        while (
            len(large_tags) < 25 and counter < 100
        ...
        assert len(api_meal.tags) == len(
            large_tags
        )  # Use actual count rather than fixed 25
...
class TestApiMealNestedObjectEdgeCases:
    ...
    def test_tags_with_edge_case_values(self):
        ...
        edge_case_tags = frozenset(
            [
                create_api_tag(
                    key="single_char", value="a", author_id=base_kwargs["author_id"]
                ),
                create_api_tag(
                    key="long_key_" + "x" * 80,
                    value="long_key",
                    author_id=base_kwargs["author_id"],
                ),  # 89 chars total, within 100 limit
                create_api_tag(
                    key="special_chars",
                    value="!@#$%^&*()",
                    author_id=base_kwargs["author_id"],
                ...
                create_api_tag(
                    key="unicode", value="中文测试", author_id=base_kwargs["author_id"]
                ),
                create_api_tag(
                    key="numbers", value="123456", author_id=base_kwargs["author_id"]
                ),
            ]
        )
