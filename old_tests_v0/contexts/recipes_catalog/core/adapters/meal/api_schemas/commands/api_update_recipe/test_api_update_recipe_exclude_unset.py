...
class TestApiAttributesToUpdateOnRecipeExcludeUnset:
        ...
        for i, scenario_kwargs in enumerate(test_scenarios):
            ...
            for field_name, field_value in scenario_kwargs.items():
                if field_name == "nutri_facts" and field_value is None:
                    assert (
                        field_name not in domain_updates
                    ), f"Field {field_name} should be excluded when None in scenario {i}"
                elif field_name == "privacy" and field_value is None:
                    assert (
                        field_name in domain_updates
                    ), f"Field {field_name} missing in scenario {i}"
                    assert (
                        domain_updates[field_name] == Privacy.PRIVATE
                    ), f"Privacy should be converted to PRIVATE in scenario {i}"
                else:
                    assert (
                        field_name in domain_updates
                    ), f"Field {field_name} missing in scenario {i}"
                    if field_name in ["ingredients", "tags"]:
                        ...
                        assert (
                            domain_updates[field_name] == field_value
                        ), f"Field {field_name} value mismatch in scenario {i}"

            # Check final count matches expectation
            assert (
                len(domain_updates) == expected_field_count
            ), f"Field count mismatch in scenario {i}: expected {expected_field_count}, got {len(domain_updates)}"
    ...
    def test_exclude_unset_consistency_across_instances(self):
        ...
        for i, instance in enumerate(instances):
            ...
            assert (
                "instructions" in domain_updates
            ), f"Instructions missing in instance {i}"
            ...
            ), f"Field set mismatch in instance {i}: expected {expected_fields}, got {actual_fields}"
    ...
    def test_partial_update_name_only(self):
        ...
        for field in excluded_fields:
            assert (
                field not in domain_updates
            ), f"Field {field} should be excluded but was found"
    ...
    def test_partial_update_with_privacy_field(self):
        ...
        update_attributes = ApiAttributesToUpdateOnRecipe(
            name="Privacy Recipe", instructions="Instructions", privacy=Privacy.PUBLIC
        )  # type: ignore
    ...
    def test_partial_update_edge_cases(self):
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            assert (
                len(domain_updates) == scenario["expected_count"]
            ), f"Scenario {i}: Expected {scenario['expected_count']} fields, got {len(domain_updates)}"

            actual_fields = set(domain_updates.keys())
            assert (
                actual_fields == scenario["expected_fields"]
            ), f"Scenario {i}: Expected fields {scenario['expected_fields']}, got {actual_fields}"
            ...
            for field_name, expected_value in scenario["input"].items():
                if field_name == "image_url":
                    ...
                    assert str(domain_updates[field_name]) == str(
                        HttpUrl(expected_value)
                    ), f"Scenario {i}: Field {field_name} URL mismatch"
                else:
                    assert (
                        domain_updates[field_name] == expected_value
                    ), f"Scenario {i}: Field {field_name} value mismatch"
    ...
    def test_partial_update_pydantic_fields_set_tracking(self):
        ...
        for i, test_case in enumerate(test_cases):
            ...
            assert (
                fields_set == expected_fields
            ), f"Test case {i}: fields_set {fields_set} != expected {expected_fields}"
            ...
            assert (
                domain_fields == expected_fields
            ), f"Test case {i}: domain fields {domain_fields} != expected {expected_fields}"
            ...
            for field_name, expected_value in test_case.items():
                if field_name == "image_url":
                    ...
                    assert str(domain_updates[field_name]) == str(
                        HttpUrl(expected_value)
                    ), f"Test case {i}: {field_name} URL mismatch"
                else:
                    assert (
                        domain_updates[field_name] == expected_value
                    ), f"Test case {i}: {field_name} value mismatch"
    ...
    def test_pydantic_fields_set_tracking_none_vs_unset(self):
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            fields_set = update_attributes.__pydantic_fields_set__
            assert (
                fields_set == scenario["expected_fields_set"]
            ), f"Scenario {i}: fields_set mismatch"

            # Check domain output
            domain_updates = update_attributes.to_domain()
            domain_fields = set(domain_updates.keys())
            assert (
                domain_fields == scenario["expected_domain_fields"]
            ), f"Scenario {i}: domain fields mismatch"
            ...
            for field_name in scenario["expected_fields_set"]:
                ...
                if field_name == "privacy" and expected_value is None:
                    assert (
                        domain_updates[field_name] == Privacy.PRIVATE
                    ), f"Scenario {i}: {field_name} should be converted to Privacy.PRIVATE"
                else:
                    assert (
                        domain_updates[field_name] == expected_value
                    ), f"Scenario {i}: {field_name} value mismatch"
    ...
    def test_consistency_between_creation_methods(self):
        ...
        method3 = ApiAttributesToUpdateOnRecipe(
            name="Consistency Test", instructions="Instructions"
        )  # type: ignore
