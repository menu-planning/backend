...
from old_tests_v0.utils.counter_manager import get_next_api_ingredient_id
...
def create_api_ingredient_kwargs(**kwargs) -> dict[str, Any]:
    ...
    ingredient_data = REALISTIC_INGREDIENTS[
        (ingredient_counter - 1) % len(REALISTIC_INGREDIENTS)
    ]
    ...
    final_kwargs = {
        ...
        "product_id": kwargs.get(
            "product_id", str(uuid4()) if ingredient_counter % 3 == 0 else None
        ),
    }
...
def create_api_ingredient_from_json(
    json_data: str | None = None, **kwargs
) -> ApiIngredient:
    """
...
def create_spice_ingredient(**kwargs) -> ApiIngredient:
    ...
    final_kwargs = {
        ...
        **{
            ...
            if k not in ["name", "quantity", "unit", "full_text", "product_id"]
        },
    }
...
def create_ingredient_with_max_full_text(**kwargs) -> ApiIngredient:
    ...
    """
    max_full_text = (
        "This is an extremely detailed ingredient description that includes preparation instructions, quality specifications, sourcing information, and cooking notes. "
        * 8
    )  # Creates ~992 chars
...
def create_ingredient_with_max_position(**kwargs) -> ApiIngredient:
    ...
    final_kwargs = {
        ...
        **{
            ...
            for k, v in kwargs.items()
            if k
            not in ["name", "quantity", "unit", "position", "full_text", "product_id"]
        },
    }
    return create_api_ingredient(**final_kwargs)
...
def create_ingredient_orm_kwargs_from_api(
