...
from old_tests_v0.utils.counter_manager import get_next_api_user_id
...
def create_api_user_kwargs(**kwargs) -> dict[str, Any]:
    ...
    role_names = COMMON_ROLE_COMBINATIONS[
        (user_counter - 1) % len(COMMON_ROLE_COMBINATIONS)
    ]
    ...
    for role_name in role_names:
        if role_name == "administrator_role":
            ...
            roles.append(
                create_api_role(
                    name="auditor", permissions=frozenset(["view_audit_log"])
                )
            )
            ...
            roles.append(
                create_api_role(
                    name="developer", permissions=frozenset(["access_developer_tools"])
                )
            )
...
def create_api_user_from_json(json_data: str | None = None, **kwargs) -> ApiUser:
    """
    ...
    if json_data is None:
        ...
        if isinstance(user_kwargs.get("roles"), (set, frozenset)):
            ...
            for role in user_kwargs["roles"]:
                role_dict = {"name": role.name, "permissions": list(role.permissions)}
        ...
        "roles": kwargs.get(
            ...
            frozenset(
                [
                    ...
                    create_api_role(
                        name="analytics_user", permissions=frozenset(["view_audit_log"])
                    ),
                ]
            ),
        ),
            ...
            frozenset(
                [
                    create_api_role(
                        name="premium_user",
                        permissions=frozenset(
                            ["access_basic_features", "manage_recipes"]
                        ),
                    ),
                    create_api_role(
                        name="standard_user",
            ...
            frozenset(
                [
                    ...
                    create_api_role(
                        name="nutritionist",
                        permissions=frozenset(
                            ["access_basic_features", "manage_meals"]
                        ),
                    ),
                    create_api_role(
                        name="premium_user",
...
def create_user_with_max_roles(**kwargs) -> ApiUser:
    ...
    roles = frozenset(
        [
            ...
            create_api_role(
                name="chef",
                permissions=frozenset(["access_basic_features", "manage_recipes"]),
            ),
            create_api_role(
                name="nutritionist",
                permissions=frozenset(["access_basic_features", "manage_meals"]),
            ),
            create_api_role(
                name="reviewer",
                permissions=frozenset(["access_basic_features", "view_audit_log"]),
            ),
            create_api_role(
                name="tester", permissions=frozenset(["access_basic_features"])
            ),
            create_api_role(
                name="social_manager",
                permissions=frozenset(["access_basic_features", "manage_clients"]),
            ),
        ]
    )
...
def create_user_with_duplicate_role_names(**kwargs) -> ApiUser:
    ...
    role1 = create_api_role(
        name="duplicate", permissions=frozenset(["access_basic_features"])
    )
...
def create_user_with_roles_with_many_permissions(**kwargs) -> ApiUser:
    ...
    overloaded_role = create_api_role(
        name="overloaded", permissions=too_many_permissions
    )
...
def create_test_user_dataset(user_count: int = 100) -> dict[str, Any]:
    ...
    return {"users": users, "json_strings": json_strings, "total_users": len(users)}
...
def create_valid_json_test_cases() -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "id": str(uuid4()),
            "roles": [{"name": "guest_user", "permissions": ["access_basic_features"]}],
        },
        # User with many roles
        {
            ...
            "roles": [
                {
                    "name": "administrator_role",
                    "permissions": ["manage_users", "manage_recipes", "manage_meals"],
                },
                {"name": "recipe_manager", "permissions": ["manage_recipes"]},
                {"name": "meal_manager", "permissions": ["manage_meals"]},
                {
...
def create_invalid_json_test_cases() -> list[dict[str, Any]]:
    ...
    return [
        ...
        # Invalid role structure (missing name)
        {
            "data": {
                "id": str(uuid4()),
                "roles": [{"permissions": ["access_basic_features"]}],  # Missing name
            },
            "expected_errors": ["roles"],
        },
        # Invalid roles type (not a list)
        {
            "data": {"id": str(uuid4()), "roles": "not_a_list"},  # Should be a list
            "expected_errors": ["roles"],
        },
...
def create_role_validation_performance_dataset(count: int = 1000) -> list[ApiUser]:
    ...
    for i in range(count):
        ...
        for j in range(role_count):
            ...
            permissions = frozenset(
                [
                    valid_permissions[k % len(valid_permissions)]
