...
import anyio
import pytest
from old_tests_v0.contexts.seedwork.adapters.repositories.conftest import timeout_test
from old_tests_v0.contexts.seedwork.adapters.repositories.testing_infrastructure.data_factories import (
    create_test_ORM_ingredient,
...
class TestSaGenericRepositoryComplexFilteringScenarios:
    ...
    @pytest.fixture
    async def meal_with_recipes_and_ingredients(
        ...
        for i, ingredient_data in enumerate(ingredients_data):
            ingredient = create_test_ORM_ingredient(
                recipe_id="join_test_recipe", position=i, **ingredient_data
            )
    ...
    async def test_meal_filter_by_recipe_properties_real_join(
        ...
        results = await meal_repository.query(
            filter={"recipe_name": "Chicken Alfredo"}, _return_sa_instance=True
        )
    ...
    async def test_meal_filter_by_ingredient_products_multi_level_join(
        ...
        from old_tests_v0.contexts.seedwork.adapters.repositories.testing_infrastructure.models import (
        ...
        for ing in ingredients:
            print(
                f"  Ingredient: {ing.name} (product_id={ing.product_id}, recipe_id={ing.recipe_id})"
            )

        # When: Filtering by ingredient products (Meal -> Recipe -> Ingredient joins)
        results = await meal_repository.query(
            filter={"products": ["chicken", "pasta"]}, _return_sa_instance=True
        )
        ...
        no_results = await meal_repository.query(
            filter={"products": ["non_existent_product"]}, _return_sa_instance=True
        )
    ...
    async def test_nutrition_filter_combination_composite_fields(
        ...
        assert (
            len(results) == 1
        ), f"Expected 1 meal filtering by composite nutritional facts but got {len(results)}"
        assert (
            results[0].nutri_facts.calories >= 600
        ), f"Expected calories >= 600, got {results[0].nutri_facts.calories}"
        assert (
            results[0].nutri_facts.calories <= 700
        ), f"Expected calories <= 700, got {results[0].nutri_facts.calories}"
        assert (
            results[0].nutri_facts.protein >= 30
        ), f"Expected protein >= 30, got {results[0].nutri_facts.protein}"
        ...
        percentage_results = await meal_repository.query(
            filter={
        ...
        assert (
            len(percentage_results) == 1
        ), f"Expected 1 meal filtering by carbohydrate/fat but got {len(percentage_results)}"
        assert (
            percentage_results[0].nutri_facts.carbohydrate >= 70
        ), f"Expected carbohydrate >= 70, got {percentage_results[0].nutri_facts.carbohydrate}"
        assert (
            percentage_results[0].nutri_facts.total_fat <= 30
        ), f"Expected total_fat <= 30, got {percentage_results[0].nutri_facts.total_fat}"

        # Test 3: Combined composite + calculated fields filtering
        combined_results = await meal_repository.query(
            filter={
        ...
        ), f"Expected 1 meal with combined filters but got {len(combined_results)}"
        ...
        multi_results = await meal_repository.query(
            filter={
        ...
        assert (
            len(multi_results) == 1
        ), f"Expected 1 meal with multi-component nutrition filter but got {len(multi_results)}"
    ...
    async def test_calculated_properties_filtering(
        ...
        density_results = await meal_repository.query(
            filter={
        ...
        assert (
            len(density_results) >= 1
        ), f"Expected at least 1 meal with calorie density 300-350 but got {len(density_results)}"
        found_calculated_meal = any(
            meal.id == "calculated_test_meal" for meal in density_results
        )
        assert (
            found_calculated_meal
        ), "Should find the meal with calculated calorie density"
        ...
        assert (
            len(percentage_results) >= 1
        ), f"Expected at least 1 meal with percentage filters but got {len(percentage_results)}"

        # Test 3: Combined calculated + composite filtering
        mixed_results = await meal_repository.query(
            filter={
        ...
        assert (
            len(mixed_results) >= 1
        ), f"Expected at least 1 meal with mixed composite/calculated filters but got {len(mixed_results)}"

        # Verify the calculated meal meets all criteria
        calculated_result = next(
            (meal for meal in mixed_results if meal.id == "calculated_test_meal"), None
        )
        assert (
            calculated_result is not None
        ), "Should find the calculated test meal in mixed results"
...
class TestSaGenericRepositoryComplexMultiLevelJoins:
    ...
    @pytest.fixture
    async def complex_meal_hierarchy(
        self, meal_repository, recipe_repository, test_session
    ):
        ...
        for recipe_data in recipes_data:
            ...
            for i, ingredient_data in enumerate(ingredients_data):
                ingredient = create_test_ORM_ingredient(
                    recipe_id=recipe.id, position=i, **ingredient_data
                )
                test_session.add(ingredient)
    ...
    async def test_deep_join_meal_recipe_ingredient_real_data(
        ...
        vegetable_meals = await meal_repository.query(
            filter={"products": ["vegetables"]},  # Filter by ingredient product_id
            _return_sa_instance=True,
        )
        ...
        beef_meals = await meal_repository.query(
            filter={"products": ["beef"]}, _return_sa_instance=True
        )
        ...
        no_meals = await meal_repository.query(
            filter={"products": ["lobster"]}, _return_sa_instance=True
        )
...
class TestSaGenericRepositoryJoinConstraintValidation:
    ...
    async def test_foreign_key_constraint_in_joins(
        ...
        results = await meal_repository.query(
            filter={"recipe_id": "valid_recipe"}, _return_sa_instance=True
        )
        assert len(results) == 1
    ...
    async def test_cascade_behavior_in_joins(
        ...
        recipe = create_test_ORM_recipe(
            id="cascade_recipe", meal_id="cascade_meal", author_id="chef_cascade"
        )
        test_session.add(recipe)
        await test_session.commit()
        ...
        results = await meal_repository.query(
            filter={"recipe_id": "cascade_recipe"}, _return_sa_instance=True
        )
        assert len(results) == 1
        ...
        await test_session.execute(
            text(
                f"DELETE FROM {TEST_SCHEMA}.test_recipes WHERE meal_id = 'cascade_meal'"
            )
        )
...
class TestSaGenericRepositoryJoinPerformance:
    ...
    async def test_complex_join_query_performance(
        ...
        async with async_benchmark_timer() as timer:
            simple_results = await meal_repository.query(
                filter={"author_id": "chef_5"}, _return_sa_instance=True
            )
        ...
        async with async_benchmark_timer() as timer:
            complex_results = await meal_repository.query(
                filter={
        ...
        async with async_benchmark_timer() as timer:
            ingredient_results = await meal_repository.query(
                filter={
                    "author_id": "chef_3",
        ...
        assert all(
            ...
            for results in [simple_results, complex_results, ingredient_results]
        )
    ...
    async def test_join_optimization_effectiveness(
        ...
        async with async_benchmark_timer() as timer:
            optimized_results = await meal_repository.query(
                filter={
        ...
        async with async_benchmark_timer() as timer:
            multi_join_results = await meal_repository.query(
                filter={
