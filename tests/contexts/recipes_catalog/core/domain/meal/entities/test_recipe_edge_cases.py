...
from tests.contexts.recipes_catalog.data_factories.meal.recipe.recipe_domain_factories import create_high_protein_recipe, create_ingredient, create_quick_recipe, create_rating, create_recipe, create_vegetarian_recipe


class TestRecipeEdgeCases:
    """Test Recipe entity edge cases and extreme scenarios using data factories."""

    @pytest.mark.parametrize("extreme_calories", [
        0.0,  # Zero calories
    ...
    @pytest.mark.parametrize("extreme_macros", [
        # (carbs, protein, fat)
    ...
    def test_recipe_with_varying_ingredient_counts(self, ingredient_count):
        ...
        recipe = create_recipe(
            name="Ingredient Count Test Recipe",
    ...
    @pytest.mark.parametrize("nutri_facts_scenario", [
    ...
    def test_recipe_with_different_nutrition_scenarios(self, nutri_facts_scenario):
        ...
        recipe = create_recipe(
            name="Nutrition Scenario Recipe",
            nutri_facts=nutri_facts
        )
    ...
    @pytest.mark.parametrize("invalid_rating_value", [
        -1, 0, 6, 10, -5, 100
    ])
    def test_recipe_rating_validation_edge_cases(self, invalid_rating_value):
        ...
        recipe.rate(
            user_id="test_user_2", 
            taste=3,
            convenience=invalid_rating_value
        )
    ...
    def test_recipe_cache_invalidation_comprehensive(self):
        ...
        new_nutri_facts = NutriFacts(
            ...
            carbohydrate=NutriValue(value=50.0, unit=MeasureUnit.GRAM),  # Different carbs
            protein=NutriValue(value=15.0, unit=MeasureUnit.GRAM),
            total_fat=NutriValue(value=8.0, unit=MeasureUnit.GRAM)
        )
    ...
    @pytest.mark.parametrize("recipe_type", [
        "quick",
        "high_protein", 
        "vegetarian"
    ])
    def test_specialized_recipe_edge_cases(self, recipe_type):
        ...
        if recipe_type == "quick":
            ...
            if recipe.nutri_facts and recipe.nutri_facts.protein and recipe.nutri_facts.protein.value is not None:
            ...
            vegetarian_tags = [tag for tag in recipe.tags if tag.key == "diet" and tag.value == "vegetarian"]
