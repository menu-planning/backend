...
async def create_required_sources_orm(clean_async_pg_session: AsyncSession):
    ...
    for source_data in sources_data:
        try:
            ...
            existing_source = await clean_async_pg_session.get(SourceSaModel, source_data["id"])
...
@pytest.fixture
async def product_repository_orm(clean_async_pg_session: AsyncSession, create_required_sources_orm) -> ProductRepo:
...
@pytest.fixture
async def test_session_with_sources(clean_async_pg_session: AsyncSession, create_required_sources_orm):
