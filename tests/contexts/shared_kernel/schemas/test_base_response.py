...
class TestBaseResponse:
    ...
    def test_base_response_invalid_status_code(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            BaseResponse[str](
                status_code=99, headers={}, body="test"  # Invalid status code (< 100)
            )
    ...
    def test_base_response_immutable(self):
        ...
        response = BaseResponse[str](status_code=200, headers={}, body="test")
    ...
    def test_base_response_generic_typing(self):
        ...
        list_response = BaseResponse[list[str]](
            status_code=200, headers={}, body=["item1", "item2"]
        )
        assert list_response.body == ["item1", "item2"]

        # Dict type
        dict_response = BaseResponse[dict[str, Any]](
            status_code=200, headers={}, body={"key": "value", "number": 42}
        )
...
class TestSuccessResponse:
    ...
    def test_success_response_default_status(self):
        ...
        response = SuccessResponse[str](headers=MOCK_CORS_HEADERS, body="success")
    ...
    def test_success_response_custom_2xx_status(self):
        ...
        response = SuccessResponse[str](
            status_code=202, headers=MOCK_CORS_HEADERS, body="accepted"
        )
    ...
    def test_success_response_invalid_non_2xx_status(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SuccessResponse[str](
                status_code=404, headers={}, body="error"  # Not a 2xx status code
            )
...
class TestCreatedResponse:
    ...
    def test_created_response_default_status(self):
        ...
        response = CreatedResponse[dict[str, str]](
            headers=MOCK_CORS_HEADERS, body={"id": "new-resource-id"}
        )
    ...
    def test_created_response_with_metadata(self):
        ...
        response = CreatedResponse[dict[str, str]](
            ...
            metadata={
                ...
                "location": "/api/resources/new-resource-id",
            },
        )
...
class TestCollectionResponse:
    ...
    def test_collection_response_total_pages_calculation(self):
        ...
        collection = CollectionResponse[str](
            items=["a", "b"], total_count=10, page_size=5
        )
        assert collection.total_pages == 2

        # Test with remainder
        collection = CollectionResponse[str](
            items=["a", "b", "c"], total_count=11, page_size=5
        )
        assert collection.total_pages == 3

        # Test with zero items
        collection = CollectionResponse[str](items=[], total_count=0, page_size=10)
    ...
    def test_collection_response_validation_errors(self):
        ...
        with pytest.raises(ValidationError):
            CollectionResponse[str](items=[], total_count=-1, page_size=10)

        # Zero page_size
        with pytest.raises(ValidationError):
            CollectionResponse[str](items=[], total_count=0, page_size=0)

        # Zero current_page
        with pytest.raises(ValidationError):
            CollectionResponse[str](
                items=[], total_count=0, page_size=10, current_page=0
            )
    ...
    def test_collection_response_in_success_response(self):
        ...
        items = [{"id": "1"}, {"id": "2"}]
        collection = CollectionResponse[dict[str, str]](
            items=items, total_count=2, page_size=10
        )

        response = SuccessResponse[CollectionResponse[dict[str, str]]](
            headers=MOCK_CORS_HEADERS, body=collection
        )
...
class TestConvenienceAliases:
    ...
    def test_success_message_response(self):
        ...
        message = MessageResponse(
            message="Recipe created successfully", details={"recipe_id": "recipe-123"}
        )

        response = SuccessMessageResponse(headers=MOCK_CORS_HEADERS, body=message)
    ...
    def test_created_message_response(self):
        ...
        response = CreatedMessageResponse(headers=MOCK_CORS_HEADERS, body=message)
...
class TestResponseSerialization:
    ...
    def test_response_json_serialization(self):
        ...
        message = MessageResponse(message="Test message", details={"key": "value"})
