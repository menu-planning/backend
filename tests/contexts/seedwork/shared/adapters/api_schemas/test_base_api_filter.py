...
class TestBaseApiFilter:
    ...
    def test_filter_with_all_fields(self):
        ...
        assert filter_obj.id == ["meal1", "meal2", "meal3"]
        assert filter_obj.name == "test_meal"
        assert filter_obj.author_id == ["user1", "user2"]
        assert filter_obj.total_time_gte == 30
        assert filter_obj.total_time_lte == 60
        assert filter_obj.products == ["product1", "product2"]
        assert filter_obj.privacy == Privacy.PUBLIC
        assert filter_obj.calories_gte == 200
        assert filter_obj.calories_lte == 500
        assert filter_obj.protein_gte == 20
    ...
    def test_validate_repository_filters_with_mappers(self):
        ...
        class MockMapper:
            filter_key_to_column_name = {"name": "name", "author_id": "author_id"}
