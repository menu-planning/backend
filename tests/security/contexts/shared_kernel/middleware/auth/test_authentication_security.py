...
@fixture
def auth_strategy(fake_iam_provider: FakeIAMProvider) -> AWSLambdaAuthenticationStrategy:
...
@fixture
def auth_middleware(auth_strategy: AWSLambdaAuthenticationStrategy) -> AuthenticationMiddleware:
...
TENANT_ISOLATION_PAYLOADS = [
    ...
    {"sub": "tenant-a-user", "custom:tenant": "tenant-a", "custom:roles": "tenant-b-admin"},
...
class TestAuthenticationMiddlewareClaimsValidation:
    ...
    async def test_authentication_middleware_claims_sanitization(
            ...
            "requestContext": {
                "authorizer": {
                    "claims": {
                        ...
                        "cognito:groups": ["  admin  ", "  user  ", "  <script>alert('xss')</script>  "],
        ...
        auth_context = await auth_strategy.extract_auth_context(event, mock_lambda_context)
            ...
            "requestContext": {
                "authorizer": {
                    "claims": {
                        ...
                        "nbf": int((current_time - timedelta(minutes=5)).timestamp()),  # 5 min clock skew tolerance
                        "custom:roles": "admin"
                    }
                }
            }
...
class TestAuthenticationMiddlewareTenantIsolation:
    ...
    async def test_authentication_middleware_tenant_isolation(
        ...
        # Security assertion: Handler should not be called
        assert not handler_called, "Handler should not be called with cross-tenant access"
...
class TestAuthenticationMiddlewareRoleValidation:
    ...
    async def test_authentication_middleware_invalid_role_rejection(
        ...
        # Security assertion: Handler should not be called
        assert not handler_called, "Handler should not be called with invalid role payloads"
    ...
    async def test_authentication_middleware_empty_roles_handling(
        ...
        """Test handling of empty or invalid role data.
        
        Security test that ensures empty or invalid role data is handled securely.
        """
        # Event with empty roles
        event = {
            "requestContext": {
                "authorizer": {
                    "claims": {
        ...
        # Security test: Should reject requests with empty roles
        with pytest.raises(AuthenticationError, match=AUTHENTICATION_REQUIRED_MSG):
            await auth_middleware(mock_handler, event, mock_lambda_context)
    ...
    async def test_authentication_middleware_insufficient_roles(
        ...
        # Security assertion: Handler should not be called
        assert not handler_called, "Handler should not be called with insufficient roles"
...
class TestAuthenticationMiddlewareIAMIntegration:
    ...
    async def test_authentication_middleware_iam_provider_success(
        ...
        auth_strategy = AWSLambdaAuthenticationStrategy(fake_iam_provider, "products_catalog")
