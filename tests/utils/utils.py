...
def _class_method_attributes(method) -> list[str]:
    if not inspect.ismethod(method):
        error_message = "The argument must be a class method."
...
def check_missing_attributes(cls_or_method, kwargs) -> list[str]:
    if inspect.isclass(cls_or_method):
        ...
        attribute_names = [
            attr for attr in attribute_names if attr not in excluded_attrs
        ]

    elif inspect.ismethod(cls_or_method):
        attribute_names = _class_method_attributes(cls_or_method)
    else:
        error_message = "The first argument must be a class or a class method."
        raise TypeError(error_message)

    return [attr for attr in attribute_names if attr not in kwargs]


def enum_value_serializer(_instance, _attribute, value):
...
def build_dict_from_instance(instance) -> dict[str, Any]:
    ...
    try:
        for attr_name, attr_value in vars(instance).items():
            clean_attr = attr_name.removeprefix("_")
            if clean_attr == "sa_instance_state":
                continue
            if isinstance(attr_value, list | set):
                result[clean_attr] = [build_dict_from_instance(i) for i in attr_value]
            elif isinstance(
                attr_value, Entity | ValueObject
            ) or dataclasses.is_dataclass(attr_value):
                result[clean_attr] = build_dict_from_instance(attr_value)
            elif isinstance(attr_value, Enum):
                result[clean_attr] = attr_value.value
            else:
                result[clean_attr] = attr_value
    except Exception:
        return instance
    return result
...
def compare_api_to_domain(
    api_model: Any, domain_obj: Any, excluded_fields: set[str] | None = None
) -> list[str]:
    """
    ...
    if not hasattr(api_model.__class__, "model_fields"):
        error_msg = f"API model {type(api_model)} does not have model_fields attribute"
        raise ValueError(error_msg)

    excluded_fields = excluded_fields or set()
    differences = []

    for field_name in api_model.__class__.model_fields:
        ...
        try:
            ...
            if not _values_are_equivalent(api_value, domain_value, field_name):
                differences.append(
                    f"Field '{field_name}': API({api_value}) != Domain({domain_value})"
                )
...
def compare_api_to_orm_kwargs(
    api_model: Any, orm_kwargs: dict[str, Any], excluded_fields: set[str] | None = None
) -> list[str]:
    """
    ...
    for field_name in api_model.__class__.model_fields:
        ...
        try:
            ...
            if not _values_are_equivalent(api_value, orm_value, field_name):
                differences.append(
                    f"Field '{field_name}': API({api_value}) != ORM({orm_value})"
                )
...
def compare_orm_to_domain(
    ...
    Args:
        ...
        domain_obj: Domain object instance
        field_names: Specific fields to compare (if None, attempts to infer from
            domain_obj)
        excluded_fields: Fields to exclude from comparison
    ...
    if field_names is None:
        if hasattr(domain_obj, "__dict__"):
            field_names = [
                name for name in domain_obj.__dict__ if not name.startswith("_")
            ]
        else:
    ...
    for field_name in field_names:
        ...
        try:
            ...
            if not _values_are_equivalent(orm_value, domain_value, field_name):
                differences.append(
                    f"Field '{field_name}': ORM({orm_value}) != Domain({domain_value})"
                )
...
def assert_api_domain_match(
    ...
    if differences:
        error_msg = "API model does not match domain object"
        if custom_message:
...
def assert_api_orm_kwargs_match(
    api_model: Any,
    ...
    if differences:
        error_msg = "API model does not match ORM kwargs"
        if custom_message:
            error_msg = f"{custom_message}: {error_msg}"
        error_msg += "\nDifferences found:\n" + "\n".join(
            f"  - {diff}" for diff in differences
...
def assert_orm_domain_match(
    ...
    """
    differences = compare_orm_to_domain(
        orm_obj, domain_obj, field_names, excluded_fields
    )
    if differences:
        error_msg = "ORM object does not match domain object"
        if custom_message:
            error_msg = f"{custom_message}: {error_msg}"
        error_msg += "\nDifferences found:\n" + "\n".join(
            f"  - {diff}" for diff in differences
...
def create_mock_orm_from_api(
    api_model: Any, excluded_fields: set[str] | None = None
) -> Mock:
    """
...
def test_api_domain_roundtrip(
    ...
    if differences:
        raise AssertionError(
            "API → Domain → API round-trip failed:\n"
            + "\n".join(f"  - {diff}" for diff in differences)
        )


def test_api_orm_roundtrip(
    api_model: Any, excluded_fields: set[str] | None = None
) -> None:
    """
    ...
    if differences:
        raise AssertionError(
            "API → ORM → API round-trip failed:\n"
            + "\n".join(f"  - {diff}" for diff in differences)
        )


def compare_api_to_api(
    api_model1: Any, api_model2: Any, excluded_fields: set[str] | None = None
) -> list[str]:
    """
    ...
    Returns:
        List of differences found, empty if objects match
    """
    if not isinstance(api_model1, type(api_model2)):
        return [f"Type mismatch: {type(api_model1)} != {type(api_model2)}"]

    if not hasattr(api_model1, "model_fields"):
        error_msg = f"API model {type(api_model1)} does not have model_fields attribute"
        raise ValueError(error_msg)

    excluded_fields = excluded_fields or set()
    differences = []

    for field_name in api_model1.model_fields:
        if field_name in excluded_fields:
            continue
        ...
        try:
            ...
            if not _values_are_equivalent(value1, value2, field_name):
                differences.append(
                    f"Field '{field_name}': API1({value1}) != API2({value2})"
                )
...
def _values_are_equivalent(value1: Any, value2: Any, _field_name: str) -> bool:
    """
    Check if two values are equivalent, handling special cases like collections,
    None values, nested objects, etc.
    """
    # Handle None values
    ...
    if (
        hasattr(value1, "value")
        and not isinstance(value1, int | float | str)
        and hasattr(value2, "value")
        and not isinstance(value2, int | float | str)
    ):
    ...
    return _are_nested_objects_equivalent(value1, value2)


def _are_nested_objects_equivalent(
    value1: Any, value2: Any, visited: set[int] | None = None
) -> bool:
    """
    Check if two nested objects are equivalent by recursively comparing their
    attributes. Handles circular references with a visited set.
    """
    if visited is None:
        visited = set()
    ...
    try:
        ...
        for field_name in fields1:
            try:
                ...
                # If one object has the field and the other doesn't, they're not
                # equivalent
                return False
...
def _are_compatible_nested_types(value1: Any, value2: Any) -> bool:
    ...
    return hasattr(value1, "__dict__") and hasattr(value2, "__dict__")


def _are_equivalent_collections(value1: Any, value2: Any) -> bool:
    """Check if two collections are equivalent despite different types, handling
    nested objects."""
    collection_types = (list, set, frozenset, tuple)

    if not (
        isinstance(value1, collection_types) and isinstance(value2, collection_types)
    ):
        return False
...
def _get_object_field_names(obj: Any) -> list[str]:
    ...
    if hasattr(obj, "__attrs_attrs__"):  # attrs class
        return [attr.name for attr in obj.__attrs_attrs__]
    if hasattr(obj, "__dict__"):
        return [name for name in obj.__dict__ if not name.startswith("_")]
    return []
