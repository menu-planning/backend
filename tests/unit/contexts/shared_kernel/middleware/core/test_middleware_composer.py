...
from src.contexts.shared_kernel.middleware.logging.structured_logger import LoggingStrategy
from src.contexts.shared_kernel.middleware.auth.authentication import AuthenticationStrategy, AuthContext
from src.contexts.shared_kernel.middleware.error_handling.exception_handler import ErrorHandlingStrategy
...
class FakeLoggingStrategy(LoggingStrategy):
    ...
    def inject_logging_context(self, request_data: dict[str, Any], logging_context: dict[str, Any]) -> None:
...
class FakeErrorStrategy(ErrorHandlingStrategy):
    ...
    def inject_error_context(self, request_data: dict[str, Any], error_context: dict[str, Any]) -> None:
...
class TestMiddlewareComposerOrdering:
    ...
    def test_middleware_order_enforcement(self):
        ...
        from src.contexts.shared_kernel.middleware.auth.authentication import AuthenticationMiddleware
        from src.contexts.shared_kernel.middleware.error_handling.exception_handler import ExceptionHandlerMiddleware
        
        logging1 = StructuredLoggingMiddleware(strategy=logging_strategy, name="logging1")
...
class TestMiddlewareComposerComposition:
    ...
    async def test_compose_multiple_middleware(self):
        ...
        expected_order = [
            "middleware1_pre", "middleware2_pre", "middleware3_pre",
            "middleware3_post", "middleware2_post", "middleware1_post"
        ]
...
class TestMiddlewareComposerErrorHandling:
    ...
    async def test_middleware_error_in_chain(self):
        ...
        execution_order = []
        middleware1 = FakeMiddleware("middleware1", execution_order=execution_order)
        failing_middleware = FakeMiddleware("failing", should_fail=True, execution_order=execution_order)
        middleware3 = FakeMiddleware("middleware3", execution_order=execution_order)
