...
class ConcreteApiSeedUser(
    ...
    """Concrete implementation of ConcreteApiSeedUser for testing purposes."""
...
class TestConcreteApiSeedUserComprehensive:
    """
    Comprehensive test suite for ConcreteApiSeedUser schema meeting task requirements:
    ...
    def sample_api_roles(self):
        ...
        return frozenset(
            [
                ...
                ConcreteApiSeedRole(
                    name="user", permissions=frozenset(["read", "comment"])
                ),
                ConcreteApiSeedRole(
                    name="moderator", permissions=frozenset(["read", "moderate"])
                ),
            ]
        )
    ...
    def valid_user_data(self, sample_api_roles):
        ...
        return {"id": str(uuid4()), "roles": sample_api_roles}

    @pytest.fixture
    def domain_user(self, sample_domain_roles):
        """Sample domain user for conversion tests."""
        return SeedUser(id=str(uuid4()), roles=sample_domain_roles)
    ...
    def edge_case_role_collections(self):
        ...
        return {
            "empty": frozenset(),
            "single": frozenset(
                [ConcreteApiSeedRole(name="single", permissions=frozenset(["read"]))]
            ),
            "large": frozenset(
                [
                    ConcreteApiSeedRole(
                        name=f"role_{i}", permissions=frozenset([f"perm_{i}"])
                    )
                    for i in range(50)
                ]
            ),
            "duplicate_names": frozenset(
                [
                    ConcreteApiSeedRole(
                        name="duplicate", permissions=frozenset(["read"])
                    ),
                    ConcreteApiSeedRole(
                        name="duplicate", permissions=frozenset(["write"])
    ...
    def test_from_domain_basic_conversion(self, domain_user):
        ...
        assert all(isinstance(role, ConcreteApiSeedRole) for role in api_user.roles)
    ...
    def test_from_domain_type_preservation(self, domain_user):
        ...
        assert isinstance(api_user, ConcreteApiSeedUser)
    ...
    def test_to_domain_basic_conversion(self, valid_user_data):
        ...
        api_user = ConcreteApiSeedUser(**valid_user_data)
    ...
    def test_from_orm_model_role_conversion(self, orm_user):
        ...
        for orm_role in orm_user.roles:
            matching_api_role = next(
                (
                    api_role
                    for api_role in api_user.roles
                    if api_role.name == orm_role.name
                ),
                None,
            )
            ...
            expected_permissions = frozenset(
                perm.strip()
                for perm in orm_role.permissions.split(", ")
                if perm.strip()
            )
    ...
    def test_to_orm_kwargs_empty_roles(self):
        ...
        api_user = ConcreteApiSeedUser(id=str(uuid4()), roles=frozenset())
        kwargs = api_user.to_orm_kwargs()
    ...
    def test_complete_four_layer_round_trip(self, sample_domain_roles):
        ...
        original_domain = SeedUser(id=str(uuid4()), roles=sample_domain_roles)

        # Domain → API
        api_user = ConcreteApiSeedUser.from_domain(original_domain)
        ...
        reconstructed_api = ConcreteApiSeedUser.from_orm_model(mock_orm)
    ...
    def test_round_trip_with_edge_cases(self, edge_case_role_collections):
        ...
        for case_name, roles in edge_case_role_collections.items():
            ...
            domain_user = SeedUser(
                id=str(uuid4()), roles=frozenset(role.to_domain() for role in roles)
            )
    ...
    def test_from_domain_error_scenarios(self):
        ...
        with pytest.raises(Exception):
            ConcreteApiSeedUser.from_domain(None)  # type: ignore

        # Error 2: Invalid object type
        with pytest.raises(Exception):
            ConcreteApiSeedUser.from_domain("not_a_user_object")  # type: ignore

        # Error 3: Object missing required attributes
        mock_invalid = Mock()
        del mock_invalid.id  # Missing id attribute
        with pytest.raises(AttributeError):
            ConcreteApiSeedUser.from_domain(mock_invalid)  # type: ignore
        ...
        with pytest.raises(Exception):
            ConcreteApiSeedUser.from_domain(mock_none_id)  # type: ignore
        ...
        with pytest.raises(Exception):
            ConcreteApiSeedUser.from_domain(mock_invalid_roles)  # type: ignore
    ...
    def test_to_domain_error_scenarios(self):
        ...
        with pytest.raises(ValueError):
            ConcreteApiSeedUser(id=str(uuid4()), roles="not_a_frozenset")  # type: ignore

        # Error 4: Invalid role objects in roles collection
        with pytest.raises(ValueError):
            ConcreteApiSeedUser(id=str(uuid4()), roles=["not_a_role_object"])  # type: ignore

        # Error 5: None id
        with pytest.raises(ValueError):
            ConcreteApiSeedUser(id=None, roles=frozenset())  # type: ignore
    ...
    def test_from_orm_model_error_scenarios(self):
        ...
        with pytest.raises((AttributeError, TypeError)):
            ConcreteApiSeedUser.from_orm_model(None)  # type: ignore

        # Error 2: Invalid object type
        with pytest.raises((AttributeError, TypeError)):
            ConcreteApiSeedUser.from_orm_model("not_an_orm_object")  # type: ignore
        ...
        with pytest.raises(ValueError):  # Validation error
            ConcreteApiSeedUser.from_orm_model(mock_orm)
    ...
    def test_to_orm_kwargs_error_scenarios(self):
        ...
        large_user = ConcreteApiSeedUser(id=str(uuid4()), roles=large_roles)
        kwargs = large_user.to_orm_kwargs()
        assert len(kwargs["roles"]) == 100

        # Error 2: Empty roles (should work)
        empty_user = ConcreteApiSeedUser(id=str(uuid4()), roles=frozenset())
    ...
    def test_validation_error_scenarios(self):
        ...
        for invalid_id, description in invalid_ids:
            with pytest.raises(ValueError):
                ConcreteApiSeedUser(id=invalid_id, roles=frozenset())  # type: ignore
        ...
        for invalid_roles_val, description in invalid_roles:
            with pytest.raises(ValueError):
                ConcreteApiSeedUser(id=str(uuid4()), roles=invalid_roles_val)  # type: ignore
    ...
    def test_edge_case_empty_roles(self):
        ...
        # API creation
        api_user = ConcreteApiSeedUser(id=user_id, roles=frozenset())
        assert api_user.roles == frozenset()
        ...
        api_from_orm = ConcreteApiSeedUser.from_orm_model(orm_user)
    ...
    def test_edge_case_single_role(self):
        ...
        user_id = str(uuid4())
        single_role = frozenset(
            [ConcreteApiSeedRole(name="reader", permissions=frozenset(["read"]))]
        )
        api_user = ConcreteApiSeedUser(id=user_id, roles=single_role)
    ...
    def test_edge_case_large_role_collection(self):
        ...
        large_roles = frozenset(
            [
                ConcreteApiSeedRole(
                    name=f"role_{i:03d}", permissions=frozenset([f"permission_{i}"])
                )
                for i in range(100)
            ]
        )
        api_user = ConcreteApiSeedUser(id=user_id, roles=large_roles)
    ...
    def test_edge_case_complex_role_permissions(self):
        ...
        complex_roles = frozenset(
            [
                ...
                ConcreteApiSeedRole(
                    name="unicode_role", permissions=frozenset(["읽기", "쓰기", "删除"])
                ),
            ]
        )

        api_user = ConcreteApiSeedUser(id=str(uuid4()), roles=complex_roles)

        # Test round-trip with complex permissions
        domain_user = api_user.to_domain()
        recovered_api = ConcreteApiSeedUser.from_domain(domain_user)

        # Verify permission preservation
        original_perms = {perm for role in api_user.roles for perm in role.permissions}
        recovered_perms = {
            perm for role in recovered_api.roles for perm in role.permissions
        }
    ...
    def test_from_domain_performance(self, domain_user):
        ...
        for _ in range(100):  # Test 100 conversions for reliable timing
            ConcreteApiSeedUser.from_domain(domain_user)
    ...
    def test_from_orm_model_performance(self, orm_user):
        ...
        for _ in range(100):  # Test 100 conversions
            ConcreteApiSeedUser.from_orm_model(orm_user)
    ...
    def test_complete_conversion_cycle_performance(self, domain_user):
        ...
        for _ in range(50):  # Test 50 complete cycles
            ...
            # ORM → API (simulated)
            mock_orm_roles = [
                RoleSaModel(**role_data) for role_data in orm_kwargs["roles"]
            ]
            mock_orm = UserSaModel(id=orm_kwargs["id"], roles=mock_orm_roles)
            reconstructed_api = ConcreteApiSeedUser.from_orm_model(mock_orm)
    ...
    def test_large_role_collection_performance(self):
        ...
        ), f"Large collection conversion {conversion_time_ms:.3f}ms exceeds 50ms limit"
    ...
    def test_base_api_model_conversion_utilities(self):
        ...
        assert isinstance(safe_api, ConcreteApiSeedUser)
    ...
    def test_pydantic_validation_integration(self):
        ...
        api_user = ConcreteApiSeedUser.model_validate_json(valid_data)
        assert api_user.id == user_id
        assert api_user.roles == frozenset()

        # Test validation fails for invalid data
        with pytest.raises(ValueError):
            ConcreteApiSeedUser.model_validate({"id": "", "roles": []})
    ...
    def test_serialization_integration(self):
        ...
        assert (
            user_dict["roles"] == frozenset()
        )  # Empty frozenset stays as frozenset in dict

        # Verify the structure can be reconstructed from simple data
        reconstructed = ConcreteApiSeedUser.model_validate(user_dict)
    ...
    def test_all_public_methods_covered(self):
        ...
        # from_domain
        result1 = ConcreteApiSeedUser.from_domain(domain_user)
        assert isinstance(result1, ConcreteApiSeedUser)
        ...
        # from_orm_model
        result3 = ConcreteApiSeedUser.from_orm_model(orm_user)
        assert isinstance(result3, ConcreteApiSeedUser)
    ...
    def test_field_validation_coverage(self):
        ...
        valid_user = ConcreteApiSeedUser(id=str(uuid4()), roles=frozenset())
        ...
        sample_role = ConcreteApiSeedRole(name="test", permissions=frozenset(["read"]))
        user_with_role = ConcreteApiSeedUser(
            id=str(uuid4()), roles=frozenset([sample_role])
        )
