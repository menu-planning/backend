...
class SchemaSyncResult:
    ...
    def __init__(self, domain_class: type, api_class: type, orm_class: type):
...
def get_domain_fields(domain_class: type) -> set[str]:
    ...
    for name in dir(domain_class):
        if not name.startswith("_") and name not in {
            "events",
            "domain_events",
        }:  # Exclude event-related properties
...
def compare_field_sets(
    ...
    if domain_only:
        result.warnings.append(f"Domain-only fields: {sorted(domain_only)}")

    if api_only:
        result.warnings.append(f"API-only fields: {sorted(api_only)}")

    if orm_only:
        result.warnings.append(f"ORM-only fields: {sorted(orm_only)}")
...
def assert_schema_sync(
    ...
    try:
        ...
        # Compare field sets
        compare_field_sets(
            result.domain_fields, result.api_fields, result.orm_fields, result
        )
...
def discover_schema_triplets() -> list[dict[str, type]]:
    ...
    try:
        ...
        from src.contexts.recipes_catalog.core.domain.meal.root_aggregate.meal import (
            Meal,
        )
...
class TestSchemaSynchronization:
    ...
    def test_schema_sync_detects_mismatch(self):
        ...
        class DummyDomain:
            ...
            def __init__(
                ...
                name: str,
                weight_in_grams: int,
                nutri_facts: NutriFactsComposite,
            ):
                self.id = id
                self.name = name
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "computed_domain_materialised_api_missing_orm",
                "warning",
            ),  # API missing from ORM = warning
            ...
            (
                "composite_orm_domain_missing_api",
                "error",
            ),  # Domain missing from API = error
            ...
            (
                "composite_orm_scalar_domain_api",
                "warning",
            ),  # Composite key will be missing from API = warning
        ],
    )
    @pytest.mark.unit
    def test_schema_sync_mismatch_scenarios(self, scenario, expected_behavior):
        ...
        if scenario == "happy_path":
            ...
            class CompositeMissingApiOrm(SaBase):
                ...
                composite_data: Mapped[TestComposite] = composite(
                    *[
                        mapped_column(field.name, Float)
                        for field in dataclass_fields(TestComposite)
                    ]
                )

            Domain, Api, Orm = (
                CompositeMissingApiDomain,
                CompositeMissingApiApi,
                CompositeMissingApiOrm,
            )
            ...
            Domain, Api, Orm = (
                CompositeMissingDomainDomain,
                CompositeMissingDomainApi,
                CompositeMissingDomainOrm,
            )
        ...
        if expected_behavior == "pass":
            ...
            assert (
                result.is_valid
            ), f"Expected {scenario} to pass but got errors: {result.errors}"
            assert (
                len(result.warnings) == 0
            ), f"Expected {scenario} to have no warnings but got: {result.warnings}"
            ...
            assert (
                result.is_valid
            ), f"Expected {scenario} to be valid (warnings only) but got errors: {result.errors}"
            assert (
                len(result.warnings) > 0
            ), f"Expected {scenario} to have warnings but got none"
    ...
    def test_schema_sync_excluded_fields(self):
        ...
        result = assert_schema_sync(
            ExcludedDomain, ExcludedApi, ExcludedOrm, strict=True
        )
