...
try:
    from contexts.client_onboarding.aws_lambda.create_form import (  # type: ignore
        async_handler as create_form_lambda_handler,  # type: ignore
    )
    from contexts.client_onboarding.aws_lambda.webhook_processor import (  # type: ignore
        async_lambda_handler as webhook_lambda_handler,  # type: ignore
    )
...
class LambdaHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _set_response(
        ...
        self.send_header(
            ...
            "Content-Type, Authorization, Typeform-Signature",
        )
    ...
    def _get_request_body(self) -> str | None:
    ...
    def do_POST(self):
        ...
        if self.path.startswith("/webhook"):
            ...
            response = {
                ...
                "available_endpoints": ["/webhook", "/onboarding-forms"],
            }
            self.wfile.write(json.dumps(response).encode("utf-8"))
