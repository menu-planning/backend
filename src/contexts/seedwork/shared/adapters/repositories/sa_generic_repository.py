            ...
            try:
                ...
                raise FilterValidationError(
                    message=(
                        ...
                        f"column mapping for {sa_model_type.__name__}"
                    ),
                    repository=self,
                    invalid_filters=[filter_key],
                    suggested_filters=list(mapping.keys())[
                        :5
                    ],  # Suggest available mapping keys
                ) from e
            except Exception as e:
                filter_operation_time = time.perf_counter() - filter_operation_start
                self._repo_logger.debug_filter_operation(
        ...
        try:
            ...
            for i, obj in enumerate(sa_objs):
                ...
                try:
                    ...
                    if (
                        i < self.MAX_MAPPING_LOG_COUNT
                    ):  # Log details for first few mappings
                        self._repo_logger.debug_performance_detail(
