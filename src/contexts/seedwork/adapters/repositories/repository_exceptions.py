...
class RepositoryError[R: SaGenericRepository](Exception):
    """
    ...
    Attributes:
        ...
        operation: The operation being performed when the error occurred (e.g., 'query',
                   'save', 'delete')
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            context: Additional context information as key-value pairs
            correlation_id: Optional correlation ID for tracking (auto-generated if
                            not provided)
        """
        ...
        self.repository_type = (
            repository.__class__.__name__ if repository else "Unknown"
        )
...
class RepositoryQueryError[R: SaGenericRepository](RepositoryError[R]):
    """
...
class FilterValidationError[R: SaGenericRepository](RepositoryError[R]):
    """
...
class JoinError[R: SaGenericRepository](RepositoryError[R]):
    """
...
class EntityMappingError[R: SaGenericRepository](RepositoryError[R]):
    """Exception raised when entity mapping fails.

    This exception captures information about failed mapping operations between
    domain objects and SQLAlchemy models, including the direction of mapping
    ...
    Example:
        ```python
        try:
            sa_obj = mapper.map_domain_to_sa(session, domain_obj)
        except EntityMappingError as e:
            logger.error(
                "Mapping failed",
                domain_obj_type=type(e.domain_obj).__name__,
                sa_obj_type=type(e.sa_obj).__name__,
                mapping_direction=e.mapping_direction
            )
        ```
    """
...
class EntityNotFoundError[R: SaGenericRepository](RepositoryError[R]):
    """Basic EntityNotFoundError for backward compatibility.

    This exception is raised when an entity cannot be found by its ID.
    It maintains backward compatibility with existing code while providing
    the enhanced context of the RepositoryError base class.
...
class MultipleEntitiesFoundError[R: SaGenericRepository](RepositoryError[R]):
    """Basic MultipleEntitiesFoundError for backward compatibility.

    This exception is raised when multiple entities are found when only
    one was expected. It maintains backward compatibility with existing
    code while providing the enhanced context of the RepositoryError base class.
