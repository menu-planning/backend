...
class QueryBuilder[E: Entity, S: SaBase]:
    """Generic query builder for constructing SQLAlchemy Select statements.
    ...
    Example:
        Basic usage:
        ```python
        from src.contexts.seedwork.adapters.filter_operators import (
            EqualsOperator,
            GreaterThanOperator,
        )
    ...
    def __init__(
        ...
        self._starting_stmt: Select | None = starting_stmt
        self._stmt: Select | None = None
        self._already_joined: set[str] = set()

    def _require_select_called(self, method_name: str) -> None:
        """Validate that select() has been called before the current method."""
        if self._stmt is None:
            error_msg = (
                f"{method_name}() must be called after select(). Call .select() "
                "first to initialize the query."
            )
            raise ValueError(error_msg)
        assert self._stmt is not None

    def _validate_positive_int(
        self, value: Any, param_name: str, method_name: str
    ) -> None:
        """Validate that a value is a positive integer."""
        if not isinstance(value, int):
            error_msg = f"{param_name} must be an integer, got {type(value)}"
            raise TypeError(error_msg)

        if value <= 0:
            error_msg = f"{param_name} must be greater than 0, got {value}"
            raise ValueError(error_msg)
    ...
    def _validate_non_negative_int(
        ...
        if value < 0:
            error_msg = f"{param_name} must be greater than or equal to 0, got {value}"
            raise ValueError(error_msg)
    ...
    def select(self) -> QueryBuilder[E, S]:
        ...
        if self._stmt is not None:
            error_msg = "select() has already been called on this QueryBuilder instance"
            raise ValueError(error_msg)
    ...
    def where(
        self, operator: FilterOperator, column: ColumnElement[Any], value: Any
    ) -> QueryBuilder[E, S]:
        """
        ...
        """
        self._require_select_called("where")
        assert self._stmt is not None

        if not isinstance(operator, FilterOperator):
            error_msg = (
                f"operator must be a FilterOperator instance, got {type(operator)}"
            )
            raise TypeError(error_msg)
    ...
    def join(
        self, target: type[SaBase], on_clause: ColumnElement[bool]
    ) -> QueryBuilder[E, S]:
        """
        ...
        Example:
            ...
            builder = (QueryBuilder[Product, ProductSaModel](session, ProductSaModel)
                      ...
                      .join(
                        CategorySaModel,
                        ProductSaModel.category_id == CategorySaModel.id
                      )
        ...
        Note:
            ...
            matching the existing repository behavior: str(target) not in
            already_joined. This ensures backward compatibility with existing
            join logic.
        """
        self._require_select_called("join")
        assert self._stmt is not None

        if not issubclass(target, SaBase):
            error_msg = (
                f"target must be a SQLAlchemy model class inheriting from SaBase, "
                f"got {type(target)}"
            )
            raise TypeError(error_msg)
    ...
    def order_by(
        ...
        *,
        ...
        """
        Add an ORDER BY clause to the query with optional descending order and
        nulls handling.
        ...
        Args:
            column: The column to sort by. Can be either:
                   ...
                   - A string column name (for convenience, though ColumnElement
                   is preferred)
            descending: Whether to sort in descending order.
                        Defaults to False (ascending).
            nulls_last_order: Whether to place NULL values last. Defaults to True to
                              match existing repository behavior.
        ...
        """
        self._require_select_called("order_by")
        assert self._stmt is not None
    ...
    def limit(self, limit_value: int) -> QueryBuilder[E, S]:
        ...
        """
        self._require_select_called("limit")
        self._validate_positive_int(limit_value, "limit_value", "limit")
        assert self._stmt is not None
    ...
    def offset(self, offset_value: int) -> QueryBuilder[E, S]:
        ...
        Args:
            offset_value: The number of rows to skip. Must be greater than or
            equal to 0. A value of 0 means no offset (start from the first row).
        ...
        """
        self._require_select_called("offset")
        self._validate_non_negative_int(offset_value, "offset_value", "offset")
        assert self._stmt is not None
    ...
    def distinct(self) -> QueryBuilder[E, S]:
        ...
        Example:
            ...
            builder = (QueryBuilder[Product, ProductSaModel](session, ProductSaModel)
                      ...
                      .join(TagSaModel, ProductSaModel.id == TagSaModel.product_id)
                      .where(
                        InOperator(), CategorySaModel.name, ["Electronics", "Books"]
                      )
        ...
        """
        self._require_select_called("distinct")
        assert self._stmt is not None
    ...
    def build(self) -> Select:
        ...
        """
        self._require_select_called("build")
        assert self._stmt is not None
    ...
    async def execute(
        self,
        data_mapper: ModelMapper | None = None,
        *,
        ...
        """
        self._require_select_called("execute")
        assert self._stmt is not None

        if not _return_sa_instance and data_mapper is None:
            error_msg = "data_mapper must be provided when _return_sa_instance is False"
            raise ValueError(error_msg)
        ...
        try:
            ...
            assert (
                data_mapper is not None
            ), "data_mapper should not be None at this point"
