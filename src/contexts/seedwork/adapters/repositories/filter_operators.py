...
class FilterOperator(abc.ABC):
    ...
    Examples:
        ...
        # WHERE products.category = 'Electronics' AND products.price > 100.0
        ```
        ...
        query = (builder
                ...
                .where(InOperator().apply(
                    stmt, ProductSaModel.tags, ["organic", "fresh"])
                )
                .build())
        ```
    """
...
class GreaterThanOperator(FilterOperator):
    ...
    def apply(self, stmt: Select, column: ColumnElement, value: Any) -> Select:
        ...
        if value is None:
            error_msg = "GreaterThanOperator does not support None values"
...
class LessThanOperator(FilterOperator):
    ...
    def apply(self, stmt: Select, column: ColumnElement, value: Any) -> Select:
        ...
        if value is None:
            error_msg = "LessThanOperator does not support None values"
            raise ValueError(error_msg)
...
class InOperator(FilterOperator):
    ...
    def apply(self, stmt: Select, column: ColumnElement, value: Any) -> Select:
        ...
        if not isinstance(value, list | set | tuple):
            error_msg = f"InOperator requires a list, set, or tuple, got {type(value)}"
...
class NotInOperator(FilterOperator):
    ...
    Examples:
        ...
        stmt = operator.apply(
            select(Product), Product.category, ["Electronics", "Books"],
        )
        # Results in: WHERE (product.category IS NULL
        # OR product.category NOT IN ('Electronics', 'Books'))
        ```
    """
    ...
    def apply(self, stmt: Select, column: ColumnElement, value: Any) -> Select:
        ...
        if not isinstance(value, list | set | tuple):
            error_msg = (
                f"NotInOperator requires a list, set, or tuple, got {type(value)}"
            )
            raise TypeError(error_msg)
        if not value:
        ...
        return stmt.where((column.is_(None)) | (~column.in_(value)))
...
class LikeOperator(FilterOperator):
    ...
    def apply(self, stmt: Select, column: ColumnElement, value: Any) -> Select:
        ...
        if value is None:
            error_msg = "LikeOperator does not support None values"
            raise ValueError(error_msg)
...
class FilterOperatorRegistry:
    ...
    _default_postfixes: list[str] = field(
    ...
    def register_postfix(
        self, postfix: str, operator: FilterOperator | None = None
    ) -> None:
...
class FilterOperatorFactory:
    ...
    def get_operator(
        self, filter_name: str, column_type: type, value: Any
    ) -> FilterOperator:
