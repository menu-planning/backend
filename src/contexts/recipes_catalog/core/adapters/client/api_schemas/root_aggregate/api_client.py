...
class ApiClient(BaseApiEntity[Client, ClientSaModel]):
    ...
    def from_domain(cls, domain_obj: Client) -> "ApiClient":
        ...
        return cls(
            ...
            contact_info=(
                ApiContactInfo.from_domain(domain_obj.contact_info)
                if domain_obj.contact_info
                else None
            ),
            address=(
                ApiAddress.from_domain(domain_obj.address)
    ...
    def from_orm_model(cls, orm_model: ClientSaModel) -> "ApiClient":
        ...
        return cls(
            ...
            contact_info=(
                ApiContactInfo.from_orm_model(orm_model.contact_info)
                if orm_model.contact_info
                else None
            ),
            address=(
                ApiAddress.from_orm_model(orm_model.address)
                if orm_model.address
                else None
            ),
    ...
    def to_orm_kwargs(self) -> dict[str, Any]:
        ...
        return {
            ...
            "contact_info": (
                self.contact_info.to_orm_kwargs()
                if self.contact_info
                else ContactInfoSaModel()
            ),
            "address": (
                self.address.to_orm_kwargs() if self.address else AddressSaModel()
            ),
