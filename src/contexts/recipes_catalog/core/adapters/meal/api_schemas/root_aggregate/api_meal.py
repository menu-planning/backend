...
class ApiMeal(BaseApiEntity[Meal, MealSaModel]):
    ...
    **API Entity Adapter** for Meal domain objects in Clean Architecture implementation.
    ...
    This class serves as the **adapter layer** between external interfaces (REST APIs,
    AWS Lambda events) and the internal domain layer, implementing the Adapter Pattern
    to isolate domain logic from external concerns.
    ...
    **Clean Architecture Flow:**
    ...
    1. **Type Conversion**: Convert between external API types (JSON strings) and
                            internal domain types (UUIDs, enums, value objects)
    2. **Validation**: Enforce data integrity at the API boundary with strict
                        Pydantic v2 validation
    3. **Serialization**: Handle JSON serialization/deserialization for HTTP responses
    4. **Security**: Sanitize inputs and prevent injection attacks through strict
                     validation

    ## Security Features (Pydantic v2 Configuration)

    - **Immutable Models**: Frozen to prevent accidental mutation
    - **Strict Validation**: No automatic type conversions - explicit validation
                             required
    ...
    **Methods**:
    ...
    def to_orm_kwargs(self) -> dict[str, Any]:
        ...
        **Clean Architecture Flow**: API Layer → Infrastructure Layer
        ...
        transforming validated API types to database/infrastructure types for
        persistence.
        Part of the complete command chain:
        JSON → API → Domain → Message Bus → Repository.
        ...
        **Usage Pattern**:
        ...
        return {
            ...
            "menu_id": self.menu_id,
            "recipes": (
                [r.to_orm_kwargs() for r in self.recipes] if self.recipes else []
            ),
            ...
            "image_url": str(self.image_url) if self.image_url else None,
            "nutri_facts": (
                NutriFactsSaModel(**self.nutri_facts.to_orm_kwargs())
                if self.nutri_facts
                else None
            ),
