...
class MiddlewareComposer:
    ...
    Enforces a fixed, secure middleware order to ensure consistent behavior
    and prevent security vulnerabilities. The execution order is:
    ...
    Attributes:
        middleware: List of middleware components in enforced order.
        default_timeout: Default timeout in seconds for the entire middleware chain.

    Notes:
        Handles timeout management using anyio cancel scopes.
        Provides exception grouping for better error handling.
        Order: runs middleware in fixed security order.
    ...
    def compose(
        ...
        Notes:
            Wraps each middleware around the current handler in reverse order.
            Applies timeout handling at the outermost level.
        """
    ...
    def _wrap_with_timeout(
        ...
        Notes:
            Uses anyio.move_on_after for graceful timeout handling.
            Ensures cancellation exceptions are properly propagated.
        """
        if timeout is None:
            return handler
        ...
        async def timeout_wrapped_handler(
            ...
            Notes:
                ...
                Cancellation exceptions are properly propagated.
