...
This middleware provides standardized authentication and authorization
across all endpoints by:
...
# Import aliases for context-specific ApiUser classes to avoid long lines
try:
    ...
    async def get_user(self, user_id: str, caller_context: str) -> dict[str, Any]:
        ...
        try:
            ...
            if caller_context == "products_catalog":
                ApiUser = ProductsApiUser
            elif caller_context == "recipes_catalog":
                ApiUser = RecipesApiUser
            elif caller_context == "client_onboarding":
                ApiUser = ClientOnboardingApiUser
...
class AuthMiddleware:
    ...
    def __init__(
        ...
        self._current_auth_context: AuthContext | None = None
    ...
    async def _authenticate_user(self, event: dict[str, Any]) -> AuthContext:
        ...
        try:
            ...
            if response.get("statusCode") != 200:
                ...
                raise AuthenticationError(f"IAM authentication failed: {error_body}")
    ...
    async def _handle_auth_error(
        self, error: Exception, error_context: str, status_code: int
    ) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "body": json.dumps(
                {"message": str(error), "correlation_id": correlation_id}
            ),
        }

    @property
    def current_user(self) -> AuthContext | None:
        """Get current authenticated user context."""
        return self._current_auth_context

    def require_permission(self, permission: str, context: str | None = None):
        """
        ...
        def decorator(handler_func):
            async def wrapper(*args, **kwargs):
                if (
                    not self._current_auth_context
                    or not self._current_auth_context.is_authenticated
                ):
                    raise AuthenticationError("Authentication required")
    ...
    def require_owner_or_permission(self, resource_owner_id: str, permission: str):
        ...
        if not self._current_auth_context.is_owner_or_has_permission(
            resource_owner_id, permission
        ):
            error_msg = f"Must be resource owner or have permission: {permission}"
...
def create_auth_middleware(
    caller_context: str, require_authentication: bool = True
) -> AuthMiddleware:
    """
    ...
    return AuthMiddleware(
        caller_context=caller_context, require_authentication=require_authentication
    )
...
def recipes_auth_middleware() -> AuthMiddleware:
    ...
    """
    return AuthMiddleware(caller_context="recipes_catalog", require_authentication=True)
...
def optional_auth_middleware(caller_context: str) -> AuthMiddleware:
    """
    Create auth middleware that doesn't require authentication
    ...
    """
    return AuthMiddleware(caller_context=caller_context, require_authentication=False)
...
async def get_current_user_legacy(
    event: dict[str, Any], caller_context: str
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "statusCode": 500,
            "body": json.dumps({"message": f"Authentication error: {e!s}"}),
        }
    finally:
        # Clean up cache
