...
def bootstrap(
    ...
    injected_command_handlers: dict[type[SeedworkCommand], partial[Coroutine]] = {
        commands.CreateUser: partial(cmd_handlers.create_user),
        commands.AssignRoleToUser: partial(cmd_handlers.assign_role_to_user),
        commands.RemoveRoleFromUser: partial(
            cmd_handlers.remove_role_from_user
        ),
    }
    return MessageBus(
