...
ERROR_MESSAGES = {
    ...
    "FIELD_ID_CANNOT_BE_EMPTY": "Field ID cannot be empty",
    "FIELD_TITLE_CANNOT_BE_EMPTY": "Field title cannot be empty",
}

# Magic number constants
PHONE_MIN_LENGTH = 7
US_PHONE_LENGTH = 10
US_PHONE_WITH_COUNTRY_CODE_LENGTH = 11
US_COUNTRY_CODE = "1"


def _validate_url_structure(url: str) -> None:
    """Validate URL structure and raise appropriate exceptions."""
    try:
        parsed = urlparse(url)
        if not parsed.netloc:
            raise ValueError(ERROR_MESSAGES["INVALID_URL_FORMAT"])
    except Exception as err:
        raise ValueError(ERROR_MESSAGES["INVALID_URL_FORMAT"]) from err
...
class SanitizedTextResponse(BaseModel):
    ...
    is_truncated: bool = Field(default=False, description="Whether value was truncated")
...
class ValidatedNumberResponse(BaseModel):
    ...
    raw_value: int | float | str = Field(..., description="Raw number value")
    numeric_value: int | float = Field(..., description="Parsed numeric value")
    ...
    @classmethod
    def parse_number(cls, v: float | str) -> int | float:
    ...
    def analyze_number(values: "ValidatedNumberResponse") -> "ValidatedNumberResponse":
        ...
        values.is_integer = isinstance(num_val, int) or (
            isinstance(num_val, float) and num_val.is_integer()
        )
...
class ValidatedPhoneResponse(BaseModel):
    ...
    def format_phone(values: "ValidatedPhoneResponse") -> "ValidatedPhoneResponse":
        ...
        if len(digits_only) == US_PHONE_LENGTH:
            # US format
            values.formatted_phone = (
                f"({digits_only[:3]}) {digits_only[3:6]}-{digits_only[6:]}"
            )
            values.country_code = "US"
        elif len(
            digits_only
        ) == US_PHONE_WITH_COUNTRY_CODE_LENGTH and digits_only.startswith(
            US_COUNTRY_CODE
        ):
        ...
        values.is_valid = len(digits_only) >= PHONE_MIN_LENGTH
        return values
...
class ValidatedDateResponse(BaseModel):
    ...
    def process_date(values: "ValidatedDateResponse") -> "ValidatedDateResponse":
        ...
        try:
            ...
            values.is_future = parsed_date > current_date
...
class ValidatedChoiceResponse(BaseModel):
    ...
    choice_ref: str | None = Field(None, description="Choice reference")
    is_other: bool = Field(default=False, description="Whether this is 'other' choice")
    other_text: str | None = Field(None, description="Text for 'other' choice")
...
class NormalizedFieldResponse(BaseModel):
    ...
    text_response: SanitizedTextResponse | None = Field(
        None, description="Text response data"
    )
    email_response: ValidatedEmailResponse | None = Field(
        None, description="Email response data"
    )
    url_response: ValidatedUrlResponse | None = Field(
        None, description="URL response data"
    )
    number_response: ValidatedNumberResponse | None = Field(
        None, description="Number response data"
    )
    phone_response: ValidatedPhoneResponse | None = Field(
        None, description="Phone response data"
    )
    date_response: ValidatedDateResponse | None = Field(
        None, description="Date response data"
    )
    choice_response: ValidatedChoiceResponse | None = Field(
        None, description="Single choice response"
    )
    choices_response: list[ValidatedChoiceResponse] | None = Field(
        None, description="Multiple choice responses"
    )
    boolean_response: bool | None = Field(None, description="Boolean response")
...
class FormResponseDataValidation(BaseModel):
    ...
    validated_fields: list[NormalizedFieldResponse] = Field(
        ..., description="Validated field responses"
    )
    ...
    def calculate_completion(
        values: "FormResponseDataValidation",
