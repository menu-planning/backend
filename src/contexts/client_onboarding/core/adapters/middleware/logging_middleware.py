...
from src.contexts.shared_kernel.middleware.logging_middleware import (
    LoggingMiddleware as BaseLoggingMiddleware,
)
...
class ClientOnboardingLoggingMiddleware(BaseLoggingMiddleware):
    ...
    def extract_typeform_context(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        typeform_context.update(
            {
                "typeform_signature": (
                    bool(headers.get("typeform-signature"))
                    if not self.log_typeform_signatures
                    else headers.get("typeform-signature")
                ),
                "typeform_user_agent": headers.get("user-agent", "").startswith(
                    "Typeform"
                ),
                ...
                typeform_context.update(
                    {
                        ...
                        "form_response": (
                            {
                                "form_id": payload.get("form_response", {}).get(
                                    "form_id"
                                ),
                                "response_id": payload.get("form_response", {}).get(
                                    "token"
                                ),
                                "submitted_at": payload.get("form_response", {}).get(
                                    "submitted_at"
                                ),
                                "landed_at": payload.get("form_response", {}).get(
                                    "landed_at"
                                ),
                            }
                            if payload.get("form_response")
                            else None
                        ),
                    }
                )
    ...
    def _determine_access_pattern(self, path: str, method: str) -> str:
        ...
        if "/forms/" in path and method == "GET":
            return "form_query"
        if "/forms/" in path and method in ["POST", "PUT"]:
    ...
    def _requires_form_ownership(self, path: str, method: str) -> bool:
        ...
        return ("/forms/" in path or "/responses/" in path) and "/webhook" not in path
        ...
        for key, value in extracted_identifiers.items():
            if key in ["email", "phone"] and value:
                ...
                sanitized_identifiers[key] = {
                    "present": True,
                    "domain": (
                        value.split("@")[1] if "@" in value and key == "email" else None
                    ),
    ...
    def log_security_event(
        ...
        if threat_level in ["HIGH", "CRITICAL"] or failure_context.get(
    ...
    def _categorize_security_event(self, event_type: str) -> str:
        ...
        if "webhook_replay" in event_type:
            return "replay_protection"
        if "webhook_rate_limit" in event_type:
    ...
    def _extract_webhook_metadata(
        ...
        if "payload_hash" in details:
            webhook_meta["payload_info"] = {
                ...
                "size_category": self._categorize_payload_size(
                    details.get("payload_size", 0)
                ),
            }
    ...
    def _analyze_failure_patterns(
        self, event_type: str, security_result: str, details: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if any(
            ...
            for keyword in ["malformed", "invalid format", "missing"]
        ):
    ...
    def _categorize_error(self, error_message: str) -> str:
        ...
        if "malformed" in error_lower or "invalid format" in error_lower:
            return "malformed_request"
        if "missing" in error_lower:
            return "missing_required_data"
        if "timeout" in error_lower or "expired" in error_lower:
            return "timing_issue"
    ...
    async def log_request_response(
        ...
        async with super().log_request_response(
            ...
            enhanced_context = {
                **operation_context,
                ...
                "client_onboarding_metadata": {
                    "is_webhook_request": typeform_context.get(
                        "typeform_user_agent", False
                    ),
                    "has_form_signature": bool(
                        typeform_context.get("typeform_signature")
                    ),
                    "requires_ownership_check": ownership_context.get(
                        "requires_form_ownership", False
                    ),
                    "access_pattern": ownership_context.get(
...
def create_webhook_logging_middleware(
    performance_threshold: float = 10.0, enable_audit: bool = True
...
def create_api_logging_middleware(
    include_response_body: bool = True, enable_audit: bool = True
) -> ClientOnboardingLoggingMiddleware:
    """
