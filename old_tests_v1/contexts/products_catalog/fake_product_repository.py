...
class FakeProductRepository:
    ...
    async def get(self, id: str) -> Product:
        ...
        if id not in self._products:
            error_msg = f"Product with id {id} not found"
    ...
    async def list_all_brand_names(self) -> list[str]:
        ...
        for product in self._products.values():
            if product.brand_id:
                brands.add(
                    product.brand_id
                )  # Simplified - real implementation would join with brands
        return list(brands)
    ...
    async def list_filter_options(
        ...
        *,
        ...
        categories = list(
            set(p.category_id for p in self._products.values() if p.category_id)
        )
    ...
    async def query(
        ...
        if "parent_category" in filter:
            ...
            if isinstance(parent_category, list):
                results = [
                    p for p in results if p.parent_category_id in parent_category
                ]
            else:
                results = [
                    p for p in results if p.parent_category_id == parent_category
                ]
