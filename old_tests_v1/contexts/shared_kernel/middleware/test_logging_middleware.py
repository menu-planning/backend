...
class TestLoggingMiddleware:
    ...
    async def test_correlation_id_context_management(
        self, middleware, mock_event, mock_context
    ):
        ...
        async with middleware.log_request_response(
    ...
    async def test_custom_correlation_id_usage(
        self, middleware, mock_event, mock_context
    ):
        ...
        async with middleware.log_request_response(
            mock_event, mock_context, custom_id
        ) as operation_context:
            assert operation_context["correlation_id"] == custom_id
            assert correlation_id_ctx.get() == custom_id

    async def test_exception_handling_preserves_context_reset(
        self, middleware, mock_event, mock_context
    ):
    ...
    async def test_middleware_decorator_functionality(
        self, middleware, mock_event, mock_context
    ):
    ...
    async def test_middleware_preserves_handler_exceptions(
        self, middleware, mock_event, mock_context
    ):
...
class TestConvenienceFunctions:
    ...
    def test_create_logging_middleware_custom_configuration(self):
        ...
        middleware = create_logging_middleware(
            logger_name="custom", log_request_body=False, max_body_size=200
        )
...
class TestIntegrationBehavior:
    ...
    async def test_correlation_id_availability_during_request(
