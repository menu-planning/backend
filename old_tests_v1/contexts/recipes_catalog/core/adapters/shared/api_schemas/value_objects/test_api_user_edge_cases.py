...
def single_role_user_kwargs(
    role_name: str = "test_role", permissions: frozenset | None = None, **overrides
) -> dict:
    ...
    return {"id": str(uuid4()), "roles": frozenset([role]), **overrides}
...
def multi_role_user_kwargs(role_specs: list | None = None, **overrides) -> dict:
    ...
    roles = frozenset(
        [ApiRole(name=name, permissions=perms) for name, perms in role_specs]
    )
...
class TestApiUserEdgeCases:
    ...
    def test_role_permission_boundary_conditions(self):
        ...
        minimal_role = ApiRole(
            name="minimal_role", permissions=frozenset(["access_basic_features"])
        )
        ...
        max_role = ApiRole(
            name="max_permissions_role", permissions=all_valid_permissions
        )
    ...
    def test_duplicate_role_name_handling(self):
        ...
        role1 = ApiRole(
            name="duplicate_role", permissions=frozenset(["access_basic_features"])
        )
        role2 = ApiRole(
            name="duplicate_role", permissions=frozenset(["manage_recipes"])
        )
    ...
    def test_empty_role_permissions_edge_case(self):
        ...
        empty_permissions_role = ApiRole(
            name="empty_perms_role", permissions=frozenset()  # Empty permissions set
        )
    ...
    def test_role_name_validation_edge_cases(self):
        ...
        min_length_role = ApiRole(
            name="min", permissions=frozenset(["access_basic_features"])
        )
        ...
        long_name_role = ApiRole(
            name=long_name, permissions=frozenset(["access_basic_features"])
        )
    ...
    def test_multiple_users_with_same_role_references(self):
        ...
        shared_role = ApiRole(
            name="shared_role", permissions=frozenset(["access_basic_features"])
        )

        # Create multiple users with the same role object (using valid UUID4 format)
        user1 = ApiUser(
            **minimal_user_kwargs(
                id=str(uuid4()), roles=frozenset([shared_role])  # Generate valid UUID4
            )
        )
        user2 = ApiUser(
            **minimal_user_kwargs(
    ...
    def test_complex_role_combinations_edge_case(self):
        ...
        complex_roles = [
            (
                "administrator_role",
                frozenset(["manage_users", "manage_recipes", "view_audit_log"]),
            ),
        ...
        assert (
            "manage_recipes" in all_permissions
        )  # Should appear from both admin and developer
