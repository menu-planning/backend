...
class TestApiUpdateMealConversion:
    ...
    def test_update_meal_command_validation(self, meal_factory):
        ...
        assert isinstance(domain_command.meal_id, str)
        assert (
            len(domain_command.meal_id.replace("-", "")) == 32
    ...
    def test_conversion_with_realistic_meal_scenarios(self, scenario_data):
        ...
        assert isinstance(
            domain_command, UpdateMeal
        ), f"Conversion failed for scenario: {scenario_data['name']}"
        assert (
            domain_command.meal_id == api_meal.id
        ), f"Meal ID mismatch for scenario: {scenario_data['name']}"
        assert isinstance(
        ...
        assert (
            domain_command.updates["name"] == api_meal.name
        ...
        if api_meal.recipes:
            if "recipes" in domain_command.updates:
                assert len(domain_command.updates["recipes"]) == len(
                    api_meal.recipes
    ...
    def test_conversion_with_meal_type_variants(self, variant_name, meal_factory):
        ...
        assert (
            domain_command.meal_id == api_meal.id
        ), f"Meal ID mismatch for variant: {variant_name}"
        assert isinstance(
            domain_command.updates, dict
        ...
        if variant_name == "minimal":
            ...
            has_collections = (
                "recipes" in domain_command.updates or "tags" in domain_command.updates
            )
            ...
            if "recipes" in domain_command.updates:
                assert (
                    len(domain_command.updates["recipes"]) > 0
    ...
    def test_meal_configuration_field_consistency(self, meal_factory):
        ...
        if "recipes" in updates:
            ...
            for recipe in updates["recipes"]:
                ...
                assert (
                    recipe.meal_id == api_meal.id
                )  # Recipe should belong to this meal
        ...
        if "tags" in updates:
            ...
            for tag in updates["tags"]:
                ...
                assert (
                    tag.author_id == api_meal.author_id
                )  # Tag should belong to meal author
    ...
    def test_conversion_with_truly_empty_optional_fields(self):
        ...
        api_update_meal = ApiUpdateMeal(meal_id=minimal_meal.id, updates=updates)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (name, data)
            for name, data in create_type_conversion_test_scenarios()
            .get("uuid_string_conversions", {})
            .items()
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (name, data)
            for name, data in create_conversion_method_test_scenarios()
            .get("api_to_domain_conversion", {})
            .items()
            if "setup" in data
        ],
    )
    def test_command_structure_consistency_comprehensive(
        self, scenario_name, scenario_data
    ):
        ...
        assert isinstance(
            domain_command, UpdateMeal
        ), f"Command type inconsistent for scenario: {scenario_name}"
        assert hasattr(
            domain_command, "meal_id"
        ), f"Missing meal_id for scenario: {scenario_name}"
        assert hasattr(
            domain_command, "updates"
        ), f"Missing updates for scenario: {scenario_name}"

        # Verify meal_id consistency
        assert isinstance(
            domain_command.meal_id, str
        ), f"meal_id type inconsistent for scenario: {scenario_name}"
        assert (
            domain_command.meal_id == api_meal.id
        ), f"meal_id value inconsistent for scenario: {scenario_name}"

        # Verify updates structure consistency
        assert isinstance(
            domain_command.updates, dict
        ), f"updates type inconsistent for scenario: {scenario_name}"

        # Verify required fields are always present
        assert (
            "name" in domain_command.updates
        ), f"name missing from updates for scenario: {scenario_name}"
        assert isinstance(
            domain_command.updates["name"], str
        ), f"name type inconsistent for scenario: {scenario_name}"
        assert (
            domain_command.updates["name"] == api_meal.name
        ), f"name value inconsistent for scenario: {scenario_name}"

    @pytest.mark.parametrize(
        "factory_name,meal_factory",
        [
    ...
    def test_command_structure_with_different_meal_factories(
        self, factory_name, meal_factory
    ):
        ...
        assert isinstance(
            domain_command.meal_id, str
        ), f"meal_id structure inconsistent for {factory_name}"
        assert isinstance(
            domain_command.updates, dict
        ...
        assert (
            len(domain_command.meal_id.replace("-", "")) == 32
        ), f"meal_id format inconsistent for {factory_name}"
        assert (
            "name" in domain_command.updates
    ...
    def test_command_field_types_comprehensive(self):
        ...
        for field in uuid_fields:
            if field in updates and updates[field] is not None:
                assert isinstance(
                    updates[field], str
                ), f"UUID field {field} is not string: {type(updates[field])}"
                assert (
                    len(updates[field].replace("-", "")) == 32
                ), f"UUID field {field} has invalid format: {updates[field]}"
        ...
        for field in boolean_fields:
            if field in updates:
                assert isinstance(
                    updates[field], (bool, type(None))
                ), f"Boolean field {field} has wrong type: {type(updates[field])}"

        # Test collection field types
        if "recipes" in updates:
            assert isinstance(
                updates["recipes"], list
            ), f"recipes field is not list: {type(updates['recipes'])}"
            for recipe in updates["recipes"]:
                ...
                assert isinstance(
                    recipe.id, str
                ), f"Recipe id wrong type: {type(recipe.id)}"
                assert isinstance(
                    recipe.name, str
                ), f"Recipe name wrong type: {type(recipe.name)}"
                assert isinstance(
                    recipe.meal_id, str
                ), f"Recipe meal_id wrong type: {type(recipe.meal_id)}"

        if "tags" in updates:
            assert isinstance(
                updates["tags"], set
            ), f"tags field is not set: {type(updates['tags'])}"
            for tag in updates["tags"]:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (name, data)
            for name, data in create_type_conversion_test_scenarios()
            .get("collection_conversions", {})
            .items()
        ],
    )
    def test_command_field_types_with_type_scenarios(
        self, scenario_name, scenario_data
    ):
        ...
        for field_name in collection_fields:
            if field_name in updates:
                if field_name == "recipes":
                    ...
                    ), f"recipes should be list, got {type(updates[field_name])}"
                elif field_name == "tags":
                    assert isinstance(
                        updates[field_name], set
                    ), f"tags should be set, got {type(updates[field_name])}"
                    ...
                    assert len(domain_tags) == len(
                        api_tags
                    ), f"Tag count mismatch after conversion"
    ...
    def test_frozen_set_handling_comprehensive(self):
        ...
        if "recipes" in updates:
            ...
            assert isinstance(
                api_recipes, list
            ), f"API recipes should be list, got {type(api_recipes)}"
            assert isinstance(
                domain_recipes, list
            ), f"Domain recipes should be list, got {type(domain_recipes)}"
            assert len(domain_recipes) == len(
                api_recipes
            ), "Recipe count should be preserved"

            # Verify no duplicates (frozenset property maintained)
            recipe_ids = [recipe.id for recipe in domain_recipes]
            assert len(set(recipe_ids)) == len(
                recipe_ids
            ), "Recipe IDs should be unique (frozenset property)"
        ...
        if "tags" in updates:
            ...
            assert isinstance(
                domain_tags, set
            ), f"Domain tags should be set, got {type(domain_tags)}"
            ...
            assert len(set(tag_keys)) == len(
                tag_keys
            ), "Tag keys should be unique (frozenset property)"
    ...
    def test_frozen_set_handling_edge_cases(self):
        ...
        if "recipes" in updates:
            assert isinstance(
                updates["recipes"], list
            ), "Empty recipes should still be list"
        ...
        if simple_meal.recipes or simple_meal.tags:
            ...
            if simple_updates.get("recipes"):
                assert isinstance(
                    simple_updates["recipes"], list
                ), "Single-item recipes should be list"
                assert (
                    len(simple_updates["recipes"]) >= 1
                ), "Should have at least one recipe"

            if simple_updates.get("tags"):
                assert isinstance(
                    simple_updates["tags"], set
                ), "Single-item tags should be set"
    ...
    def test_frozen_set_recipe_id_uniqueness(self):
        ...
        assert len(set(api_recipe_ids)) == len(
            api_recipe_ids
        ), "API recipe IDs should be unique"
        ...
        if "recipes" in updates:
            ...
            assert len(set(domain_recipe_ids)) == len(
                domain_recipe_ids
            ), "Domain recipe IDs should remain unique"
            assert len(domain_recipe_ids) == len(
                api_recipe_ids
            ), "Recipe count should be preserved"

            # Verify same IDs are preserved
            assert set(domain_recipe_ids) == set(
                api_recipe_ids
            ), "Recipe IDs should be exactly preserved"
        ...
        if max_meal.tags and "tags" in updates:
            ...
            assert len(set(domain_tag_keys)) == len(
                domain_tag_keys
            ), "Domain tag keys should be unique"
            assert set(domain_tag_keys) == set(
                api_tag_keys
            ), "Tag keys should be exactly preserved"
