...
class TestApiUpdateMealFromApiMeal:
    ...
    def test_complex_meal_conversion(self):
        ...
        original_tags = {
            (tag.key, tag.value, tag.author_id) for tag in complex_meal.tags
        }
        update_tags = {
            (tag.key, tag.value, tag.author_id) for tag in update_meal.updates.tags
        }
        assert original_tags == update_tags
    ...
    def test_all_realistic_meal_scenarios(self, scenario_data):
        ...
        assert (
            update_meal.meal_id == api_meal.id
        ), f"Failed for scenario: {scenario_data['name']}"
        assert (
            update_meal.updates.name == api_meal.name
        ), f"Failed for scenario: {scenario_data['name']}"
        assert (
            update_meal.updates.description == api_meal.description
        ), f"Failed for scenario: {scenario_data['name']}"
        assert (
            update_meal.updates.notes == api_meal.notes
        ), f"Failed for scenario: {scenario_data['name']}"
        assert (
            update_meal.updates.like == api_meal.like
        ), f"Failed for scenario: {scenario_data['name']}"
        assert (
            update_meal.updates.menu_id == api_meal.menu_id
        ), f"Failed for scenario: {scenario_data['name']}"

        # Verify image_url remains as HttpUrl (no conversion to string)
        assert (
            update_meal.updates.image_url == api_meal.image_url
        ), f"Failed for scenario: {scenario_data['name']}"
        ...
        assert update_meal.updates.recipes is not None
        assert update_meal.updates.tags is not None
        assert len(update_meal.updates.recipes) == len(
            api_meal.recipes
        ), f"Failed for scenario: {scenario_data['name']}"
        assert len(update_meal.updates.tags) == len(
            api_meal.tags
        ), f"Failed for scenario: {scenario_data['name']}"
        ...
        for j, recipe in enumerate(update_meal.updates.recipes):
            ...
            assert (
                recipe.id == original_recipe.id
            ), f"Failed for scenario: {scenario_data['name']}, recipe {j}"
            assert (
                recipe.name == original_recipe.name
            ), f"Failed for scenario: {scenario_data['name']}, recipe {j}"
            assert (
                recipe.meal_id == original_recipe.meal_id
            ), f"Failed for scenario: {scenario_data['name']}, recipe {j}"
    ...
    def test_meal_collection_fixtures(self, meal_index):
        ...
        assert (
            update_meal.meal_id == api_meal.id
        ), f"Failed for meal {meal_index} in collection"
        assert (
            update_meal.updates.name == api_meal.name
        ), f"Failed for meal {meal_index} in collection"
        assert (
        ...
        assert isinstance(
            update_meal, ApiUpdateMeal
        ), f"Failed for meal {meal_index} in collection"
        assert isinstance(
            update_meal.updates, ApiAttributesToUpdateOnMeal
        ), f"Failed for meal {meal_index} in collection"
    ...
    def test_validate_created_api_update_meal_instances(self, meal_factory):
        ...
        assert isinstance(
            update_meal.updates, ApiAttributesToUpdateOnMeal
        ), "Updates must be ApiAttributesToUpdateOnMeal instance"
        ...
        assert hasattr(
            update_meal.updates, "menu_id"
        ), "Updates must have menu_id attribute"
        ...
        for field in optional_fields:
            ...
            if field_value is not None:
                if field == "image_url":
                    ...
                    assert isinstance(
                        field_value, bool
                    ), f"{field} must be boolean when not None"

        # Validate collections in updates
        assert hasattr(
            update_meal.updates, "recipes"
        ), "Updates must have recipes attribute"
        ...
        for recipe in update_meal.updates.recipes:
            assert isinstance(
                recipe, ApiRecipe
            ), "Each recipe must be ApiRecipe instance"
            ...
            assert (
                recipe.meal_id == update_meal.meal_id
            ), "Recipe meal_id must match update meal_id"
    ...
    def test_validate_field_completeness(self):
        ...
        assert expected_fields_set.issubset(
            actual_fields
        ), "Missing required fields in ApiAttributesToUpdateOnMeal"
    ...
    def test_validate_conversion_completeness(self, meal_factory):
        ...
        if api_meal.image_url is not None:
            ...
            assert (
                update_meal.updates.image_url is None
        ...
        assert len(update_meal.updates.recipes) == len(
            api_meal.recipes
        ), "recipes collection size changed"
        assert len(update_meal.updates.tags) == len(
            api_meal.tags
        ), "tags collection size changed"

        # Verify recipe data preservation
        for i, (original_recipe, update_recipe) in enumerate(
            zip(api_meal.recipes, update_meal.updates.recipes, strict=False)
        ):
            assert (
                original_recipe.id == update_recipe.id
            ), f"recipe {i} id conversion failed"
            assert (
                original_recipe.name == update_recipe.name
            ), f"recipe {i} name conversion failed"
            assert (
                original_recipe.meal_id == update_recipe.meal_id
            ), f"recipe {i} meal_id conversion failed"
