...
class TestApiMealValidationPerformance:
    ...
    def test_field_validation_performance_with_recipe_count(self, recipe_count):
        ...
        ), f"Validation with {recipe_count} recipes should complete in <1s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 50
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"

        # Performance scales linearly with recipe count (allow some overhead)
        expected_max_time = (
            0.01 * recipe_count + 0.1
        )  # 10ms per recipe + 100ms overhead
        assert (
            metrics.execution_time < expected_max_time
        ), f"Performance should scale linearly with recipe count"
    ...
    def test_field_validation_performance_with_tag_count(self, tag_count):
        ...
        tags = [
            ApiTag(
                key=f"test={i}", value=f"value={i}", type="meal", author_id=author_id
            )
        ...
        ), f"Validation with {tag_count} tags should complete in <0.5s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 30
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
        ...
        assert (
            metrics.execution_time < expected_max_time
        ), f"Performance should scale linearly with tag count"
    ...
    def test_field_validation_performance_with_complexity(self, complexity):
        ...
        assert (
            metrics.execution_time < expected_max_time
        ), f"Validation for {complexity} meal should complete in <{expected_max_time}s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 100
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
    ...
    def test_json_serialization_performance(self):
        ...
        assert (
            metrics.execution_time < 0.5
        ), f"JSON serialization should complete in <0.5s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 50
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
    ...
    def test_json_deserialization_performance(self):
        ...
        assert (
            metrics.execution_time < 1.0
        ), f"JSON deserialization should complete in <1.0s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 100
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
...
class TestApiMealConversionPerformance:
    ...
    @pytest.mark.parametrize("recipe_count", [1, 5, 10, 25, 50])
    def test_from_domain_performance_with_recipe_count(
        self, recipe_count, domain_meal_factory
    ):
        ...
        assert (
            metrics.execution_time < expected_max_time
        ), f"from_domain with {recipe_count} recipes should complete in <{expected_max_time}s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 50
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
    ...
    def test_to_domain_performance_with_recipe_count(self, recipe_count):
        ...
        # Performance assertions
        expected_max_time = (
            0.015 * recipe_count + 0.15
        )  # 15ms per recipe + 150ms overhead (domain conversion is more complex)
        assert (
            metrics.execution_time < expected_max_time
        ), f"to_domain with {recipe_count} recipes should complete in <{expected_max_time}s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 75
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"

    @pytest.mark.parametrize("recipe_count", [1, 5, 10, 25, 50])
    def test_from_orm_model_performance_with_recipe_count(
        self, recipe_count, orm_meal_factory
    ):
        ...
        # Performance assertions
        expected_max_time = (
            0.012 * recipe_count + 0.12
        )  # 12ms per recipe + 120ms overhead
        assert (
            metrics.execution_time < expected_max_time
        ), f"from_orm_model with {recipe_count} recipes should complete in <{expected_max_time}s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 60
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
    ...
    def test_to_orm_kwargs_performance_with_recipe_count(self, recipe_count):
        ...
        # Performance assertions
        expected_max_time = (
            0.008 * recipe_count + 0.08
        )  # 8ms per recipe + 80ms overhead
        assert (
            metrics.execution_time < expected_max_time
        ), f"to_orm_kwargs with {recipe_count} recipes should complete in <{expected_max_time}s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 40
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
    ...
    def test_conversion_methods_performance_by_complexity(self, complexity):
        ...
        assert (
            metrics.execution_time < expected_max_time
        ), f"Conversion cycle for {complexity} meal should complete in <{expected_max_time}s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 150
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
    ...
    def test_nested_objects_conversion_performance(self):
        ...
        for _ in range(20):  # Run multiple times for better averaging
            ...
            if api_meal.nutri_facts and complex_meal.nutri_facts:
                assert (
                    api_meal.nutri_facts.calories.value
                    == complex_meal.nutri_facts.calories.value
                )
        ...
        assert (
            metrics.execution_time < 1.0
        ), f"Nested object conversion should complete in <1.0s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 100
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
    ...
    def test_computed_properties_performance(self):
        ...
        assert (
            metrics.execution_time < 0.5
        ), f"Computed properties calculation should complete in <0.5s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 50
        ), f"Memory usage should be reasonable, used {metrics.memory_delta:.2f}MB"
...
class TestApiMealMemoryPerformance:
    ...
    def test_memory_usage_with_increasing_recipe_count(self):
        ...
        for i in range(1, len(memory_usage_data)):
            ...
            memory_growth_ratio = curr_data["memory_delta"] / max(
                prev_data["memory_delta"], 1
            )
            ...
            assert (
                memory_growth_ratio < recipe_growth_ratio * 3
    ...
    def test_memory_efficiency_during_round_trip_conversions(self):
        ...
        assert (
            metrics.memory_delta < 200
        ), f"Memory usage during round-trip conversions should be <200MB, used {metrics.memory_delta:.2f}MB"
        assert (
            metrics.execution_time < 2.0
        ), f"Round-trip conversions should complete in <2.0s, took {metrics.execution_time:.3f}s"
    ...
    def test_memory_cleanup_after_operations(self):
        ...
        assert (
            memory_growth < 50
        ), f"Memory growth after cleanup should be <50MB, grew {memory_growth:.2f}MB"
...
class TestApiMealEdgeCasePerformance:
    ...
    def test_empty_collections_performance(self):
        ...
        ), f"Empty collections should be very fast, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 10
        ), f"Empty collections should use minimal memory, used {metrics.memory_delta:.2f}MB"
    ...
    def test_maximum_collections_performance(self):
        ...
        assert (
            metrics.execution_time < 2.0
        ), f"Maximum collections should complete in <2.0s, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 200
        ), f"Maximum collections should use reasonable memory, used {metrics.memory_delta:.2f}MB"
    ...
    def test_null_optional_fields_performance(self):
        ...
        ), f"Null fields should be fast, took {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_delta < 20
        ), f"Null fields should use minimal memory, used {metrics.memory_delta:.2f}MB"
    ...
    def test_concurrent_operations_performance(self):
        ...
        avg_execution_time = sum(result["execution_time"] for result in results) / len(
            results
        )

        assert (
            max_execution_time < 2.0
        ), f"Max execution time under concurrent load should be <2.0s, was {max_execution_time:.3f}s"
        assert (
            avg_execution_time < 0.5
        ), f"Average execution time under concurrent load should be <0.5s, was {avg_execution_time:.3f}s"
