        ...
        complex_meal = create_complex_api_meal(
            ...
            tags=frozenset(
                [
                    ...
                    create_api_tag(
                        author_id=author_id, type="meal", key="meal_type", value="lunch"
                    ),
                ]
            ),
        )
...
class TestApiMealJSONValidationErrors:
            ...
            for key, value in invalid_data.items():
                try:
                    ...
                    if key in ["id", "name", "author_id"]:
                        meal_dict[key] = (
                            None  # Set to invalid value to trigger validation error
                        )
            json_str = json.dumps(meal_dict)
    ...
    def test_strict_validation_prevents_coercion_in_json(
        self, field_name, coercion_value
    ):
        ...
        if isinstance(coercion_value, str):
            ...
            json_str = json_str.replace(
                f'"{field_name}":{json_current_value}',
                f'"{field_name}":"{coercion_value}"',
            )
        else:
    ...
    @pytest.mark.parametrize(
        "scenario_name,test_case", _generate_nested_object_validation_test_cases()
    )
    def test_nested_object_validation_errors(self, scenario_name, test_case):
        ...
        try:
            assert (
                problematic_field in error_details
                or "validation" in error_details.lower()
            )
...
class TestApiMealJSONFieldValidation:
    ...
    @pytest.mark.parametrize(
        "validation,test_scenarios", create_field_validation_test_suite().items()
    )
...
class TestApiMealJSONPydanticConfiguration:
    ...
    @pytest.mark.parametrize(
        "config_test", create_pydantic_config_test_cases().values()
    )
    ...
    def test_parametrized_json_edge_cases(self, edge_case):
        ...
        if expected_behavior == "should_pass":
            ...
            if expected_behavior.get("raises"):
                ...
                for field, expected_value in expected_behavior.get(
                    "field_values", {}
                ).items():
