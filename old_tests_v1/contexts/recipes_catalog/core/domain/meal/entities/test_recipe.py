...
class TestRecipeCharacterisation:
    ...
    @pytest.mark.xfail(
        reason="Shared @lru_cache bug - different recipes may share cache"
    )
    ...
    def test_recipe_nutrition_facts_behavior(self):
        ...
        meal = Meal.create_meal(
            name="Test Meal", author_id="user123", meal_id="meal123", menu_id="menu123"
        )
...
class TestRecipeCacheInvalidation:
    ...
    @pytest.mark.xfail(
        reason="Cache invalidation bug - @lru_cache not invalidated on ratings change"
    )
    ...
    @pytest.mark.xfail(
        reason="Cache invalidation bug - @lru_cache not invalidated on nutrition change"
    )
