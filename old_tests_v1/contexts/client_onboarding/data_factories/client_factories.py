...
def create_form_response_received_event_kwargs(**kwargs) -> dict[str, Any]:
    ...
    default_response_data = {
        ...
        "form_response": {
            ...
            "landed_at": (
                base_time + timedelta(minutes=float(response_counter))
            ).isoformat(),
            "submitted_at": (
                base_time + timedelta(minutes=float(response_counter), seconds=30)
            ).isoformat(),
            "answers": [
                {
    ...
    final_kwargs = {
        ...
        "typeform_response_id": kwargs.get(
            "typeform_response_id", f"typeform_resp_{response_counter:06d}"
        ),
        "response_data": kwargs.get("response_data", default_response_data),
        "webhook_timestamp": kwargs.get(
            "webhook_timestamp",
            (base_time + timedelta(minutes=float(response_counter))).isoformat(),
        ),
...
def create_client_data_extracted_event_kwargs(**kwargs) -> dict[str, Any]:
    ...
    final_kwargs = {
        ...
        "extraction_timestamp": kwargs.get(
...
def create_onboarding_form_webhook_setup_event_kwargs(**kwargs) -> dict[str, Any]:
    ...
    final_kwargs = {
        ...
        "webhook_url": kwargs.get(
            "webhook_url", f"https://api.example.com/webhooks/setup/{webhook_counter}"
        ),
...
def create_onboarding_form_kwargs(**kwargs) -> dict[str, Any]:
    ...
    final_kwargs = {
        ...
        "typeform_id": kwargs.get("typeform_id", f"typeform_{form_counter:03d}"),
        "webhook_url": kwargs.get(
            "webhook_url", f"https://api.example.com/webhooks/form/{form_counter}"
        ),
        "status": kwargs.get("status", OnboardingFormStatus.ACTIVE),
        "created_at": kwargs.get(
            "created_at", base_time + timedelta(hours=float(form_counter))
        ),
        "updated_at": kwargs.get(
            "updated_at",
            base_time + timedelta(hours=float(form_counter), minutes=float(30)),
        ),
    }
...
def create_restaurant_onboarding_form(**kwargs) -> OnboardingForm:
    ...
    final_kwargs = {
        ...
        "webhook_url": kwargs.get(
            "webhook_url", "https://api.example.com/webhooks/restaurant"
        ),
...
def create_catering_onboarding_form(**kwargs) -> OnboardingForm:
    ...
    final_kwargs = {
        ...
        "webhook_url": kwargs.get(
            "webhook_url", "https://api.example.com/webhooks/catering"
        ),
        **{k: v for k, v in kwargs.items() if k not in ["typeform_id", "webhook_url"]},
    }
    return create_onboarding_form(**final_kwargs)
