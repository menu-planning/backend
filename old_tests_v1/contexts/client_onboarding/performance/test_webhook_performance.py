            ...
            field_responses.append(
                {
                    ...
                    "text": (
                        f"Response value {i} for payload {self.payload_count}"
                        if i % 2 == 0
...
class TestWebhookProcessingPerformance:
    ...
    async def test_single_webhook_processing_latency(self):
        ...
        ), f"Processing latency {processing_latency_ms:.2f}ms, expected < 50ms"
    ...
    async def test_concurrent_webhook_processing_performance(self):
            ...
            for i in range(webhooks_per_request):
                ...
                try:
                    ...
                    batch_results.append(
                        {
                            "success": success,
                            "result": {"error": error, "response_id": response_id},
                        }
                    )
        ...
        total_processed = sum(
            batch["webhooks_processed"] for batch in successful_batches
        )
        ...
        assert (
            len(successful_batches) == concurrent_requests
        ), f"Only {len(successful_batches)}/{concurrent_requests} batches successful"
        assert (
            total_processed == total_webhooks
        ), f"Processed {total_processed}/{total_webhooks} webhooks"
        ...
        assert (
            processing_rate > 100
        ), f"Processing rate {processing_rate:.1f}/s, expected > 100/s"

        # Concurrent processing should complete reasonably quickly
        assert (
            total_duration < 30.0
        ), f"Total duration {total_duration:.2f}s, expected < 30s"
        ...
        print(
            f"   {total_processed} webhooks in {total_duration:.2f}s ({processing_rate:.1f}/s)"
        )
    ...
    async def test_signature_verification_performance_under_load(self):
        ...
        assert (
            successful_verifications == verification_count
        ), f"Only {successful_verifications}/{verification_count} signatures verified"

        # Signature verification should be fast
        assert (
            verification_rate > 500
        ), f"Verification rate {verification_rate:.1f}/s, expected > 500/s"
        assert (
            avg_verification_time_ms < 2.0
        ), f"Average verification time {avg_verification_time_ms:.2f}ms, expected < 2ms"

        print(f"✅ Signature verification performance:")
        print(
            f"   {verification_count} verifications in {verification_duration:.3f}s ({verification_rate:.1f}/s)"
        )
    ...
    async def test_database_storage_performance_under_load(self):
        ...
        for i in range(storage_count):
            webhook_data = self.payload_generator.generate_webhook_payload(
                form_id=form_id, field_count=8  # Moderate complexity
            )
        ...
        assert (
            successful_storage == storage_count
        ), f"Only {successful_storage}/{storage_count} webhooks stored"

        # Storage should be efficient
        assert storage_rate > 50, f"Storage rate {storage_rate:.1f}/s, expected > 50/s"
        assert (
            avg_storage_time_ms < 20.0
        ), f"Average storage time {avg_storage_time_ms:.2f}ms, expected < 20ms"
        ...
        print(
            f"   {storage_count} responses stored in {storage_duration:.2f}s ({storage_rate:.1f}/s)"
        )
    ...
    async def test_memory_usage_under_sustained_webhook_load(self):
        ...
        for round_num in range(rounds):
            ...
            print(
                f"   Round {round_num + 1}: {webhooks_per_round} webhooks in {round_duration:.2f}s, "
        ...
        if len(memory_samples) > 1:
            ...
            assert (
                memory_variance < 200
            ), f"Memory variance {memory_variance:.1f}, expected stable usage"
...
class TestWebhookPerformanceStress:
    ...
    async def test_extreme_concurrent_webhook_processing(self):
        ...
        successful_webhooks = sum(sum(task_result) for task_result in successful_tasks)

        # Stress test assertions
        assert (
            len(successful_tasks) == concurrent_tasks
        ), f"Only {len(successful_tasks)}/{concurrent_tasks} tasks completed"
        assert (
            total_processed == total_webhooks
        ), f"Processed {total_processed}/{total_webhooks} webhooks"
        ...
        success_rate = (
            (successful_webhooks / total_processed) * 100 if total_processed > 0 else 0
        )
        assert (
            success_rate > 90.0
        ), f"Success rate {success_rate:.1f}% under stress, expected > 90%"

        # Should maintain reasonable performance under stress
        processing_rate = total_processed / stress_duration
        assert (
            processing_rate > 50
        ), f"Stress processing rate {processing_rate:.1f}/s, expected > 50/s"
    ...
    async def test_large_payload_processing_performance(self):
        ...
        for i in range(large_payload_count):
            webhook_data = self.payload_generator.generate_webhook_payload(
                form_id=form_id, field_count=fields_per_payload
            )
        ...
        assert (
            successful_processing == large_payload_count
        ), f"Only {successful_processing}/{large_payload_count} large payloads processed"

        # Should maintain reasonable performance with large payloads
        assert (
            processing_rate > 10
        ), f"Large payload processing rate {processing_rate:.1f}/s, expected > 10/s"
        assert (
            avg_processing_time_ms < 100
        ), f"Average large payload processing time {avg_processing_time_ms:.1f}ms, expected < 100ms"

        print(f"✅ Large payload processing performance:")
        print(
            f"   {large_payload_count} payloads with {fields_per_payload} fields each"
        )
