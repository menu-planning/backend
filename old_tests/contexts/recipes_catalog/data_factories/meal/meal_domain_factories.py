...
COMMON_MEAL_TYPES = [
    ...
    "appetizer", "side_dish", "main_course", "light_meal", "heavy_meal"
]
...
CUISINE_TYPES = [
    ...
    "fusion", "traditional", "contemporary"
]

DIETARY_TAGS = [
    "vegetarian", "vegan", "gluten-free", "dairy-free", "low-carb", "keto", 
    "paleo", "pescatarian", "healthy", "light", "hearty", "comfort-food",
    "low-calorie", "high-protein", "plant-based", "organic", "whole-food",
    "raw", "sugar-free", "low-sodium"
]

DIFFICULTY_LEVELS = ["easy", "medium", "hard"]

OCCASION_TYPES = [
    "everyday", "quick", "family", "date-night", "entertaining", "holiday",
    "special", "casual", "formal", "outdoor", "picnic", "party", "romantic",
    "festive", "comfort", "seasonal", "summer", "winter", "spring", "fall"
]

MEAL_STYLES = [
    "healthy", "comfort", "elegant", "rustic", "modern", "traditional",
    "fusion", "simple", "complex", "gourmet", "casual", "fine-dining",
    "homestyle", "restaurant-style", "fresh", "hearty", "light"
]
...
def generate_meal_like_value(counter: Optional[int] = None) -> bool:
    """
...
def create_recipe_for_meal(meal_id: str, author_id: str, recipe_type: str = "main", **kwargs) -> Any:
    """
    ...
    if recipe_type == "appetizer":
        ...
        recipe_kwargs.setdefault("name", f"{recipe_type.title()} Recipe for Meal {meal_id[-8:]}")
        return create_recipe(**recipe_kwargs)


def create_multiple_recipes_for_meal(meal_id: str, author_id: str, recipe_count: int = 3, **kwargs) -> List[Any]:
    """
...
def create_recipes_with_invalid_computed_properties(meal_id: str, author_id: str, count: int = 2) -> List[Any]:
    """
...
def create_meal_with_computed_property_issues(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get("description", "Meal for testing computed property edge cases"),
        "notes": kwargs.get("notes", "This meal has recipes with missing data that affects computed properties"),
        "recipes": kwargs.get("recipes", recipes),
        "tags": kwargs.get("tags", {
            create_meal_tag(key="category", value="test", author_id=author_id, type="meal"),
            create_meal_tag(key="purpose", value="computed-properties", author_id=author_id, type="meal"),
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "author_id", "name", "description", "notes", "recipes", "tags"]}
    }
...
def create_meal_with_aggregation_edge_cases(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get("description", "Meal for testing aggregation edge cases and extreme values"),
        "notes": kwargs.get("notes", "This meal has recipes with extreme values to test aggregation logic"),
        "recipes": kwargs.get("recipes", recipes),
        "tags": kwargs.get("tags", {
            create_meal_tag(key="category", value="test", author_id=author_id, type="meal"),
            create_meal_tag(key="purpose", value="aggregation", author_id=author_id, type="meal"),
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "author_id", "name", "description", "notes", "recipes", "tags"]}
    }
...
def create_meal_without_recipes(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get("description", "Meal for testing scenarios without recipes"),
        "notes": kwargs.get("notes", "This meal has no recipes for testing edge cases"),
        "recipes": kwargs.get("recipes", []),
        "tags": kwargs.get("tags", {
            create_meal_tag(key="category", value="test", author_id=author_id, type="meal"),
            create_meal_tag(key="purpose", value="no-recipes", author_id=author_id, type="meal"),
        }),
        **{k: v for k, v in kwargs.items() if k not in ["author_id", "name", "description", "notes", "recipes", "tags"]}
    }
...
def create_low_calorie_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="diet", value="low-calorie", author_id=author_id, type="meal"),
            create_meal_tag(key="category", value="health", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="healthy", author_id=author_id, type="meal")
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
    return create_meal(**final_kwargs)
...
def create_quick_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="difficulty", value="easy", author_id=author_id, type="meal"),
            create_meal_tag(key="occasion", value="quick", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="healthy", author_id=author_id, type="meal")
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
...
def create_vegetarian_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="diet", value="vegetarian", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="healthy", author_id=author_id, type="meal"),
            create_meal_tag(key="category", value="lunch", author_id=author_id, type="meal")
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
    return create_meal(**final_kwargs)
...
def create_high_protein_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="diet", value="high-protein", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="fitness", author_id=author_id, type="meal"),
            create_meal_tag(key="occasion", value="post-workout", author_id=author_id, type="meal")
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
    return create_meal(**final_kwargs)
...
def create_family_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="occasion", value="family", author_id=author_id, type="meal"),
            create_meal_tag(key="difficulty", value="medium", author_id=author_id, type="meal"),
            create_meal_tag(key="category", value="dinner", author_id=author_id, type="meal")
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
    return create_meal(**final_kwargs)
...
def create_simple_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "notes": kwargs.get("notes", "Basic meal with straightforward preparation and familiar flavors."),
        "like": kwargs.get("like", True),
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="difficulty", value="easy", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="simple", author_id=author_id, type="meal"),
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
    return create_meal(**final_kwargs)
...
def create_complex_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get("description", "A sophisticated meal with multiple courses and complex flavors designed for comprehensive testing scenarios"),
        ...
        "tags": kwargs.get("tags", {
            ...
            create_meal_tag(key="occasion", value="special", author_id=author_id, type="meal"),
            create_meal_tag(key="cuisine", value="fusion", author_id=author_id, type="meal"),
            create_meal_tag(key="category", value="dinner", author_id=author_id, type="meal"),
        }),
...
def create_minimal_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "tags": kwargs.get("tags", set()),
        "recipes": kwargs.get("recipes", []),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "image_url", "tags", "recipes", "author_id"]}
    }
    return create_meal(**final_kwargs)
...
def create_meal_with_max_fields(**kwargs) -> Meal:
    ...
    for key, value in tag_combinations:
        tags.add(create_meal_tag(key=key, value=value, author_id=author_id, type="meal"))
    
    final_kwargs = {
        "id": meal_id,
        "author_id": author_id,
        "name": kwargs.get("name", "Complete Meal Experience with Maximum Fields"),
        "description": kwargs.get("description", "A comprehensive meal with all possible fields populated for testing maximum data scenarios and edge cases"),
...
def create_breakfast_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get("description", "Perfect breakfast meal to start the day with sustained energy and nutrition"),
        "notes": kwargs.get("notes", "Balanced breakfast with protein, healthy fats, and complex carbs for sustained energy throughout the morning."),
        "like": kwargs.get("like", True),
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="category", value="breakfast", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="healthy", author_id=author_id, type="meal"),
            create_meal_tag(key="difficulty", value="easy", author_id=author_id, type="meal"),
            create_meal_tag(key="occasion", value="everyday", author_id=author_id, type="meal"),
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
    return create_meal(**final_kwargs)
...
def create_dessert_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get("description", "Indulgent dessert meal perfect for special occasions and sweet cravings"),
        "notes": kwargs.get("notes", "Rich and satisfying dessert with balanced sweetness. Best enjoyed in moderate portions."),
        "like": kwargs.get("like", True),
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="category", value="dessert", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="sweet", author_id=author_id, type="meal"),
            create_meal_tag(key="occasion", value="special", author_id=author_id, type="meal"),
            create_meal_tag(key="diet", value="vegetarian", author_id=author_id, type="meal"),
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
...
def create_vegan_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get("description", "Completely plant-based meal rich in nutrients and satisfying flavors"),
        "notes": kwargs.get("notes", "Nutrient-dense vegan meal with complete amino acids and essential vitamins. Perfect for plant-based lifestyle."),
        "like": kwargs.get("like", True),
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="diet", value="vegan", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="plant-based", author_id=author_id, type="meal"),
            create_meal_tag(key="category", value="healthy", author_id=author_id, type="meal"),
            create_meal_tag(key="difficulty", value="medium", author_id=author_id, type="meal"),
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
...
def create_keto_meal(**kwargs) -> Meal:
    ...
    final_kwargs = {
        ...
        "description": kwargs.get("description", "High-fat, low-carb meal perfect for ketogenic lifestyle"),
        "notes": kwargs.get("notes", "Carefully balanced for ketosis with high-quality fats and minimal carbs. Track macros for best results."),
        "like": kwargs.get("like", True),
        "recipes": recipes,
        "tags": kwargs.get("tags", {
            create_meal_tag(key="diet", value="keto", author_id=author_id, type="meal"),
            create_meal_tag(key="style", value="low-carb", author_id=author_id, type="meal"),
            create_meal_tag(key="category", value="high-fat", author_id=author_id, type="meal"),
            create_meal_tag(key="difficulty", value="medium", author_id=author_id, type="meal"),
        }),
        **{k: v for k, v in kwargs.items() if k not in ["id", "name", "description", "notes", "like", "recipes", "tags", "author_id"]}
    }
...
def create_meals_by_cuisine(cuisine: str, count: int = 3) -> List[Meal]:
    ...
    for i in range(count):
        meal = create_meal(
            ...
            tags={create_meal_tag(key="cuisine", value=cuisine, author_id=author_id, type="meal")}
        )
        meals.append(meal)
    return meals
...
def create_meals_by_difficulty(difficulty: str, count: int = 3) -> List[Meal]:
    ...
    for i in range(count):
        meal = create_meal(
            ...
            tags={create_meal_tag(key="difficulty", value=difficulty, author_id=author_id, type="meal")}
        )
        meals.append(meal)
    return meals
...
def create_meals_by_category(category: str, count: int = 3) -> List[Meal]:
    ...
    for i in range(count):
        meal = create_meal(
            ...
            tags={create_meal_tag(key="category", value=category, author_id=author_id, type="meal")}
        )
        meals.append(meal)
    return meals
...
def create_meals_by_diet(diet: str, count: int = 3) -> List[Meal]:
    ...
    for i in range(count):
        if diet == "keto":
            ...
            meal = create_vegetarian_meal(author_id=author_id, name=f"Vegetarian Meal {i+1}")
        elif diet == "high-protein":
            meal = create_high_protein_meal(author_id=author_id, name=f"High Protein Meal {i+1}")
        elif diet == "low-calorie":
            meal = create_low_calorie_meal(author_id=author_id, name=f"Low Calorie Meal {i+1}")
        else:
            meal = create_meal(
                author_id=author_id,
                name=f"{diet.title()} Meal {i+1}",
                description=f"A {diet} meal for testing",
                tags={create_meal_tag(key="diet", value=diet, author_id=author_id, type="meal")}
            )
        meals.append(meal)
    return meals
...
def create_conversion_performance_dataset(count: int = 100) -> Dict[str, Any]:
    ...
    return {
        "domain_meals": domain_meals,
        "total_count": count
    }


def create_nested_object_validation_dataset(count: int = 50, tags_per_meal: int | None = None, recipes_per_meal: int | None = None) -> List[Meal]:
    """
...
def create_meal_with_nested_recipes_dataset(count: int = 20, min_recipes: int = 1, max_recipes: int = 6) -> List[Meal]:
    """
    ...
    for i in range(count):
        ...
        if i % 8 == 0:
            meal = create_simple_meal(
                author_id=author_id,
                id=meal_id,
                recipes=create_multiple_recipes_for_meal(meal_id, author_id, recipe_count)
            )
        elif i % 8 == 1:
            meal = create_complex_meal(
                author_id=author_id,
...
def create_meal_performance_benchmark(small_count: int = 10, medium_count: int = 50, large_count: int = 100) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "computed_properties_data": create_meal_computed_properties_test_dataset(small_count)
    }
