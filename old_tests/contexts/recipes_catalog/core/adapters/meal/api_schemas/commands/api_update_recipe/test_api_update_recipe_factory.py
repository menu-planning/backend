...
class TestApiUpdateRecipeFromApiRecipe:
    ...
    def test_simple_recipe_conversion(self):
        ...
        if simple_recipe.nutri_facts:
            ...
            assert update_recipe.updates.nutri_facts.calories == simple_recipe.nutri_facts.calories
        else:
        ...
        for ingredient in update_recipe.updates.ingredients:
            ...
            assert any(ing.quantity == ingredient.quantity for ing in simple_recipe.ingredients)
        
        # Verify tags mapping
        original_tags = {(tag.key, tag.value, tag.author_id) for tag in simple_recipe.tags}
        update_tags = {(tag.key, tag.value, tag.author_id) for tag in update_recipe.updates.tags}
    ...
    def test_complex_recipe_conversion(self):
        ...
        if complex_recipe.nutri_facts:
            assert update_recipe.updates.nutri_facts is not None
            assert update_recipe.updates.nutri_facts.calories == complex_recipe.nutri_facts.calories
            assert update_recipe.updates.nutri_facts.protein == complex_recipe.nutri_facts.protein
        ...
        original_ingredients = {(ing.name, ing.quantity, ing.unit) for ing in complex_recipe.ingredients}
        update_ingredients = {(ing.name, ing.quantity, ing.unit) for ing in update_recipe.updates.ingredients}
        assert original_ingredients == update_ingredients
        
        # Verify all tags are correctly mapped
        original_tags = {(tag.key, tag.value, tag.author_id) for tag in complex_recipe.tags}
        update_tags = {(tag.key, tag.value, tag.author_id) for tag in update_recipe.updates.tags}
        assert original_tags == update_tags
    ...
    def test_recipe_with_max_fields_conversion(self):
        ...
        assert update_recipe.updates.weight_in_grams == max_fields_recipe.weight_in_grams
        ...
        if max_fields_recipe.nutri_facts:
            assert update_recipe.updates.nutri_facts is not None
            assert update_recipe.updates.nutri_facts.calories == max_fields_recipe.nutri_facts.calories
            assert update_recipe.updates.nutri_facts.protein == max_fields_recipe.nutri_facts.protein
            assert update_recipe.updates.nutri_facts.carbohydrate == max_fields_recipe.nutri_facts.carbohydrate
            assert update_recipe.updates.nutri_facts.total_fat == max_fields_recipe.nutri_facts.total_fat
        ...
        assert update_recipe.updates.ingredients is not None
        assert update_recipe.updates.tags is not None
        assert len(update_recipe.updates.ingredients) == len(max_fields_recipe.ingredients)
    ...
    def test_recipe_with_incorrect_averages_conversion(self):
        ...
        assert update_recipe.updates.weight_in_grams == recipe_with_computed.weight_in_grams
        ...
        assert update_recipe.updates.ingredients is not None
        assert update_recipe.updates.tags is not None
        assert len(update_recipe.updates.ingredients) == len(recipe_with_computed.ingredients)
    ...
    @pytest.mark.parametrize("field_combination", [
        {"name": "Recipe 1", "description": "Test 1", "notes": None, "total_time": 30},
        {"name": "Recipe 2", "description": None, "notes": "Test 2", "total_time": None},
        {"name": "Recipe 3", "description": "Test 3", "notes": "Test 3", "total_time": 15},
        {"name": "Recipe 4", "description": None, "notes": None, "total_time": None},
    ])
    ...
    def test_field_mapping_preserves_data_types(self):
        ...
        assert type(update_recipe.updates.instructions) == type(test_recipe.instructions)
        
        # Verify optional fields maintain their types (or None)
        if test_recipe.description is not None:
            assert type(update_recipe.updates.description) == type(test_recipe.description)
        else:
        ...
        if test_recipe.total_time is not None:
            assert type(update_recipe.updates.total_time) == type(test_recipe.total_time)
        else:
            assert update_recipe.updates.total_time is None
            
        if test_recipe.weight_in_grams is not None:
            assert type(update_recipe.updates.weight_in_grams) == type(test_recipe.weight_in_grams)
        else:
    ...
    def test_all_realistic_recipe_scenarios(self, scenario_data):
        ...
        assert update_recipe.recipe_id == api_recipe.id, f"Failed for scenario: {scenario_data['name']}"
        assert update_recipe.updates.name == api_recipe.name, f"Failed for scenario: {scenario_data['name']}"
        assert update_recipe.updates.description == api_recipe.description, f"Failed for scenario: {scenario_data['name']}"
        assert update_recipe.updates.instructions == api_recipe.instructions, f"Failed for scenario: {scenario_data['name']}"
        assert update_recipe.updates.notes == api_recipe.notes, f"Failed for scenario: {scenario_data['name']}"
        assert update_recipe.updates.total_time == api_recipe.total_time, f"Failed for scenario: {scenario_data['name']}"
        assert update_recipe.updates.privacy == api_recipe.privacy, f"Failed for scenario: {scenario_data['name']}"
        
        # Verify image_url remains as HttpUrl (no conversion to string)
        assert update_recipe.updates.image_url == api_recipe.image_url, f"Failed for scenario: {scenario_data['name']}"
        ...
        assert update_recipe.updates.ingredients is not None
        assert update_recipe.updates.tags is not None
        assert len(update_recipe.updates.ingredients) == len(api_recipe.ingredients), f"Failed for scenario: {scenario_data['name']}"
        assert len(update_recipe.updates.tags) == len(api_recipe.tags), f"Failed for scenario: {scenario_data['name']}"
        
        # Verify ingredients are correctly mapped
        original_ingredients = {(ing.name, ing.quantity, ing.unit) for ing in api_recipe.ingredients}
        update_ingredients = {(ing.name, ing.quantity, ing.unit) for ing in update_recipe.updates.ingredients}
        assert original_ingredients == update_ingredients, f"Failed for scenario: {scenario_data['name']}"
    ...
    def test_recipe_collection_fixtures(self, recipe_index):
        ...
        # Verify basic structure
        assert update_recipe.recipe_id == api_recipe.id, f"Failed for recipe {recipe_index} in collection"
        assert update_recipe.updates.name == api_recipe.name, f"Failed for recipe {recipe_index} in collection"
        assert update_recipe.updates.instructions == api_recipe.instructions, f"Failed for recipe {recipe_index} in collection"
        ...
        assert update_recipe.updates.ingredients is not None
        assert update_recipe.updates.tags is not None
        assert len(update_recipe.updates.ingredients) == len(api_recipe.ingredients), f"Failed for recipe {recipe_index} in collection"
        assert len(update_recipe.updates.tags) == len(api_recipe.tags), f"Failed for recipe {recipe_index} in collection"
        
        # Verify instance type
        assert isinstance(update_recipe, ApiUpdateRecipe), f"Failed for recipe {recipe_index} in collection"
        assert isinstance(update_recipe.updates, ApiAttributesToUpdateOnRecipe), f"Failed for recipe {recipe_index} in collection"
    ...
    def test_validate_created_api_update_recipe_instances(self, recipe_factory):
        ...
        assert isinstance(update_recipe, ApiUpdateRecipe), "Must be ApiUpdateRecipe instance"
        ...
        assert isinstance(update_recipe.updates, ApiAttributesToUpdateOnRecipe), "Updates must be ApiAttributesToUpdateOnRecipe instance"
        ...
        assert update_recipe.recipe_id.count('-') == 4, "recipe_id must be valid UUID format"
        
        # Validate required fields in updates
        assert hasattr(update_recipe.updates, 'name'), "Updates must have name attribute"
        assert hasattr(update_recipe.updates, 'instructions'), "Updates must have instructions attribute"
        assert isinstance(update_recipe.updates.name, str), "Name must be string"
        assert isinstance(update_recipe.updates.instructions, str), "Instructions must be string"
        assert len(update_recipe.updates.name.strip()) > 0, "Name must not be empty"
        assert len(update_recipe.updates.instructions.strip()) > 0, "Instructions must not be empty"
        
        # Validate optional fields in updates
        optional_fields = ['description', 'notes', 'utensils', 'total_time', 'weight_in_grams', 'image_url', 'privacy']
        for field in optional_fields:
            assert hasattr(update_recipe.updates, field), f"Updates must have {field} attribute"
            ...
            if field_value is not None:
                if field == 'image_url':
                    ...
                    assert isinstance(field_value, int), f"{field} must be int when not None"
                elif field == 'privacy':
                    from src.contexts.shared_kernel.domain.enums import Privacy
                    assert isinstance(field_value, Privacy), f"{field} must be Privacy enum when not None"
        
        # Validate collections in updates
        assert hasattr(update_recipe.updates, 'ingredients'), "Updates must have ingredients attribute"
        assert hasattr(update_recipe.updates, 'tags'), "Updates must have tags attribute"
        assert isinstance(update_recipe.updates.ingredients, frozenset), "Ingredients must be frozenset"
        assert isinstance(update_recipe.updates.tags, frozenset), "Tags must be frozenset"
        
        # Validate ingredient instances in collection
        for ingredient in update_recipe.updates.ingredients:
            assert isinstance(ingredient, ApiIngredient), "Each ingredient must be ApiIngredient instance"
            assert hasattr(ingredient, 'name'), "Ingredient must have name attribute"
            assert hasattr(ingredient, 'quantity'), "Ingredient must have quantity attribute"
    ...
    def test_validate_field_completeness(self):
        ...
        expected_fields = [
            'name', 'description', 'instructions', 'notes', 'utensils', 
            'total_time', 'weight_in_grams', 'image_url', 'privacy', 
            'ingredients', 'tags', 'nutri_facts'
        ]
        ...
        assert expected_fields_set.issubset(actual_fields), "Missing required fields in ApiAttributesToUpdateOnRecipe"
    ...
    def test_validate_conversion_completeness(self, recipe_factory):
        ...
        assert update_recipe.updates.description == api_recipe.description, "description conversion failed"
        assert update_recipe.updates.instructions == api_recipe.instructions, "instructions conversion failed"
        assert update_recipe.updates.notes == api_recipe.notes, "notes conversion failed"
        assert update_recipe.updates.utensils == api_recipe.utensils, "utensils conversion failed"
        assert update_recipe.updates.total_time == api_recipe.total_time, "total_time conversion failed"
        assert update_recipe.updates.weight_in_grams == api_recipe.weight_in_grams, "weight_in_grams conversion failed"
        assert update_recipe.updates.privacy == api_recipe.privacy, "privacy conversion failed"
        
        # Verify image_url remains as HttpUrl (no conversion to string)
        if api_recipe.image_url is not None:
            assert update_recipe.updates.image_url == api_recipe.image_url, "image_url conversion failed"
        else:
            assert update_recipe.updates.image_url is None, "image_url None conversion failed"
        
        # Verify nutri_facts preservation
        if api_recipe.nutri_facts is not None:
            assert update_recipe.updates.nutri_facts is not None, "nutri_facts should not be None"
            assert update_recipe.updates.nutri_facts.calories == api_recipe.nutri_facts.calories, "nutri_facts calories conversion failed"
        else:
            assert update_recipe.updates.nutri_facts is None, "nutri_facts None conversion failed"
        ...
        assert update_recipe.updates.ingredients is not None
        assert update_recipe.updates.tags is not None
        assert len(update_recipe.updates.ingredients) == len(api_recipe.ingredients), "ingredients collection size changed"
        assert len(update_recipe.updates.tags) == len(api_recipe.tags), "tags collection size changed"
        
        # Verify ingredient data preservation
        original_ingredients = {(ing.name, ing.quantity, ing.unit) for ing in api_recipe.ingredients}
        update_ingredients = {(ing.name, ing.quantity, ing.unit) for ing in update_recipe.updates.ingredients}
        assert original_ingredients == update_ingredients, "ingredients data conversion failed"
